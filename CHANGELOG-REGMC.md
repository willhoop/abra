...
- The 0.96.0 comment claiming every `.map()` / `.filter()` write clears the added type is corrected: those two
...
### Notes
- Filed in 0.97.0's report §6. Roost, the only other type removal, goes through the `Type` event and never touches
  `addedType`; it is already carried by hand (0.96.0). Report `docs/_reports/2026-09-24-burnup-added-type.md`.
...
### Notes
- Filed in 0.95.0's report as gap 2. Both added-type moves (Trick-or-Treat, Forest's Curse) are legal in both
