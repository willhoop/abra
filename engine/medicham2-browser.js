...
function dmgRange(att,def,mv,field,spread){
  ...
  const roll=r=>{let d=Math.floor(base*r/100);if(stab!==1)d=Math.floor(d*stab);d=Math.floor(d*eff);if(burn<1)d=Math.floor(d*burn);if(lo>1)d=Math.floor(d*lo);return d;};
