...
const SOURCES = [
  ...
  /* 2026-09-24 (ENGINE, abra/regmc 0.87.0) -- THE SOLVER-FACING WRAPPER, frozen in the commit that lands it
...
/* ================================================================================================
 * WHEN THE DIGEST MOVES, SAY WHY — 2026-09-05
 * ================================================================================================
 *
 * THE RECEIPT, TWICE IN EIGHT DAYS. 2026-08-28 09:58Z->10:06Z the MEDICHAM gate fell from 7 of 8
 * clauses PASS to 5 of 8 with no code change; 2026-09-04/05 it read 7 FAILING of 9 and an agent
 * re-ran five heavy clauses to restore it. Both times the cause was the same and neither run could
 * see it: `engine/medicham2-browser.js` had its LINE ENDINGS changed. All 26 frozen sources were
 * content-identical — `diff --strip-trailing-cr` reported zero differences. The id moved, every
 * pinned artifact was correctly stranded, and five heavy re-runs bought the discovery that nothing
 * had changed.
 *
 * WHAT THIS IS NOT, because both of the obvious doors are shut on purpose:
 *
 *   NOT A PIN. `.gitattributes:70-77` names nine sources — medicham2-browser.js, rollout_leaf.js,
 *   position_features.js, engine/tags.js, mc_key.js, set_priors.js, smogon_priors.js,
 *   data/quality-filter.json, data/engine-data.js — as CRLF in the working tree, and states that
 *   pinning them to LF would rewrite them, move every release id, and break `tests/roster.js`, whose
 *   red demonstrations match `\r\n` against the simulator's source. That is a separate, larger job
 *   and it is filed, not done here.
 *
 *   NOT A NORMALISED DIGEST. `treeDigest()` above hashes RAW BYTES and must keep doing so. This
 *   repository's own record: *never normalise the comparator to hide it — the difference is already
 *   observable to an instrument.* An artifact pinned to bytes that no longer exist stays stranded.
 *
 * SO THIS IS THE THIRD THING: A DIAGNOSIS, AND IT EXCUSES NOTHING. The id still moves. `drift()`
 * still lists the file. `open()` still refuses a modified snapshot. `pin_guard.js` still WITHHOLDS
 * every figure in an artifact measured against other bytes. What changes is that the refusal now
 * carries the CAUSE, so the next caller spends seconds rather than five heavy runs.
 *
 * WHY A TRAILING-CR-INSENSITIVE COMPARISON IS SAFE HERE AND WOULD NOT BE AS THE DIGEST.
 * The digest is an IDENTITY: it decides whether a measurement may be reused, and a wrong answer
 * there publishes a number about bytes nobody ran. This comparison DECIDES NOTHING. It changes no
 * id, un-strands no artifact, permits no re-use, and gates nothing; it only labels a difference that
 * has ALREADY been detected by the raw-byte digest. A wrong label costs one misleading sentence; a
 * wrong digest costs a void measurement. That asymmetry is the whole argument, and it is why the
 * normalisation lives in the reporter and never in `sha12`.
 *
 * AND IT IS A PROPERTY, NOT A LIST. Nothing below reads a filename, a path, a `.gitattributes`
 * entry, or SOURCES. It is handed two byte strings. A tenth CRLF source added tomorrow, a data file
 * regenerated by a Windows tool, a JSON artifact rewritten through a different writer — all are
 * diagnosed identically with no edit here. Will's standing test is *would this catch a second
 * instance, spelled differently, through another door?*, and a list of the nine answers no.
 *
 * WHAT `eol-only` DOES AND DOES NOT CLAIM, said out loud rather than implied. It claims the two byte
 * strings are equal once every line terminator is normalised — i.e. NOBODY EDITED A CHARACTER. It
 * does NOT claim no number can change: a line terminator inside a template literal or a string is
 * semantic, and `tests/roster.js` matches `\r\n` against the simulator's source deliberately. So the
 * honest reading is "the digest moved and the source was not edited", which is exactly the fact the
 * five re-runs were missing — not "carry on, it is fine". The summary says so in those words.
 */

/* AND IT DELIBERATELY DOES NOT GO THROUGH `engine/read_text.js`, which is this repository's one
 * implementation of "read a text file whatever its line endings". That is the right door everywhere
 * else and the wrong one here: it exists to make the distinction invisible, and the distinction is
 * precisely what is being measured. Reusing it would answer `same` for every case this function was
 * built to tell apart.
 *
 * latin1 is a byte-exact round trip, so this is a comparison of BYTES wearing a string's clothes.
 * CR (0x0D) and LF (0x0A) are both below 0x80, so the normalisation cannot be confused by a
 * multi-byte UTF-8 sequence — decoding as utf8 could, which is why it does not. */
const asBytes = buf => buf.toString('latin1');
const eolNorm = s => s.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
/* LOOKBEHIND, NOT A CONSUMING CHARACTER CLASS, AND THE FIRST VERSION GOT THIS WRONG IN THE ONE
 * DIRECTION THAT MATTERS. `[^\r]\n` eats the character before the newline, so a blank line counted as
 * one terminator instead of two. Against the live tree it printed "the frozen copy has 39866 LF, the
 * live copy has 39932 CRLF" for two files that are content-identical — a difference in the ALARMING
 * direction, printed by the very line built to stop a false alarm. The classification was never
 * affected (that is full normalised equality), only the sentence a person reads, which is the whole
 * product here. */
 ...
 *
 *   same          byte-identical. Not drift at all.
 *   eol-only      different bytes, identical content once line terminators are normalised.
 *   changed       different content. A re-measurement is owed.
 *   gone          the live file is not there. Stronger than changed, never weaker.
 *   unavailable   the frozen bytes are not there (a pruned release). UNANSWERABLE — and answering
 *                 `eol-only` here would be the reassuring direction, which is the dangerous one. */
function classifyChange(frozen, live) {
  if (frozen == null && live == null) return { class: 'unavailable', why: 'neither copy could be read' };
  if (frozen == null) return { class: 'unavailable', why: 'the frozen bytes are not on disk, so whether the content changed CANNOT be answered' };
  if (live == null) return { class: 'gone', why: 'the file is no longer in the tree at all' };
  const a = asBytes(frozen), b = asBytes(live);
  if (a === b) return { class: 'same', why: 'byte-identical' };
  const na = eolNorm(a), nb = eolNorm(b);
  if (na === nb) {
    const ca = countEol(a), cb = countEol(b);
    const side = s => (s.crlf ? s.crlf + ' CRLF' : '') + (s.crlf && (s.lf || s.cr) ? ' + ' : '')
      + (s.lf ? s.lf + ' LF' : '') + (s.cr ? (s.lf ? ' + ' : '') + s.cr + ' bare CR' : '')
      || 'no line terminators';
    return { class: 'eol-only', why: 'LINE ENDINGS ONLY — the frozen copy has ' + side(ca)
      + ', the copy it is compared with has ' + side(cb) + '; identical once terminators are normalised, so no character was edited',
      frozen_eol: ca, live_eol: cb };
  }
  /* A cheap, honest description of the real change: where it starts and how the size moved. Not a
   * diff — a diff here would be a second implementation of something `git diff` already does, and
   * the point of this line is only to stop the reader wondering whether it is line endings again. */
  const la = na.split('\n'), lb = nb.split('\n');
  let first = -1;
  for (let i = 0; i < Math.max(la.length, lb.length); i++) { if (la[i] !== lb[i]) { first = i + 1; break; } }
  return { class: 'changed', why: 'CONTENT CHANGED — first difference at normalised line ' + first
    + ' (' + la.length + ' lines frozen, ' + lb.length + ' live). A re-measurement is owed.',
    first_differing_line: first, lines_frozen: la.length, lines_live: lb.length };
}

/* Same explicit-override discipline as `store()` above, and for the same reason: a silent default
 * here would look exactly like a diagnosis of the real tree while reading somewhere nobody ships. */
function liveRootOf(opts) {
  const r = opts && opts.liveRoot;
  if (!r) return ROOT;
  console.error('  (live tree OVERRIDDEN -> ' + r + '  — the real tree at ' + ROOT + ' is untouched)');
  return r;
}
...
function readOrNull(abs) {
  try { return fs.readFileSync(abs); }
    ...
    if (e.code !== 'ENOENT') console.error('  (cannot read ' + abs + ' for the drift diagnosis: ' + e.message + ')');
    return null;
  }
}

/* WHY DID THIS RELEASE'S DIGEST STOP MATCHING THE TREE? Returns a verdict, per-file rows for the
 * files that MOVED, and a one-line summary a refusal can quote verbatim.
 *
 * IT NEVER THROWS FOR THE ORDINARY FAILURES. `pin_guard.js` calls this from inside the gate, and a
 * gate that dies because a release directory was pruned has turned a diagnosis into an outage. An
 * unreadable manifest is a RESULT here — `UNDIAGNOSABLE`, with the reason — and it is said on stderr
 * as well, because a diagnosis that quietly degrades is the thing this file exists to stop. */
/* `opts.against` — COMPARE AGAINST ANOTHER RELEASE'S SNAPSHOT INSTEAD OF THE LIVE TREE.
 *
 * It is the same question one step to the side, and it is the question that was actually asked on
 * both bad nights: *these two ids differ — did the engine change?* Without it the next person writes
 * a `node -e` that reads two snapshots and compares them, which is the hand-rolled second
 * implementation this repository has paid for more than once. One classifier, two subjects. */
function driftDiagnosis(id, opts) {
  const S = store(opts);
  const against = (opts && opts.against) || null;
  const liveRoot = against ? path.join(S.releases, against) : liveRootOf(opts);
  const subject = against ? 'release ' + against : 'the live tree';
  const dir = path.join(S.releases, id);
  const out = { id, against, verdict: null, summary: null, moved: 0, eol_only: 0, changed: 0, gone: 0,
                undiagnosable: 0, sources_added_since: [], files: [] };
  let man;
  try { man = JSON.parse(fs.readFileSync(path.join(dir, 'release.json'), 'utf8')); }
  catch (e) {
    console.error('  (drift diagnosis: cannot read the manifest for release ' + id + ' — ' + e.message + ')');
    out.verdict = 'UNDIAGNOSABLE';
    out.summary = 'WHY THE DIGEST MOVED: cannot say. Release ' + id + ' has no readable manifest here ('
      + String(e.message).split('\n')[0] + '), so its frozen digests are unavailable to compare against.';
    return out;
  }
  out.cut = man.cut || null;
  out.bodies_pruned = !!man.bodies_pruned;

  for (const [rel, want] of Object.entries(man.files || {})) {
    const liveBuf = readOrNull(path.join(liveRoot, rel));
    const got = liveBuf === null ? null
      : crypto.createHash('sha256').update(liveBuf).digest('hex').slice(0, 12);
    if (got === want) continue;                       /* not moved — no bytes need reading */
    out.moved++;
    const frozenBuf = man.bodies_pruned ? null : readOrNull(path.join(dir, rel));
    const c = classifyChange(frozenBuf, liveBuf);
    out.files.push(Object.assign({ file: rel, frozen_digest: want, live_digest: got }, c));
    if (c.class === 'eol-only') out.eol_only++;
    else if (c.class === 'changed') out.changed++;
    else if (c.class === 'gone') out.gone++;
    else out.undiagnosable++;
  }

  /* THE THIRD WAY AN ID MOVES, and it is neither a byte nor a line ending: SOURCES itself grew. It
   * has grown four times and every growth retroactively gives the live tree a new id with no file
   * edited at all. Only meaningful against the REAL tree, so it is not computed under a liveRoot
   * override — a synthetic fixture would report all 26 as "added" and that is noise, not a finding —
   * nor against another release, where the two manifests answer the question directly. */
  if (!(opts && (opts.liveRoot || opts.against))) out.sources_added_since = SOURCES.filter(s => !(s in (man.files || {})));

  /* PRECEDENCE, AND CONTENT DOMINATES. A mixture of a real edit and a line-ending change is a real
   * edit; reporting it as EOL-ONLY would be the auto-excuse this whole block refuses to be. */
  const owed = ' A re-measurement IS owed.';
  const notOwed = ' The id moved and the source was NOT edited. That is not the same as "nothing can '
    + 'change": a line terminator inside a template literal is semantic, and tests/roster.js matches '
    + '\\r\\n against the simulator source on purpose. It does mean no re-measurement is owed for a '
    + 'code change, because there was none.';
  const names = cls => out.files.filter(f => f.class === cls).map(f => f.file).join(', ');

  if (out.changed || out.gone) {
    out.verdict = 'CONTENT-CHANGED';
    out.summary = 'WHY THE DIGEST MOVED: ' + (out.changed + out.gone) + ' of ' + out.moved
      + ' moved source(s) really changed — ' + [names('changed'), names('gone')].filter(Boolean).join(', ')
      + (out.eol_only ? '; ' + out.eol_only + ' other(s) differ only in line endings (' + names('eol-only') + ')' : '')
      + '.' + owed;
  } else if (out.undiagnosable) {
    out.verdict = 'UNDIAGNOSABLE';
    out.summary = 'WHY THE DIGEST MOVED: cannot say for ' + out.undiagnosable + ' of ' + out.moved
      + ' moved source(s) — ' + names('unavailable') + '. '
      + (man.bodies_pruned ? 'Release ' + id + ' was PRUNED, so the frozen bytes are gone and only the '
        + 'digests survive. ' : '')
      + 'Whether the content changed is UNANSWERABLE from here, so it is not answered.' + owed;
  } else if (out.eol_only) {
    out.verdict = 'EOL-ONLY';
    out.summary = 'WHY THE DIGEST MOVED: no source changed content. All ' + out.eol_only + ' moved '
      + 'source(s) are byte-different and content-identical modulo line endings — ' + names('eol-only')
      + '.' + notOwed;
  } else if (out.sources_added_since.length) {
    out.verdict = 'SOURCE-SET-GREW';
    out.summary = 'WHY THE DIGEST MOVED: not a byte in the tree. Release ' + id + ' froze '
      + Object.keys(man.files || {}).length + ' files and SOURCES now names ' + SOURCES.length
      + '; the ' + out.sources_added_since.length + ' added since (' + out.sources_added_since.join(', ')
      + ') change every future id with nothing edited.';
  } else {
    out.verdict = 'NO-DRIFT';
    out.summary = 'WHY THE DIGEST MOVED: it did not. Every file release ' + id + ' froze is '
      + 'byte-identical in ' + subject + ' right now.';
  }
  /* WHICH TWO THINGS WERE COMPARED, in the summary itself. A verdict quoted out of context into a
   * refusal message must carry its own subject, or the next reader supplies one by guessing. */
  if (against) out.summary = out.summary.replace(/^WHY THE DIGEST MOVED:/,
    'WHY ' + id + ' AND ' + against + ' DIFFER:');
  return out;
}
...
module.exports = { cut, list, verify, drift, open, rerender, surface, compat, sha12, sha12OrNull, authorityDrift,
                    ...
                    * exported so it can be SHOWN correct on constructed bytes rather than assumed;
                    * see tests/probe_release_drift_diagnosis.js. */
                   driftDiagnosis, classifyChange };
...
if (require.main === module) {
  ...
  if (cmd === 'cut') {
    ...
    /* WHY DID IT MOVE — the verb that did not exist on the two nights this cost five heavy re-runs.
     * `list` already prints "N of M files have moved since"; that number is what sent somebody
     * re-measuring, and it cannot tell a code edit from a line ending. This can. */
    let id = arg;
    if (!id) {
      try { id = JSON.parse(fs.readFileSync(POINTER, 'utf8')).current; }
      catch (e) { console.error('no release id given and the pointer ' + POINTER + ' could not be read: ' + e.message); process.exit(2); }
    }
    /* `--against <id>` asks the SAME question about two snapshots, which is the form the question
     * actually arrived in on 2026-08-28 and 2026-09-04: two ids, and nobody could say what moved. */
    const ai = process.argv.indexOf('--against');
    const against = ai > -1 ? process.argv[ai + 1] : null;
    const d = driftDiagnosis(id, against ? { against } : undefined);
    console.log('\n  RELEASE ' + d.id + '  first cut ' + (d.cut || '(unknown)'));
    console.log('  ' + d.moved + ' frozen source(s) differ from '
      + (against ? 'release ' + against : 'the live tree') + '.\n');
    for (const f of d.files) console.log('    ' + f.class.toUpperCase().padEnd(12) + f.file + '\n                  ' + f.why);
    if (d.sources_added_since.length) {
      console.log('\n    SOURCE-SET-GREW  ' + d.sources_added_since.length + ' file(s) were added to SOURCES after this'
        + ' release was cut and\n                     therefore change the id with nothing edited: ' + d.sources_added_since.join(', '));
    }
    console.log('\n  ' + d.verdict + '\n  ' + d.summary);
    console.log('\n  This CHANGES NOTHING. The id still moved, artifacts pinned to it are still stranded,'
              + '\n  and no figure is excused. It only says which of the two causes it was.\n');
    ...
    console.log('usage: engine_release.js cut "<why>" | list | verify [id] | rerender [id]\n'
              + '       engine_release.js drift [id] [--against <id>]   (WHY a digest moved: content, or line endings)\n'
