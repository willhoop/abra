...
function sprintActive() { return exists(SPRINT_LOG); }
function livingDocs() {
  return liveDocs()
...
function readDoc(rel) { return stripCR(rawText(rel)); }

/* ---- ONE READER, TWO TREES: THE HOOK JUDGES THE COMMIT, NOT THE WORKING TREE — 2026-09-11 ------
 *
 * EVERY FILE THIS MODULE READS GOES THROUGH `rawText` / `exists` / `listFiles`, and they read one of
 * two trees. By default, the WORKING TREE, exactly as before — status.js, open_work.js, provenance.js,
 * major_readiness.js and a hand run of the test all see what is on disk. After `useIndex()`, THE STAGED
 * TREE: every document and artifact as it will exist in the commit being made — the index version
 * where staged, HEAD's where not, which is simply what the index holds. `.githooks/pre-commit` asks
 * for that (`tests/test-docs-current.js --staged`).
 *
 * WHY. The hook ran the docs gate against the working tree, and several agents write that tree at
 * once. Three times on 2026-09-11 an unrelated commit was blocked by bytes it did not contain: a
 * mid-rebuild data/abra-tags.js; a census regenerated after a same-day notes row; and
 * docs/ABRA-whitepaper.md:1696's `487`, which the COMMITTED data/roster.moves.json still supports,
 * blocking a commit that touched neither file because an ENGINE run had rewritten the roster on disk.
 * A gate judging bytes that are not in the commit is judging somebody else's work.
 *
 * WHAT "IN THE COMMIT" MEANS, EXACTLY. A path is in the staged tree iff it is a stage-0 regular file
 * in the index (`git ls-files -s`, which honours GIT_INDEX_FILE, so `git commit -a`, `git commit
 * <paths>` and a scratch index all mean what they say). An UNTRACKED file is absent; so is an IGNORED
 * one — data/diff-team-pool.json is regenerated by every differential run and is exactly the kind of
 * working-tree byte that must not decide an unrelated commit. A clone sees the same tree.
 *
 * THE DISK IS A CACHE, VERIFIED BY CONTENT. Streaming ~70 MB of blobs out of git on every commit would
 * cost seconds for nothing: almost every file on disk IS its index blob. So a file is read from disk and
 * hashed as git would hash it (as-is, then with CRLF folded, because core.autocrlf checks LF blobs out
 * as CRLF); if the hash is the index's, the bytes are the staged bytes. Otherwise — modified, mid-
 * rewrite, deleted, or written by another agent between two reads — the blob is read from the object
 * store (`git cat-file blob`). This is a content check and not an mtime or a `git diff` snapshot, so a
...
const normRel = rel => String(rel).replace(/\\/g, '/').replace(/^\.\//, '');
const gitText = (args, opts = {}) => execFileSync('git', args,
  { cwd: ROOT, encoding: 'utf8', maxBuffer: 1 << 30, stdio: ['pipe', 'pipe', 'pipe'], ...opts });

/** Switch every read in this module to the staged tree. Call before the first read; caches are dropped. */
function useIndex() {
  SOURCE = 'index';
  INDEX = null;
...
function indexEntries() {
  ...
  INDEX = new Map();
  for (const rec of gitText(['ls-files', '-s', '-z']).split('\0')) {
    const m = rec.match(/^(\d{6}) ([0-9a-f]{40}|[0-9a-f]{64}) (\d)\t([\s\S]+)$/);
    /* Stage 0 only: git refuses to commit an unmerged path, so a conflict stage is never "the commit".
     * Regular files only: a symlink or a submodule is not a document or an artifact. */
    if (!m || m[3] !== '0' || !/^100(?:644|755)$/.test(m[1])) continue;
    INDEX.set(m[4], { mode: m[1], sha: m[2] });
  }
  return INDEX;
}

/** git's object id for these bytes as a blob, in the repository's hash (sha1, or sha256 by id length). */
function blobId(buf, idLength) {
  return crypto.createHash(idLength === 64 ? 'sha256' : 'sha1')
    .update('blob ' + buf.length + '\0').update(buf).digest('hex');
}
/** The index blob's bytes if `buf` is that blob as checked out (as-is, or LF blob / CRLF checkout), else null. */
function asBlob(buf, sha) {
  if (blobId(buf, sha.length) === sha) return buf;
  if (!buf.includes(13)) return null;
  const lf = Buffer.from(buf.toString('latin1').replace(/\r\n/g, '\n'), 'latin1');
  return blobId(lf, sha.length) === sha ? lf : null;
}
/** The disk copy of `r`, or null when there is none to read. A failed read is COUNTED (readerReport),
 *  never swallowed: the caller then asks git, so a missing disk copy costs a process, not a verdict. */
function diskBytes(r) {
  try { return fs.readFileSync(D(r)); }
...
function rawBytes(rel) {
  ...
  if (!e) {
    READS.absent.add(r);
    const err = new Error(r + ' is not in the staged tree (not a file in the index)');
    err.code = 'ENOENT';
    throw err;
  }
...
function exists(rel) {
  const r = normRel(rel);
  return SOURCE === 'worktree' ? fs.existsSync(D(r)) : indexEntries().has(r);
}
/** Names of the files directly inside `dir` ('.' for the root) in the tree this run reads. */
function listFiles(dir) {
  ...
  const pre = (d === '.' || d === '') ? '' : d.replace(/\/$/, '') + '/';
  const out = [];
  for (const k of indexEntries().keys()) {
...
/** Write a gate's own output file. In the working tree, always. Against the index, ONLY when the disk
 *  copy is byte-for-byte the index blob — then the hook's named `git add` stages it as it always has.
 *  If the disk copy differs, somebody else has an unstaged edit there: it is left alone and the write
 *  is SKIPPED, which is safe because the only thing written is a monotone tightening that the next
 *  green run derives again. Returns { written, why }. */
function writeThrough(rel, text) {
  const r = normRel(rel);
  if (SOURCE === 'worktree') { fs.writeFileSync(D(r), text); return { written: true, why: 'working tree' }; }
  const e = indexEntries().get(r);
  const buf = fs.existsSync(D(r)) ? diskBytes(r) : null;
  const clean = e ? !!(buf && asBlob(buf, e.sha)) : buf === null;
  if (!clean) return { written: false, why: 'the working-tree copy differs from the staged one, so it is somebody '
...
function readerReport() {
  return { source: SOURCE, index_entries: INDEX ? INDEX.size : null, verified_on_disk: READS.disk.size,
    from_index: [...READS.objects].sort(), absent: [...READS.absent].sort(), unreadable: [...READS.unreadable].sort() };
}
...
function crlfProof() {
  ...
  for (const rel of liveDocs()) {
    let bytes;
    try { bytes = rawText(rel); } catch (e) { leaked.push(rel + ' (unreadable: '
...
function artifactObject(rel) {
  ...
  if (!exists(rel)) j = undefined;
    ...
    try {
      j = parseArtifactText(rel, rawText(rel));
    } catch (parseErr) {
...
function lineCommitInstant(rel, line) {
  if (!blameCache.has(rel)) {
    let times = null;
    try {
      /* Against the index, blame THE STAGED TEXT (`--contents -`), not the file on disk: the line numbers
       * must be the lines the rule just read, and a line staged but not committed must come back as
       * "Not Committed Yet" — judged, never excused. */
      const staged = SOURCE === 'index';
      const out = execFileSync('git', staged ? ['blame', '--porcelain', '--contents', '-', '--', rel]
                                             : ['blame', '--porcelain', '--', rel],
        { cwd: ROOT, encoding: 'utf8', maxBuffer: 1 << 28, stdio: [staged ? 'pipe' : 'ignore', 'pipe', 'pipe'],
          ...(staged ? { input: rawText(rel) } : {}) });
...
function grandfatheredTraces() {
  const f = 'data/docs-currency-baseline.json';
  if (!exists(f)) return null;
  const g = (JSON.parse(rawText(f)).known || {}).untraced_grandfathered;
  if (!g) return null;
...
function archiveState() {
  ...
  for (const rel of archiveDocs()) {
    ...
    out.push({
      ...
      replacement,
      replacementExists: replacement ? exists(replacement) : false,
    });
  }
  return out;
}
...
if (require.main === module) {
  /* `--staged` reads the tree the next commit would contain instead of the working tree — the same
   * switch the pre-commit hook throws for the docs gate. See useIndex(). */
  if (process.argv.includes('--staged')) useIndex();
