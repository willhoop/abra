 ...
 *
 * Definitive source: Dex.forFormat(<the active regulation, from data/regulations.json>).
 * Output: ABRA/data/*.js.
...
/* THE FORMAT ID IS NOT WRITTEN HERE, AND `--check` IS THE REASON IT MATTERS. 2026-09-08.
 * This opened a hardcoded M-B dex, so the day `active` moves in data/regulations.json this
 * generator keeps writing the PREVIOUS regulation's moves and megas — and `--check` compares what is
 * on disk against what THIS SCRIPT would write today, which was the same stale answer. Both files
 * are frozen into every engine release; data/mega-formes.js decides what a mega turns into on ~26%
 * of format usage. A green test asking nothing is worse than no test.
 * Through champions_sim, which owns the one derivation AND refuses a format id this checkout does
 * not carry — `Dex.forFormat` returns the BASE mod for an unknown id, i.e. the National Dex wearing
 * the format's name, which is exactly the failure the header above records for CHOMP's data. */
const CS = require(path.join(ABRA, 'engine', 'champions_sim.js'));
...
for (const t of TARGETS) {
  ...
  const header =
    ...
    ` * Source of truth: Dex.forFormat('${FORMAT}') — NOT CHOMP, NOT mainline.\n` +
