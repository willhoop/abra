...
const KNOB_SET = process.env.MEDI_DEALT_BEFORE_CLAMP === '1';
...
if (KNOB_SET && !CHILD) {
  console.log('');
  console.log('  MEDI_DEALT_BEFORE_CLAMP=1 WAS SET FROM OUTSIDE THIS PROCESS.');
...
if (KNOB_SET) {
  ...
  const c = spawnSync(process.execPath, [...(process.execArgv || []), __filename],
    { env: { ...process.env, MEDI_DEALT_BEFORE_CLAMP: '1', ABRA_PROBE_CONTROL_ARM: '1' }, encoding: 'utf8' });
