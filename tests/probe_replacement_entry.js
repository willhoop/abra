...
const KNOB_SET = process.env.MEDI_ENTRY_HAZARD_INLINE === '1' || process.env.MEDI_REPLACE_SPEED_MODIFIED === '1';
...
if (KNOB_SET && !CHILD) {
  console.log('');
  console.log('  ONE OF MEDI_ENTRY_HAZARD_INLINE / MEDI_REPLACE_SPEED_MODIFIED WAS SET FROM OUTSIDE THIS PROCESS.');
...
if (KNOB_SET) {
  ...
  for (const [knob, key, mine] of [['MEDI_ENTRY_HAZARD_INLINE', 'batch', PLAIN.meTok.join(' ')],
    ...
    const c = spawnSync(process.execPath, [...(process.execArgv || []), __filename],
      { env: { ...process.env, [knob]: '1', ABRA_PROBE_CONTROL_ARM: '1' }, encoding: 'utf8' });
