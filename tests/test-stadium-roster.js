...
const NOT_A_MODEL = {
  ...
  'engine/seed_source_audit.js':     'ROADMAP #282 and the open remainder of #244 — an audit of the DERIVED TABLES on our own seeding path against the regulation: which volatile durations came off a move this format does not contain, and which basePowerCallback moves read state board.movePower\'s stub does not build. It fits nothing and predicts nothing; it opens a Dex, plays no game and touches no store, and every figure in it is a statement about OUR OWN code rather than about Champions',
  ...
  /* --- TWO CONFORMANCE SWEEPS DECLARED 2026-08-25, each with its own reason and its own reversal
   *     condition. Both DERIVE a rule out of the authority and then ask whether OUR simulator
   *     reproduces it. That is the same class as the two GATEs above and NOT the same class as a
   *     census of the game: the settling question is "if this number is wrong, who is misled?" and
   *     the answer is an ENGINE developer deciding whether medicham2 conforms, never a player and
   *     never a bot. Neither artifact is read to choose an action anywhere in this repository.
   *     Written out separately rather than sharing a reason, because a shared reason is the
   *     shape-based excuse this table forbids. --- */
  'engine/immunity_sweep.js':        'A DIFFERENTIAL SWEEP, not a census. It derives every immunity class in the format from TypeInfo.damageTaken, derives the moves from move.status / move.flags.powder / move.weather, derives the bypassing abilities from the AUTHORITY\'s own handler set rather than from data/tags.json — and then PLAYS each one in both engines and compares. The derivation is a fact about Champions and the ARTIFACT is a fact about us: data/immunity-sweep.json carries tagGap, abilUnreachable and bodyWhy, which are our coverage, our reachability and our fixture choices. If a row is wrong, the people misled are ENGINE developers deciding whether medicham2 reproduces an immunity; nothing in this repository reads it to choose an action. REVERSAL CONDITION, so this call can be undone rather than re-derived: the day anything DECIDES off immuneTypes or bypassAbilities — an engine reading the artifact instead of deriving, or a bot filtering a move menu with it — this declaration is false and it owes docs/MODELS.md an entry instead',
  'engine/switchin_order.js':        'A CONFORMANCE DERIVATION of ONE sort key. It reads every ability declaring onSwitchInPriority out of the authority, because Battle#comparePriority sorts on Order, then Priority, then Speed — and medicham2-browser.js sorted entrants on speed alone, so it could not produce the authority\'s answer on any board holding one. data/switchin-order.json is the list of carriers and the key each declares; its `what` field says so in one line. That is a repair spec for our simulator, and the ENGINE derives the rule itself rather than reading this file — which is exactly why the artifact is a report and not a model. It is card 5 of data/divergence-turns.json, and the divergence report that ranks those cards is already declared here for the same reason. REVERSAL CONDITION: if the simulator ever LOADS this artifact at run time instead of deriving the priority, the fact has moved into the file and the file owes docs/MODELS.md an entry',
