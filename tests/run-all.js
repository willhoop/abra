...
const GATES = ['engine/selftest.js', 'engine/conformance.js', 'engine/artifact_audit.js',
  ...
  /* ============ WIRED IN 2026-09-08 BY MEASURE, AND ITS EXEMPTION WAS FALSE =====================
   *
   * `tests/probe_red_demo.js` sat in PENDING_WIRE below reading "it already HAS a runner:
   * engine/register_reality.js executes it on every pass as the VERIFIED BY of #273 and #449 ... If
   * register_reality ever stops running it, wire it here that day." Every clause of that is true and
   * the CONCLUSION it supported was not. Three things, each derived rather than argued:
   *
   *   1. `engine/register_reality.js` IS ITSELF IN PENDING_WIRE — the "sting in the tail" the header
   *      of that list already names. So the chain is run-all -> nothing -> register_reality ->
   *      probe_red_demo, and the first link is missing. "Covered elsewhere" was true of the marker
   *      and false of the coverage.
   *   2. `data/register-reality.json` is stamped 2026-08-27. Twelve days, across which the engine
   *      moved constantly and every anchor in the demo file is a literal source string.
   *   3. #273 and #449 are CLOSED rows, and `quarantine.js`'s `no open, known engine defect` clause
   *      counts evidence only from OPEN ones. A red here therefore reached no gate at all, which is
   *      why the file could be RED AT HEAD with 15 stale certificates and 2 counted as hollow while
   ...
   * exit 1 gets published as a measured engine defect (ROADMAP #449). The file declares
   * `ABRA-EXIT 0/1/2` now: 0 GREEN, 1 a HOLLOW row, 2 CANNOT-ANSWER for a stale reversal. All three
   * were demonstrated on this tree on 2026-09-08 by deliberate break — a no-op reversal gives
   * `1 HOLLOW ... ABRA-EXIT 1`, an unmatchable anchor gives `1 COULD NOT BE APPLIED ...
   * ABRA-EXIT 2`, and the restored file gives `0 HOLLOW, 0 COULD NOT BE APPLIED ... ABRA-EXIT 0`.
   *
   * IT PLAYS A GAME, which is the condition every entry in PENDING_WIRE shares — but not against the
   * authority: it loads the LIVE engine/medicham2-browser.js and reverts it in memory, exactly like
...
const PENDING_WIRE = {
  ...
  'tests/probe_selfdrop_address.js': 'Landed 2026-09-06 with the `sdrop` address bucket. A REAL CHECK, shown RED before the change and RED again under `MEDI_MID_SELFDROP_SHARED=1`: five governed arms part on the address and two of them part a BOARD, while three controls on the identical bodies stay green. The mechanism: `BattleActions#selfDrops` draws `random(100)` (battle-actions.ts:1325) on every self-drop move, NO legal move in this format reads the value, and while it sat in the shared `any` bucket it pushed the post-hit ability coin — Poison Touch, Flame Body, Cursed Body — to nth 1 on the authority against nth 0 here. Blocker for wiring HERE: it loads tests/staged_board.js, so it opens a release and plays games, the same blocker as probe_ability_volatile_line.js. No VERIFIED BY marker yet; giving it one is OWED and is named in docs/ENGINE.md.',
  'tests/probe_selfdestruct_winner.js':'Landed 2026-08-22 in cfa3cab9. Re-checked 2026-09-21: docs/ROADMAP.md NAMES IT, so it is on the record and this entry no longer says otherwise. Two earlier sentences here were retracted after tests/test-claim-truth.js called them false: one about the absence of a marker, which the ROADMAP row made untrue, and one about the absence of a runner, which that check treats as untrue too because it counts a marker as the file being accounted for. Both were removed rather than reworded around the matcher. Note for whoever edits this next: the matcher reads the sentence as an assertion and cannot tell a live claim from a quotation of a retracted one, so describe the old wording rather than repeating it. It plays a game. It is the file that proves this assertion was needed: under the old predicate a brand-new check could land in tests/ and nothing anywhere would say so.',
