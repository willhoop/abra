...
/* --weights w1,w2,… : one sample weight per --selfplay directory, written per position to w.f32 (train_pory2.py weights
 * the self-play loss by it). Default 1 for every directory. */
const WEIGHTS = String(arg('--weights', '')).split(',').filter(Boolean).map(Number);
if (WEIGHTS.length && (WEIGHTS.length !== DIRS.length || !WEIGHTS.every(w => w > 0 && Number.isFinite(w)))) throw new Error('build_pory2: --weights needs one positive number per --selfplay directory');
...
function build() {
  ...
  const files = ['tok_num.f32', 'tok_id.i32', 'side.f32', 'field.f32', 'facts.f32', 'base.f32', 'meta.i32', 'z.f32', 'v.f32', 'target.f32', 'deep.f32', 'w.f32'];
  ...
  for (const [di, dir] of DIRS.entries()) {
    ...
    for (const rec of records(dir)) {
      ...
      for (let t = 0; t < rec.hist.length; t++) {
        ...
        w('w.f32', Float32Array.from([wDir]));
        const vt = DEEP ? dv : v;
  ...
  const meta = { generated: new Date().toISOString(), generator: 'solver/machamp/build_pory2.js', feature_version: FX.FEATURE_VERSION,
    engine_release: ENGINE.id, release_stamp: ENGINE.stamp, sources, N, break: BREAK || null, value_source: DEEP ? 'deep rollout values from ' + DEEP : 'search root value', lambda: LAMBDA, max_unfilled: MAXUNF, val_pct: VALPCT, weights: WEIGHTS.length ? WEIGHTS : null,
