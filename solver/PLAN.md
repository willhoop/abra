...
| **MEW** | Self-play factory | release, agents, teams → stamped games | frozen MEDICHAM release | To build (old one archived) | Games/hour measured; every record stamped with release id + weights digest; zero-counter check | — (infrastructure) |
| **MACHAMP** | Training loop / league | MEW games → next checkpoint | MEW, PORYGON2, MAG/DODUO | To build | G1: generation n+1 beats n by SPRT on the same release | the previous checkpoint |
