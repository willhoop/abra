...
async function startDryServer(port) {
  if (await portUp(port)) throw new Error('port ' + port + ' is already in use: a dry run needs its OWN server, started with the netguard preload, or it cannot prove zero public traffic');
  const MAIN = ROOT.includes(path.sep + '.claude' + path.sep) ? ROOT.split(path.sep + '.claude' + path.sep)[0] : ROOT;
  const SD = [path.join(ROOT, '..', 'pokemon-showdown-mc'), path.join(MAIN, '..', 'pokemon-showdown-mc')].find(x => fs.existsSync(x));
  const stub = await require('./login_stub.js').start({ pubkeyFile: path.join(OUT, 'login-stub.pub.pem') });
  const guardPath = path.join(__dirname, 'netguard.js').split(path.sep).join('/');
  const env = Object.assign({}, process.env, { NODE_OPTIONS: ((process.env.NODE_OPTIONS || '') + ' --require "' + guardPath + '"').trim(),
    ROTOM_NETGUARD: path.join(OUT, 'netguard-server.jsonl'), ROTOM_DRY_PUBKEY: path.join(OUT, 'login-stub.pub.pem') });
  const logFd = fs.openSync(path.join(OUT, 'server.log'), 'a');
  log('starting pokemon-showdown-mc on localhost:' + port + ' (--no-security, netguard preload, login stub ' + stub.url + ') from ' + SD);
  const srv = cp.spawn(process.execPath, ['pokemon-showdown', 'start', '--skip-build', String(port), '--no-security'], { cwd: SD, env, stdio: ['ignore', logFd, logFd], windowsHide: true });
  PIDS.server = srv.pid; savePids();
  /* up = the server ANSWERS a websocket with a challstr (the port listens long before the workers serve: measured ~2 min) */
  for (let i = 0; i < 60; i++) { if (await serverAnswers(port)) return { srv, stub, checkout: SD }; if (srv.exitCode != null) break; await sleep(2000); }
  try { cp.spawnSync('taskkill', ['/PID', String(srv.pid), '/T', '/F']); } catch (e) { /* gone */ }   // my own server, by pid
  stub.close();
  throw new Error('server did not answer on ' + port + ' within ~5 min (exit code ' + srv.exitCode + '); see ' + path.join(OUT, 'server.log'));
}
...
async function main() {
  ...
  if (DRY) {
    ...
    const common = ['--dry-run', '--server', server, '--login-url', S.stub.url];
  ...
  if (DRY) {
    ...
    R.netguard = { files: ng, supervisor_live: require('./netguard.js').stats(), login_stub: S.stub.counts,
                   rule: 'every non-loopback connection in every process of the run was REFUSED before DNS; a blocked attempt sends no packet' };
    try { cp.spawnSync('taskkill', ['/PID', String(S.srv.pid), '/T', '/F']); } catch (e) { /* gone */ }   // my own server tree, by pid
    S.stub.close();
  }
