# A type spend clears the added type: Burn Up and Double Shock (2026-09-24, abra/regmc 0.98.0)

Brief: fix the defect filed in `docs/_reports/2026-09-24-reflect-type-corners.md` §6. Base commit `1079f6f9`
(abra/regmc 0.97.0). The version is provisional; the coordinator renumbers.
...
The engine stores the added type as the last element of `types`, and names it in `types._added` (0.96.0).
...
The 0.96.0 comment said every `.map()` / `.filter()` write clears the added type the way `setType` does. That was
...
|---|---|---|
| Reg M-B | 1010 / 0 / 1010 (0.97.0 tree: 1009) | 1009 / 1 / 1010. The census REFUSED to write |
| Reg M-C | 1014 / 0 / 1014 (0.97.0 tree: 1013) | not run |

I restored the census artifacts to the committed bytes after reading them, as in 0.96.0 and 0.97.0. Republishing is
...
The flags and pins match 0.96.0 / 0.97.0:
...
These are the same figures as the 0.97.0 fix. The one Reg M-B protocol divergence is the same declared Supreme
