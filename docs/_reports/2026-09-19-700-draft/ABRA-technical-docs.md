...
**7.0.0 - THE FOLD-IN. EVERY NOTES ROW FROM 6.0.1 TO 6.69.0 IS IN THIS DOCUMENT.**

**HOW TO READ THIS DOCUMENT.** This block is the newest. Every block below it is dated evidence. Each one is true of the release that it names. Do not rewrite a dated block. Do not read a dated block as the state of today.

**GATE.** Run `node engine/quarantine.js`. The verified reading at this release is: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`, on release `6180c4712761` (CHANGELOG 6.69.0). All ten gating clauses pass, and this includes the blind-spot clauses (CHANGELOG 6.64.0, 6.66.1, 6.67.1). The dated readings below this block describe earlier releases. Do not quote them as the reading of today. Run the command.

**RULE.** Do not write "MEDICHAM is correct" or "MEDICHAM is done". Write: "every gating clause passes, including the blind-spot clauses". Then give the limits in **LIMITS OF THE VERDICT** below. Will, 2026-09-19: *"stop saying medicham is done when all these blind spots remain"*.

**HOW THE GATE OPENED.** CHANGELOG 6.53.0 to 6.69.0 record it. Board-material on the three lattices was 0 / 1 / 2 on `a1c7dcd5696b` (6.53.0), 0 / 1 / 1 on `8a4140de3eaa` (6.55.0) and 0 / 0 / 0 on `1a6550ea5ec6` (6.58.0). No game joined at any step. The narration clause then gated. Undeclared narration was 0 / 2 / 6 on `1a6550ea5ec6` and 0 / 0 / 0 on `4c9b0cc4a4da` (6.60.0). On `4c9b0cc4a4da` the gate read OPEN under nine clauses (6.61.0). CHANGELOG 6.64.0 then added the blind-spot clauses. Under them, the artifacts of `4c9b0cc4a4da` would not open the gate: 63 in-scope rows were unproven, and 4 lattice games threw. CHANGELOG 6.63.0 fixed the driver and six engine defects. On `d92bdfb50d88` the gate read CLOSED, 2 of 10: one roster red plant was dead, and 9 staged rows were unproven (6.65.0). CHANGELOG 6.66.0 to 6.68.0 closed both causes. On `6180c4712761` the gate reads OPEN, 10 of 10 (6.69.0).

**WHAT THE GATE CHECKS.** The gate is open only when each gating clause passes. `node engine/quarantine.js` prints each clause on its own line. The clauses are these:

1. The whole-game BOARD-MATERIAL clause, on each of three team lattices. See **LATTICES** below. A lattice game that threw fails this clause (CHANGELOG 6.64.0). The clause reads `threw` and `errors[]`. If the two do not agree, it uses the larger. If both are absent, it gives CANNOT-ANSWER.
2. The whole-game NARRATION clause, on the same three lattices. This clause reports only while clause 1 fails. It starts to gate when clause 1 passes. Nobody sets this. It is the verdict of clause 1, read on the same run (`narrationClause` in `engine/quarantine.js`).
3. The damage differential, `data/engine-diff.json`.
4. The deliberate roster, one clause for each stage: `data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`. A stage passes when FIRED-AND-BOARDS-DIFFER is 0, DID-NOT-FIRE is 0, each red demonstration behaves as its rule predicts, each red plant anchor is live, and no owner deferral passes on its own. An in-scope COULD-NOT-STAGE row fails the stage (CHANGELOG 6.56.0).
5. The staged mechanics, `data/all-mechanics-fire.json`. Since CHANGELOG 6.64.0 this clause has a PROOF half. Each in-scope ability or item must read FIRED with a control arm. Each in-scope move must resolve on both engines. A missing row fails. A board that parts while the protocol agrees (`board.verdict` STATE) is judged as a divergence. Since CHANGELOG 6.66.1 a board that parts in the CONTROL arm fails the clause. Since CHANGELOG 6.67.1 a FIRED row on a live control fails unless the subject's own receipt earns it. Two things excuse a row: the `tests/roster.js DEFERRED` map, and the Illusion closet. A control arm is excused only when the CONTROL mechanic is on the shelf of the owner, not when the subject is.
6. No open, known engine defect. This clause reads `docs/ROADMAP.md`.
7. Coverage: each mechanic that is used is measured by something.
8. Board leaves: no leaf that can stand at a turn boundary goes uncompared (CHANGELOG 6.64.0). The clause reads `derive().standing_at_the_boundary` in `tests/probe_uncompared_leaves.js`. If `BS.snapshot` has a second caller, the clause gives CANNOT-ANSWER.

The command prints ten clause lines on release `6180c4712761`. Items 1, 2, 3, 5, 6, 7 and 8 are one line each. Item 4 is three lines, one for each stage. Each of the ten lines reads PASS.

**LATTICES.** `--games` is part of the sample. It is not a budget. `engine/diff_swarm.js` picks teams by a stride. The step of the stride comes from `--games`. Thus a different `--games` plays different teams. It does not play the same teams and more. CHANGELOG 6.45.0 records the measurement: on one release, `--games 1200` played 961 games and 0 boards parted, and `--games 12000` played 7178 games and 84 boards parted. **RULE.** The gate reads three lattices, not one. `LATTICE_SAMPLES` in `engine/quarantine.js` names them:

| `--games` | artifact |
|---|---|
| `1200` | `data/game-differential.json` |
| `1350` | `data/game-differential.g1350.json` |
| `1950` | `data/game-differential.g1950.json` |

A lattice that is missing, stale, not pinned, labelled wrongly, or equal to another lattice gives CANNOT-ANSWER. CANNOT-ANSWER is not a pass. The eight fields that must be the same on the three artifacts are in `LATTICE_SAME` in `engine/quarantine.js`: the engine release, `steering.policy`, the census digest, the pinned pool, the mode, `turns_cap`, `end_state_mode` and `until_covered`. The procedure to measure a lattice is in *How to measure against a frozen engine* at the end of this document.

**WHAT AN OPEN GATE DOES NOT DO.** An open gate makes an artifact below MEDICHAM re-runnable. It does not make that artifact true. `node engine/quarantine.js` lists each downstream artifact as RE-RUNNABLE. Versions 6.15.0 to 6.25.0 removed from this document every figure that was read from such an artifact. This version restores none of them. None was run again. Will gave that order: these artifacts stay stale until he has done the work on MAG and MILTANK again. The re-run list is ROADMAP #57. Leaf calibration stays absent. It needs a new run of `node engine/backtest_winrate.js`. **RULE.** Do not put a caption on a withheld figure. Remove the figure and name the generator. **RULE.** An open gate does not stop the documents check. `engine/docs_scan.js` continues to charge a figure that is read from a downstream artifact that does not name the current release (CHANGELOG 6.30.0, 6.62.0).

**THE ENGINE THAT THE FIGURES BELOW WERE MEASURED ON.** The release is `6180c4712761`. `git ls-files data/releases/6180c4712761` lists 29 files. Read `engine_release` in `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json`, `data/engine-diff.json`, `data/all-mechanics-fire.json`, `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`. All eight artifacts record `6180c4712761`. **RULE.** The release must be tracked. Run `git ls-files data/releases/<id>`. If the output is empty, RULE 5 of `engine/provenance.js` forbids you to publish a figure from it.

**WHOLE GAMES, MIDDLE ARM, THREE LATTICES.** For each lattice artifact, read `state.games` and `state.games_board_never_diverged`. Subtract the second from the first. That is the board-material count. `--games 1200`: 961 − 961 = 0 (`data/game-differential.json`). `--games 1350`: 1069 − 1069 = 0 (`data/game-differential.g1350.json`). `--games 1950`: 1497 − 1497 = 0 (`data/game-differential.g1950.json`). The undeclared narration-only count is 0 on each lattice. The raw count is 0, 1 and 2. The declared row is Supreme Overlord `fallenundefined`. The clause subtracts it 0, 1 and 2 times. `threw` is 0 on each lattice. `state.games_void_excluded` is 0 on each lattice. `state.games_cut_off_by_the_turn_cap` is 0 on each lattice. `turns_cap` is 50. `end_state[0].summary.verdicts` shows SAME-END-STATE 961, 1069 and 1497, and THREW 0. `state.turn_boundaries_compared` equals `state.turn_boundaries_identical`: 10716, 11856 and 16725. The four games that threw on `4c9b0cc4a4da` now play to the end. CHANGELOG 6.63.0 fixed the driver: it does not click a move that the authority refuses (`tests/probe_driver_refused_click.js`; knobs `MEDI_DRIVER_HIDDEN_DISABLE_UNREAD` and `MEDI_DRIVER_FALLBACK_FIRST_SLOT`).

**THE SAMPLE.** Read `steering` in each lattice artifact. `--games 1200` (`data/game-differential.json`): `team_pool_teams` 8778, `team_pool_picked` 1968, `team_pool_digest` `0d103fb9fa87`. `--games 1350` (`data/game-differential.g1350.json`): 8778, 2206, `7e7a37ded7fc`. `--games 1950` (`data/game-differential.g1950.json`): 8778, 3069, `a5ce76242f8d`. `input_digest` is `57deebd09273` on all three. `driver_code.digest` is `faf70ecbca71` on all three. **CAUTION.** The driver digest changed in CHANGELOG 6.63.0 and 6.65.1. Do not compare these lattices with a lattice from before 6.69.0. **RULE.** Record every flag with the result.

**CORNER ARMS.** Two arms hold the damage roll at an extreme. They are not part of the gate. They were not measured again on the 7.0.0 release. The last reading is on release `b42b81899631`; CHANGELOG 6.22.0 records it. On that release, board-material was 1 of 961 at top-tie-first and 2 of 961 at bottom-tie-first, and the protocol parted in 12 and 15 games (`data/verification/game-differential-top-tie-first.json`, `data/verification/game-differential-bottom-tie-first.json`). **RULE.** A corner reading is true of the release that it names. Do not write it as a reading of another release. The games that stayed apart were end-of-battle clock cards. ROADMAP #584 declares them.

**DAMAGE.** Read `data/engine-diff.json`. `requested` 6000, `compared` 6000, `agreed` 6000, `disagreed` 0, `seed` 20260804, `skipped_multihit` 0, `skipped_ability_multihit` 0, `volley.move_rows` 130, `volley.bond_rows` 12, `volley.no_arrival` 4, `dropped_by_exception` 1 (Dragon Darts, ROADMAP #574). Each of the 16 entries in `arms` shows `compared` 6000 and `disagreed` 0. **RULE.** The volley loop is compared now. CHANGELOG 6.1.0 records the change. Do not repeat the older sentence that says the tool never applies a multi-hit move.

**THE DELIBERATE ROSTER.** `engine/legal_scope.js` decides which mechanics are in scope. Read `counts` in `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`. Items: FIRED-AND-BOARDS-MATCH 148 of 148 in scope; FIRED-AND-BOARDS-DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 0. Abilities: 194 of 200 in scope, 116 out of scope; DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 6. Moves: 494 of 497 in scope, 3 out of scope; DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 3. `plant_anchors`: 22, 63 and 37 checked, `dead` empty, `reds_ran` true. All 22, 63 and 37 red demonstrations are CAUGHT. **RULE.** Do not write "unaccounted". Write the category. **RULE.** An entity that no legal body can carry is out of scope. It is not a staging gap (CHANGELOG 6.28.0).

**THE FIXTURES ARE LEGAL NOW.** CHANGELOG 6.24.0 gave the roster a rule: a fixture body must learn the move that it clicks, and a move must go to a body that learns it. CHANGELOG 6.50.1 applied the same rule to the idle click: see *Stage a roster row* in section 2. **CAUTION.** On `6180c4712761` each roster stage printed some fixture sets as illegal and not baselined: items 17 sets, abilities 94 sets, moves 57 sets (`docs/_reports/2026-09-19-6180-remeasure.md`). Nobody has checked if these are new. This is open.

**DEFERRED BY THE OWNER.** Will set some rows aside. The roster reports them as DEFERRED-BY-OWNER. They are named, not hidden. Read the `results` of each roster artifact. The abilities are Anticipation, Forewarn, Frisk, Illusion (ROADMAP #160), Pickup and Stall (ROADMAP #195). The moves are Axe Kick, Copycat and Electrify. Will, 2026-09-18: *"set frisk aside."* (CHANGELOG 6.49.0). **CAUTION.** A deferral is not a pass. Frisk and Axe Kick have open items. See **LIMITS OF THE VERDICT**.

**CENSUS.** Read `data/mechanics-census.json`. `probed` 969, `live` 969, `armed` 969, `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true.

**STAGED MECHANICS.** Read `data/all-mechanics-fire.json`. `games_played` 4638, `games_threw` 0, `sheets_unassembled` 0, `red_ok` true, `planner.played.games` 3936. `summary.moves.diverged` 0, `summary.abilities.diverged` 0, `summary.items.diverged` 0. PROOF: abilities FIRED 198 of 200 in scope; moves resolved 497 of 497 on the authority and on MEDICHAM; items FIRED 148 of 148. Two ability rows are excused by the owner: Frisk (DID-NOT-FIRE) and Illusion (UNPROVEN-UNCONTROLLED). CONTROL WATCH (`summary.abilities.control_watch`): the receipt of the subject earns each FIRED row that is on a live control. 0 are not earned. CONTROL ARMS (`summary.control_arm_partings`): 841 rows have a control arm, 2 parted, `board_material` 0. The 2 are ANNOUNCEMENT-ONLY rows on Bitter Malice and Night Daze, which the owner shelved because their only carriers hold Illusion. The mechanics clause prints: *0 diverge (+1 board-only), 0 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0*. The board-only row is Axe Kick. See **LIMITS OF THE VERDICT**. The harness plays the fixtures that `engine/stage_planner.js` builds. CHANGELOG 6.14.0 records the integration.

**OPEN DEFECTS.** Run `node engine/open_work.js`. Do not type a list of what is open. PASS — *clean: no open row names an instrument that is RED — no open defect is backed by a failing measurement (194 verdict(s) read)*. At HEAD `165b338c`, `node engine/open_work.js` shows 585 register rows and 201 open rows. 20 of them assert breakage. 18 of those 20 have no instrument that decides them. The clause also shows that `data/register-reality.json` (generated 2026-09-12) is older than `docs/ROADMAP.md`. At CHANGELOG 6.52.0, seven engine leads were known and not fixed. All seven are closed or refuted (CHANGELOG 6.54.0). The Tidy Up order and the `-unboost|0` at −6 are fixed (CHANGELOG 6.63.0). **STILL OPEN.** Coaching prints `-fail` after `[notarget]` in one Perish Song game (notes row 6.57.0). No register row names it, so the open-defect clause cannot see it. It parts no lattice game on this release.

**LIMITS OF THE VERDICT.** Every gating clause passes, including the blind-spot clauses. The items below are still NOT covered. Each count comes from `node engine/coverage.js` or from the artifact that is named.

1. **Illusion.** It is in the closet and it is not modelled. `closet.teams_dropped` is 43, 63 and 71 on the three lattices (ROADMAP #160). The staged row is UNPROVEN-UNCONTROLLED.
2. **Frisk. OPEN. The fix is in progress.** The row reads DID-NOT-FIRE. Only the owner deferral excuses it. The authority announces Frisk only for a foe that holds an item. The foes in the fixture hold no item.
3. **Axe Kick. OPEN. The fix is in progress.** The fixture board parts with no line difference: `vol.confusion` of the target is 2 in the authority and 1 in MEDICHAM. The mechanics clause judges it below the reach shelf, and the roster row is deferred. No clause fails on it.
4. **The sample.** Three lattices of 961, 1069 and 1497 games (`state.games` in `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json`). All come from one frozen open-sheet pool of 8778 teams. `turns_cap` is 50. One driver, `empirical-click/v1`. The rule of three on the largest lattice gives an upper bound of about 3 / 1497 ≈ 0.20% for the rate of games whose boards part (Hanley and Lippman-Hand, 1983). The lattices share teams, so this bound is generous.
5. **Closed team sheets and best-of-one play.** No lattice samples them.
6. **Spreads.** No differential body is on a REAL spread (`node engine/coverage.js`). `declared_gaps` in `data/game-differential.json` shows 17440 bodies with the nature of the sheet and 96 with Serious.
7. **Effects that print nothing.** 76 leaves print nothing when they fire. Move leaves whose EFFECT was exercised: 7 of 11 (`summary.moves.leaf_effect` in `data/all-mechanics-fire.json`).
8. **Uncomparable leaves.** 56 board leaves are compared. 23 of the 24 uncomparable leaves have a writer that fired in another instrument. `volatile:powershift` has none.
9. **Entities exercised in a real game.** 860 of 901, with 68 unmeasurable and 41 measurable but not exercised (`coverage` in `data/game-differential.json`).
10. **Ranged mechanics fully staged by an arm.** 0 of 8.
11. **Tags with no engine consumer.** Two: `needsUntrackedState` and `readsOwnItem` (`node engine/coverage.js`).
12. **Declared divergences.** One row, Supreme Overlord `fallenundefined`, in `DECLARED_DIVERGENCE` in `engine/quarantine.js`.
13. **Corner arms.** Not measured again on this release.
14. **Downstream models.** Re-runnable. Not run again, by the decision of the owner.

**RULE.** A clean verdict is a statement about its sample, its driver and its invented spread. Do not write it as a statement about the ladder.

**PROTOCOL EVENTS.** Read `emittedCount` in `data/protocol-events.json`. `emittedCount` is 46 and `notEmittedCount` is 48. CHANGELOG 6.63.0 changed it: the engine now emits the `-endability` of Gastro Acid.

**WHAT IS NEW IN THIS DOCUMENT.** Section 2 has the new tasks: scope, fixture planning, exit codes, tag lookups, the commit gates, the traceability of a figure, the quarantine classifier, conformance, the store declaration, the lattices, the roster control click and the fixture legality check. §3.4 to §3.8 have the new reference tables. §3.7 lists the engine mechanisms and their knobs. The last section has the three pins, the three lattices and the rules about a release.
...
**PUBLICATION.** The tree is release `cbd510bc2b13`. The authority commit is `20ad99ffc9a5a4a4e8fb56ab04ad8e4255b3f2b4`. All six gate artifacts stamp both. `data/game-differential.json` was generated at `2026-09-10T09:57:55Z`.

**BOARD-MATERIAL.** Read `state.games` in `data/game-differential.json`. It is **961**. Read `state.games_board_never_diverged`. It is **961**. Subtract. The result is **0**. Read `state.games_void_excluded`. It is 0. Read `state.games_cut_off_by_the_turn_cap`. It is 0. The cap is `turns_cap` **50**. The longest game reaches turn **36**. Read `state.turn_boundaries_compared` and `state.turn_boundaries_identical`. Both are **10,705**. Read `state.first_board_divergences`. It is empty. Read `end_state[0].summary.verdicts`. SAME-END-STATE is **960**. DIFFERENT-END-STATE is 0. ENDED-APART is 0. NO-COMPARABLE-BOARD is 0. THREW is 1. The THREW game is a harness choice rejection. It is in `errors[0]`.

**NARRATION.** Read `state.protocol_diverged_board_never_did` in `data/game-differential.json`. It is 1. Subtract the declared rows. There is 1 declared row. The result is **0**. The declared row is the perish drain written above `|upkeep|`. The authority writes it below. Will closeted it on 2026-08-28. It is ROADMAP #440. It does not vote. `engine/status.js` prints it on every run. **RULE.** The narration count is a lower bound. A game records its first divergence only.

**PINS.** Arm `middle`. Mode A. Pin digest `de38d17e15a2`. Steering `empirical-click/v1`. Driver inputs are `data/move-priors.json` and `data/rollout-switch-census.json`. The voluntary switch rate is **9.98%**. The census pin digest is `ab219c68f165`. The team store is `data/team-pool-frozen`. Read `steering.team_pool_teams` in `data/game-differential.json`. It is **8,778**. Read `steering.team_pool_picked`. It is **1,968**. The pool digest is `0d103fb9fa87`. The flags were `--steering empirical --arm middle --end-state --games 1200 --turns 50 --team-store data/team-pool-frozen`. **RULE.** `--games` is part of the sample definition. It is not a budget. Record the whole command.

**DECLARED EXCLUSIONS.** Read `closet.teams_dropped` in `data/game-differential.json`. It is **43**. Those teams carry Illusion. ROADMAP #160 applies. Read `declared_gaps.spreads_absent`. The spreads are synthetic. An open team sheet does not reveal a spread. Read `declared_gaps.nature_declared`. It is **17,440**. Read `declared_gaps.nature_fallback_to_serious`. It is **96**. The natures are real. Read `declared_gaps.mega_stones_kept`. It is **2,626**. Read `declared_gaps.mega_stones_stripped`. It is 0.

**SCOPE OF THE BOARD COMPARISON.** Run `tests/probe_uncompared_leaves.js`. Its `derive()` reads `compared` **54**, `ceiling` **54**, `total` **80**, `hole` **20** and `standing_at_the_boundary` 0. **18** of the 20 leaves have duration 1 and end in the residual. 2 leaves remove themselves inside the action. Neither can be standing when the comparator looks. Read `state.not_compared` in `data/game-differential.json`. It lists 8 declared fields. Each carries a reason. **RULE.** A board difference that ends before a turn boundary is not visible to this instrument. The measured rate on the batch V population was **2 of 19** narration causes. Both were fixed.

**DAMAGE DIFFERENTIAL.** Read `data/engine-diff.json`. `requested` is **6,000**. `compared` is **6,000**. `agreed` is **6,000**. `disagreed` is 0. The `arms` block holds `top`, `bottom` and `idx01` to `idx14`. Each reads 6,000 compared and 0 disagreed. `seed` is 20260804. `band_missing` is 0. `skipped_multihit` is **134**. `skipped_ability_multihit` is **17**. `pool.dropped` is 9. `accuracy_conformance.compared` is 500 and `.disagreed` is 0. `accuracy_modifier_conformance` reads 13 handlers over 14 rows with 0 disagreed. `substitute_bypass_conformance.missing` and `.extra` are empty. **RULE.** The volley loop is not damage-compared here. It is compared in whole games and staged in the roster.

**ROSTER.** Scope is decided by `engine/legal_scope.js`. Bodies are drawn from the legal species. Read `scope.carrier_derivation` in `data/roster.items.json`. There are **347** legal species. **72** are mega formes. **83** are battle-only formes. Read `scope` in `data/roster.items.json`: in scope 148, tested **142**, differ 0, DID-NOT-FIRE 0, fixture gaps 6. Read `scope` in `data/roster.abilities.json`: total 316, out of scope **116** (114 no legal carrier, 1 validator-refused, 1 no legal reader), in scope 200, tested **139**, differ 0, DID-NOT-FIRE 0, fixture gaps 43, CONTROL-NOT-QUIET 13, deferred 5. Read `scope` in `data/roster.moves.json`: total 500, out of scope 3, in scope 497, tested **486**, differ 0, DID-NOT-FIRE 0, fixture gaps 8, deferred 3. Read `plant_anchors` in each. `dead` is empty in all three. `reds_ran` is true in all three. **RULE.** Do not write "unaccounted". Write the category.

**CENSUS.** Read `data/mechanics-census.json`. `probed` is **835**. `live` is **835**. `armed` is **835**. `missing` is 0. `unarmed` is 0. `hollow` is 0. `threw` is 0. `run_ok` is true. Read `data/all-mechanics-fire.json`. `games_played` is **1,313**. `games_threw` is 0. `sheets_unassembled` is 0.

**THE CORNER ARMS DO NOT READ ZERO.** The headline is one arm. It is `middle`. Two further arms pin the damage roll to an extreme. Read `data/verification/gd-top-tie-first-2026-09-10.json`. Board-material is **16 of 961**. Read `data/verification/gd-bottom-tie-first-2026-09-10.json`. Board-material is **15 of 961**. Both are stamped release `cbd510bc2b13`. Both are recorded in `CHANGELOG.md` 5.282.0. Both engines draw from the same constant at a corner. This is not dice noise. The speed ties themselves are clean. `speed_ties.tied_groups_resolved` reads 38,319 on the first and 32,690 on the second. No parted board is attributable to tie order.

**EVIDENCE CHAIN.** `engine/engine_release.js cut` refuses if the Showdown HEAD is not `PINNED_COMMIT`. A release is a copy of the frozen sources. It is not a checksum. Read the source count from `SOURCES` in that file. Do not read it from this block.

**PARTIAL LIFT.** Run `node engine/quarantine.js`. It prints GATE: OPEN. It lists **64** artifacts that are downstream of MEDICHAM and are now re-runnable. Run `node engine/major_readiness.js`. **40** of them would become quotable on a re-run. **24** stay withheld. The 24 have a generator that writes or reads `data/policy-weights.json`, or is MILTANK. **RULE.** Re-runnable is not true. **None of the 40 was re-run for this release.** Will instructed on 2026-09-09 that they stay stale until he has reworked MAG and MILTANK. Do not caption a withheld figure. Omit it and state the generator.

**STILL OWED.** The MAG refit. It is a refit, not a restamp. The damage table under the fitted vector moved. Phase 2 of the plan (ROADMAP #62) is not started. The lab tail is carried: 43, 8 and 6 fixture gaps, 14 CONTROL-NOT-QUIET rows, 5 and 3 rows deferred by the owner.
...
**PUBLICATION.** `data/game-differential.json` holds board-material 27 of 961 and protocol first divergence 93 of 961. The board-material figure is a subtraction. It is `state.games` less `state.games_board_never_diverged`. Both operands are in `state`. Read them. Do not copy them into prose. An operand copied into prose becomes stale at the next republication. A field name does not. **CONDITIONS.** Release `d9e551ed0d5a`. The release does not move, because no engine source moved. 961 games. Cap 20. Arm `middle`. Steering `empirical-click/v1`. End-state comparison on. Census pin `data/verification/census-pin-9446a684709d.json`. Pool `--team-store data/team-pool-frozen`. Driver code stable on all runs.

**DEFECT.** `BattleActions#selfDrops` draws `random(100)` at `sim/battle-actions.ts:1325`. The draw occurs on every self-drop move. The value is never read in this format. All ten moves with `self.boosts` leave `self.chance` undefined. Curse assigns `self` at run time and also leaves `self.chance` undefined. The `typeof … === 'undefined'` test short-circuits first. **EFFECT.** The draw used an address in the shared `any` bucket at `nth 0`. The post-hit ability draw moved to `nth 1` on the authority. It stayed at `nth 0` in this engine. The two engines then drew different numbers for the same event. **ACTION.** Put the self-drop draw in its own address category, `sdrop`. This engine does not draw in that category. Do not pin the category. The authority keeps its address-keyed value. **RESULT.** Board-material 34 to 27. Protocol 100 to 93. The `any` addresses named by the authority and not by this engine fall to 18.

**SCOPE.** The change is in one file. The file is `engine/game_differential.js`. The file is the instrument. `engine/medicham2-browser.js` is not changed.

**SUB-FAMILY VERDICTS.** Poison Touch: CONFIRMED. Flame Body: CONFIRMED. Cursed Body: CONFIRMED. Each verdict has a control on the same bodies. The control shares its address. **Sleep: REFUTED.** Staged Sleep Powder shares its address. Both engines sleep the target for the same three turns. Sleep is a different defect. It is open. **Freeze: NOT MOVED, AS PREDICTED.** The Champions thaw runs below `setActiveMove` and above the hit. No self-drop draw can precede it. Freeze is a different defect. It is open.

**COMPARABILITY.** The addressing contract is hashed into `PIN_DIGEST`. The pins moved. The instrument file moved. `engine/arms_comparable.js` gives NOT COMPARABLE for the prior artifact against the new artifact. It names both causes. **ACTION.** Do not publish that pair as a before/after. Run a third arm. **THIRD ARM.** The same bytes. The same pins. The restore knob `MEDI_MID_SELFDROP_SHARED=1` is set. The output is `data/verification/game-differential.selfdrop-shared.json`. `engine/arms_comparable.js` gives COMPARABLE for that pair. **RESULT.** The third arm reproduces the superseded publication in every field. **LIMITATION.** `engine/arms_comparable.js` cannot read an environment variable. It gives COMPARABLE while the two arms differ by the variable under test. **ACTION.** Both artifacts publish `mid_void.selfdrop_knob` and `mid_void.selfdrop_draws`. The two files declare the difference.

**PREDICTION RECORD.** The card is `data/verification/_prediction-selfdrop-address.json`. It was written before the measuring run. The result is ten of twelve. **MISS 1.** Board-material was predicted at 30. It measured 27. The registered band is 26 to 34. The miss is inside the band. **MISS 2.** The prediction that the new category takes only `random(100)` draws is false on the pool. `outrage|random(2,4)` also enters the category. A range-form draw is pinned. It consumes no shared address. The effect is inert. **ACTION.** The probe assertion is now in two clauses. The clause that matters is that no one-argument draw other than the `random(100)` leaves the shared bucket.

**RECEIPTS.** `mid_void.selfdrop_enters` reads 2235. `mid_void.selfdrop_draws` reads 1925. `mid_void.selfdrop_knob` reads false. `mid_void.selfdrop_seen` is keyed `move|random(args)`.

**GATE.** `engine/status.js` computes nine clauses. Two clauses fail. They are the whole-game BOARD-MATERIAL clause and the whole-game NARRATION clause. Both fail on measured counts. Eight of the nine clauses gate. The narration clause does not gate. One gating clause fails. The gate reads CLOSED. Do not quote the two framings together. **NOT MOVED.** The census reads 829 live of 829 probed and 0 missing. The damage differential reads 0 of 6000 at the midpoint and at each of the sixteen band indices. The seed is 20260804.

**NEXT DEFECT, NAMED.** Five of the 27 games part a board with no protocol divergence. Three of the five are one family. The path is `p*.active[*].stall`. This engine reads 0. The authority reads 3. **STAGING RESULT.** Protect then attack does not reproduce it. Protect, Protect, then attack does not reproduce it. The `duration: 2` grace is modelled. The condition is not known. **ACTION.** Sweep the six legal `stall` movers. Include a faint, a drag, and a turn that does not reach the residual.

**HOW TO REPRODUCE.** Run `node tests/probe_selfdrop_address.js`. It must pass. Run `MEDI_MID_SELFDROP_SHARED=1 node tests/probe_selfdrop_address.js`. It must fail on seven checks. Run `node engine/status.js`. It must read two of nine clauses failing.

**5.263.0 - FIVE ENGINE DEFECTS ARE CORRECTED. EACH ONE IS MEASURED ALONE.**

**PUBLICATION.** `data/game-differential.json` holds board-material 34 of 961 and protocol first divergence 100 of 961. The board-material figure is a subtraction. It is `state.games` less `state.games_board_never_diverged`. Both operands are in `state`. Read them; do not copy them here. The operand was typed out until 2026-09-06, when a republication of the same artifact moved it and the docs-currency gate caught a figure attributed to a file that no longer contained it. **CONDITIONS.** Release `d9e551ed0d5a`. 961 games. Cap 20. Arm `middle`. Steering `empirical-click/v1`. End-state comparison on. Census pin `data/verification/census-pin-9446a684709d.json`. Pool `--team-store data/team-pool-frozen`. Driver code stable on all runs.
...
**WHICH ARTIFACT HOLDS WHICH WHOLE-GAME FIGURE.** `data/verification/fix-batch-M6-sidesel.json` holds board-material 53 of 961 and protocol 154 of 961. `data/game-differential.json` was not rewritten and still holds board-material 77 of 961 and protocol 168. **SUPERSEDED 2026-09-08, narration batch R.** The second sentence was true when it was written and is no longer: `data/game-differential.json` has been rewritten by every whole-game run since, and it no longer holds the protocol figure quoted above at all. The figure is historic and belongs to the verification artifact named beside it; what the gate clause prints today is what `node engine/status.js` prints, and nothing here is restated as current. That is the PUBLISHED figure. Both are correctly measured. Name the artifact each time either is quoted.
...
`engine/medicham2-browser.js`. `Battle#getRandomTarget` (`sim/battle.ts:2492`) returns the USER for
...
**UPDATE (7.0.0).** The procedure above is correct and it is not complete. A measurement pins three things: the release, the census and the team store. The whole-game gate also reads three samples, not one. The flags, the three lattices and the proof of an equal sample are in *How to measure against a frozen engine* at the end of this document.
...
**UPDATE, 6.0.0.** The frozen-source count in the block above was correct when it **was measured** and
has grown since. Read it from `SOURCES` in `engine/engine_release.js` on the day. Do not read it from
this block.
...
**UPDATE (7.0.0).** The paragraph above is dated evidence and its last two sentences are superseded. CHANGELOG 6.1.0 changed the tool. It enters the official engine at `hitStepMoveHitLoop` for a volley row, and it reads the number of arrivals back from that engine. A single-hit row still enters at `moveHit`. Read `data/engine-diff.json`: `skipped_multihit` and `skipped_ability_multihit` must be 0. The volley rows are `volley.move_rows` and `volley.bond_rows`; the head of this document gives their values on the 7.0.0 release.
...
cases. The pair-layer fit now uses all four team-sheet channels. The turn count and the
channel reach are withheld with the pair-layer weights. The value of the sheet was measured on
held-out decisions. The sample size and the result are withheld, because the gate withholds that
artifact. The pory family was refit on the current corpus. The DEAD-tag
...
6. Read the state. Run `node engine/status.js`. Each figure in the output comes from an artifact.
7. Read the open work. Run `node engine/open_work.js`. Do not type a list of what is open.

You now have the site, one validated model, and the state of the project.

## 2. How-to — common tasks

**Ask whether a mechanic is in the regulation (added 6.10.0).**
...
`tests/test-stage-planner.js` clause `oneScope` fails if the planner and the coverage counter give
different sets. The codes are in §3.4.

**Plan a legal fixture for a mechanic (added 6.9.1, changed 6.14.0).**
...
differs in one leaf. It asks the official `TeamValidator` about each team that it emits. A mechanic
gets a fixture or a refusal with a code. **A COULD-NOT-STAGE result is a statement about the fixture.
It is not a statement about the mechanic.** `engine/all_mechanics_fire.js` plays these fixtures. For
an ability and for an item it compares the fixture with its control on each variant: near side, far
side, and slot b. A move keeps the older ladder and also plays the planner fixture. Run
`node tests/test-stage-planner.js` after you change the planner. `STAGE_PLANNER_BREAK=<mode>` runs a
planner that is broken on purpose, to show that the test can go red.

**Read what an exit code means (added 6.12.1).**
...
`node engine/exit_codes.js --selftest` drives each branch. `engine/register_reality.js`,
`tests/run-all.js` and `engine/wire_ladder.js` read this file. Before it, two of them read one exit
code differently: one saw a declared red, the other saw a check that did not run. The table is in
§3.5.

**Find which tags a source file reads (added 6.9.0, changed 6.12.0).**
...
The scanner reads each `TAGS.param`, `TAGS.has`, `TAGS.withTag` and `TAGS.reactorsTo` call to its
balanced close. A `)` inside a string, a template, a regular expression or a comment does not close
the call. A tag is a string at the argument level of the call itself. A literal inside a nested call
is not a tag. A call that does not close is reported in `unclosed`. It is never dropped without a
report. `engine/tag_dex.js`, `engine/coverage.js` and `tests/mutation_harness.js` read this file.

**Know why a move fails against an ability (added 6.12.0).**
Read the row `refusedByAbilityFlag` in `data/tags.json`. Its field `n` is 6. Its field `examples`
names Entrainment, Gastro Acid, Role Play, Simple Beam, Skill Swap and Worry Seed. The move fails
when the ability of the target, or of the user, carries one of the flags in the parameter.
`engine/medicham2-browser.js` asks `abilityFlagRefusal` at three branches. The knob
`MEDI_ABILITY_FLAG_REFUSAL_UNREAD=1` restores the earlier behaviour.

**Commit a change (changed 6.12.3, 6.15.0, 6.17.0).**
...
`engine/docs_scan.js` `useIndex()` is the one reader. It obeys `GIT_INDEX_FILE`, so `git commit -a`
and `git commit <paths>` are judged on the tree that git will write. Each gate prints a `(--staged`
line that names each path where the disk is different from the commit.
...
not use `--no-verify`. If a gate is wrong, correct the gate.

**Add a figure to a living document (changed 6.6.2, 6.8.1).**
...
1. an artifact that the paragraph of the figure cites, and that holds the value. If the citation
   names a field, the value must be in that field;
...
3. a commit-pinned citation: `<commit>:data/<file>.json`, or `<commit>:data/<file>.json:<field>` for
   one field. The check reads these bytes from git, and not from the file of today.

A hit somewhere else in `data/`, or somewhere else in the CHANGELOG, is not a trace. A citation of
the open-work artifact or of the baseline artifact is not a trace. Figures that were in the documents
before this rule are on a list of exceptions, written one time by
`node tests/test-docs-current.js --bootstrap-grandfather`. That list can only become smaller. A
figure on the list loses its place when its value or its sentence changes. **RULE.** Put the
citation in the same paragraph as the figure. Do not write a figure into the CHANGELOG to give it a
trace.

**See what the next major release owes (changed 6.6.0).**
`node engine/docs_scan.js --owed` prints the backlog and how near it is to the cap. The cap is
`OWED_CAP` in `engine/docs_scan.js`. Above the cap the build fails. Clear the backlog with a
documentation pass at any version. **Do not make a release `X.0.0` to empty a backlog.** A major
release is declared by a change of basis and by nothing else.

**Decide whether an artifact is withheld (changed 6.15.0, 6.20.0).**
`node engine/quarantine.js` prints the gate and the withheld set. It derives that set from the
source. It does not read a list of names. `node engine/quarantine.js --selftest` drives each branch
on synthetic input. The three routes are in §3.6.

**Check the engine against the project standards (changed 6.4.1).**
`node engine/conformance.js`. Standard S12 is that nothing is written in code which belongs in a
configuration file. A replay id is not a hardcoded format. Standard S13 is that there is no state
which is kept by hand. A scratch dump with a `_` prefix is skipped only while no code names it. The
permitted sources are in the `DECLARED_SOURCES` table of `engine/conformance.js`. **A test derives
the format id from `engine/champions_sim.js`. Do not write the format id in a test.** After the
regulation changes, a format id that is written by hand tests the earlier game and says nothing.

**Know why a probe has no runner (changed 6.4.1, 6.13.0).**
`tests/run-all.js` derives the PENDING-WIRE state for a probe that loads a module which plays a game
and that is named in `docs/ENGINE.md`. It prints each of these probes by name. It reads each exit
code through `engine/exit_codes.js`.

**Exclude a damaged store row (added 6.6.1).**
`data/quality-filter.json` holds the rule `exclude_corrupt_winner` with the reason code
`corrupt_winner`. It names two ladder games whose stored winner is neither player. `engine/quality.js`
and `engine/quality.py` apply it as the last stage of the funnel. **It is a declaration. It is not a
detector.** `python engine/sanity_check.py` fails on a bad winner that is not declared, and on a
declared game whose winner is correct now, so the declaration cannot outlive its defect. Do not
repair a store row by hand. `build/compress-stores.js` cannot express a correction.

**Read the whole-game gate on three lattices (added 6.46.0).**
Run `node engine/quarantine.js`. The whole-game lines name three samples. Each sample is one value of
`--games`. The table of samples is `LATTICE_SAMPLES` in `engine/quarantine.js`. Each sample must read
zero for the clause to pass. A sample that is missing, stale, not pinned, labelled wrongly, or equal to
another sample gives CANNOT-ANSWER. **CANNOT-ANSWER is not a pass.** `engine/game_differential.js`
writes `games_requested` into the artifact, so the sample is recorded and is not inferred. To measure
the three samples, use *Procedure — measure the three lattices* at the end of this document.
`node engine/quarantine.js --selftest` drives each branch of the lattice rule on synthetic input.

**Read the narration clause (changed 6.46.0).**
The narration clause counts games whose commentary parts from the official engine while the board
does not. It reports only while the board-material clause fails. It gates when the board-material
clause passes. Nobody sets this. `narrationClause` in `engine/quarantine.js` reads the verdict of the
board clause on the same run. `node engine/quarantine.js --narration` exits non-zero on narration
alone. A declared divergence is subtracted by name. The declarations are `DECLARED_DIVERGENCE` in
`engine/quarantine.js`. Each one has a reason.

**Stage a roster row (changed 6.36.0, 6.42.0, 6.50.1).**
Run `node tests/roster.js --stage <items|abilities|moves> --reds --write` against a frozen release.
The stage builds two arms for each row: the subject arm and the control arm. The control arm must
differ from the subject arm in the one mechanic and in nothing else. **RULE.** If the control arm can
act on the board, the row is CONTROL-NOT-QUIET. It is not a green.
- The Skill Swap control records its own bookkeeping separately, as
  `control_arm_bookkeeping_dropped`. The bookkeeping of the control is never evidence for the subject
  (CHANGELOG 6.36.0). `tests/probe_control_self_name.js` checks this.
- The quiet set is decided per arm by `quietAsControl(id, arm)`. A crit blocker is not quiet on the
  arm where every crit lands (CHANGELOG 6.42.0).
- The idle click is a move that the fixture body can learn. It is Focus Energy where each holder learns
  it. Otherwise it is Sleep Talk, which is inert on a body that is awake. Each run prints
  `THE CONTROL CLICK` for each stage and a sleeping-body alarm. `ROSTER_INERT_FOCUSENERGY=1` restores
  the old fixture for a before/after (CHANGELOG 6.50.1).
- A row that Will set aside reads DEFERRED-BY-OWNER. Anticipation, Forewarn and Frisk are these rows.
  Do not stage them. Do not count them as gaps.
- `--reds` plants a defect for each shape rule and reads whether the stage catches it. A red that reads
  NOT CAUGHT means that the greens of that rule prove nothing. Correct the plant or the rule before you
  publish a count.

**Check that fixture bodies are legal (added 6.48.0).**
`engine/fixture_legality.js` asks the official `TeamValidator` about each body that a test builds. It
also checks status codes and genders against the species. At run time it checks each body that
`buildPair` builds for a caller under `tests/`. It reports. It does not fail a run. A directed game
hashes the same with the check on and off.

**Check the register against its instruments (changed 6.29.0, 6.47.1).**
Run `node engine/register_reality.js`. It checks each register row against the instrument that the row
names, and reports whether the two agree. A CANNOT-ANSWER result is not a disagreement. Run
`node engine/register_reality.js --selftest` after you change it. **RULE.** A closed row must name an
instrument that a person can run. A row that names the whole-game gate as its proof reads PREMATURE
...
| Export | Does |
|---|---|
| `M.TRACE_EVENTS` | the event names this engine claims it can emit. Read `emittedCount` in `data/protocol-events.json`. It is 46 (CHANGELOG 6.63.0). Read the artifact, not this cell |
...
### 3.4 Scope codes (`engine/legal_scope.js`, added 6.10.0)

`verdict(kind, id)` gives one of these codes. Each part is read from the format. No part is written
by hand.
...
| `INJECTED` | move | the simulator gives it to a body that has no learnset for it. Struggle stays in scope this way |
| `CONFERRED` | ability | no accepted carrier, but a source in scope writes the ability onto a body. Simple is in scope through Simple Beam |
| `HELD` | item | legal. A mega stone only while one of its mega formes is legal. An item with a named user only while one of those users is legal |
| `NO-LEGAL-CARRIER` | any | nothing legal carries, learns or holds it, and nothing in scope confers or injects it |
| `VALIDATOR-REFUSED` | any | legal species carry it and the validator refuses each one. Battle Bond is refused this way |
| `NO-LEGAL-READER` | ability, item | each handler only writes a value that nothing in the regulation reads. Gluttony is out this way, because the berries that read its value are not in this regulation |

### 3.5 Exit codes (`engine/exit_codes.js`, added 6.12.1)
...
| any other code | not a verdict, unless the instrument declares one at the start of a line: `ABRA-EXIT <code> CANNOT-ANSWER` or `ABRA-EXIT <code> VERDICT-RED`. Only a declaration that names the code of the actual exit is read. The last such line wins |
...
`CANNOT-ANSWER` becomes SKIP. An exit code 2 that is not declared becomes SKIP. Each other unclear
result becomes FAIL. **A crash is not a skip.**

### 3.6 Quarantine classifier routes (`engine/quarantine.js`, added 6.20.0)

The classifier holds or releases an artifact by a derived rule. It does not read a list of names.

| route | what it decides |
|---|---|
| `sideBySideInstruments` | a generator that requires MEDICHAM and the official engine together, and that reaches no `board.js`, is an instrument. An instrument is not withheld |
| `foreignSource` | a Python generator is read for the dumps that it names, which MEDICHAM played |
| `describedBySelf` | an artifact with no writer that can be found is held by what it says about itself: an exact twin of the key set of a held artifact, or a field that names a held artifact as its source. **This route never releases an artifact** |

`engine/provenance.js` does not count a write into a temporary directory as a write into `data/`
(6.15.0). Before that correction, the quarantine withheld instrument artifacts.

### 3.7 Engine mechanisms added since 6.0.0

Each correction has a knob that restores the earlier behaviour. Each one had a probe that was red
before the code existed.

| mechanism | knob | version |
|---|---|---|
| Simple applies its multiplier when Simple Beam gives it | `MEDI_SIMPLE_UNAMPLIFIED=1` | 6.11.0 |
| Magnet Rise has a duration and writes its end line | `MEDI_MAGNETRISE_NO_CLOCK=1` | 6.11.0 |
| A second Psychic Noise keeps the heal-block clock that is already running | `MEDI_HEALBLOCK_REFRESH=1` | 6.11.0 |
| A Parental Bond volley pays its secondary effects for each arrival | `MEDI_BOND_SECONDARY_ONCE=1` | 6.11.0 |
| A rampage draws its length on the `range` stream and keeps both counters | `MEDI_RAMPAGE_TWO_TURNS=1` | 6.11.0 |
| A move that an ability flag refuses asks that flag | `MEDI_ABILITY_FLAG_REFUSAL_UNREAD=1` | 6.12.0 |
| Endure, Focus Sash, Sturdy and Focus Band answer each arrival of a volley | `MEDI_HITCOUNT_DROP_ON_COLLAPSE=1` | 6.22.0 |
| A sleep that the residual walk sets is offered to its cure | `MEDI_WALK_STATUS_UNCURED=1` | 6.24.0 |
| A body in the faint queue runs the remainder of the residual walk | `MEDI_ZOMBIE_SKIPS_RESIDUAL=1` | 6.26.0 |
| The first follower after a perish expiry pays the queue at its own position | `MEDI_FOLLOWER_COUNTS_CORPSES=1` | 6.26.0 |
| The faint epoch belongs to the battle, not to the process | `MEDI_FAINT_EPOCH_GLOBAL=1` | 6.14.1 |
| An Early Bird body spends two ticks of the sleep counter each turn | `MEDI_SLEEP_TICKS_AS_TURNS=1` | 6.41.0 |
| An Electric charge is spent after an aborted click and after a status click | `MEDI_ELECTRIC_CHARGE_SURVIVES_ABORT=1` | 6.45.0 |
| A mega evolution runs the end handler of the ability that it replaces, so a Flash Fire boost ends | `MEDI_MEGA_KEEPS_ABSORB_GIFT=1` | 6.45.0 |
| Beat Up prices each hit from the set species of the ally, not from its current forme | `MEDI_BEATUP_FIELD_FORME=1` | 6.47.0 |
| A Future Sight payout hits a Substitute unless the user is on the field with Infiltrator | `MEDI_DELAYED_HIT_THROUGH_DOLL=1` | 6.48.0 |
| Harvest restores only a berry | `MEDI_HARVEST_GATES_ON_ATEBERRY=1` | 6.48.0 |
| The Prankster refusal reads the target of the move | `MEDI_PRANKSTER_TARGET_BLIND=1` | 6.48.0 |
| Accuracy modifiers are read from the `accuracyMod` tag, not from a table of names | `MEDI_ACCMOD_BY_NAME=1` | 6.48.0 |
| Protection moves are read from the `shieldsUser` tag, not from a table of names | `MEDI_SHIELD_BY_NAME=1` | 6.48.0 |
| Mold Breaker gets through Good as Gold | `MEDI_STATUS_REFUSAL_UNBREAKABLE=1` | 6.49.0 |
| Mold Breaker gets through Magic Bounce | `MEDI_BOUNCE_UNBREAKABLE=1` | 6.49.0 |
| Mold Breaker gets through Sticky Hold against Knock Off | `MEDI_STICKYHOLD_UNBREAKABLE=1` | 6.49.0 |
| The Speed drop from Gooey goes through the shared boost road, so Contrary, Defiant and Mirror Armor react | `MEDI_PUNISH_RAW_BOOST=1` | 6.50.0 |
| Mummy and Wandering Spirit obey the flags that refuse them | `MEDI_CONTACT_REWRITE_FLAGS_UNREAD=1` | 6.50.0 |
| An Electric move that stops early still spends Charge | `MEDI_ELECTRIC_CHARGE_KEPT_ON_EARLY_EXIT=1` | 6.50.0 |
| A switch-out clears the Ally Switch counter | `MEDI_ALLYSWITCH_SURVIVES_SWITCH=1` | 6.50.0 |
| Transform runs the start handler of the copied ability | `MEDI_TRANSFORM_NO_COPIED_START=1` | 6.50.0 |
| Parental Bond does not open its second hit on a fainted target | `MEDI_BOND_REACT_DRAWN=1` | 6.50.0 |
| Grav Apple gets its multiplier under Gravity | `MEDI_GRAV_APPLE_IGNORES_GRAVITY=1` | 6.52.0 |
| Heal Pulse from a Mega Launcher user heals more, with the rounding of the official engine | `MEDI_HEAL_PULSE_IGNORES_LAUNCHER=1` | 6.52.0 |
| Decorate, Coaching, Aromatic Mist and Howl send target boosts through the shared boost road | `MEDI_TARGET_BOOST_RAW=1` | 6.52.0 |
| The "raised a stat this turn" flag clears on arrival | `MEDI_BOOST_SNAP_SURVIVES_SWITCH=1` | 6.52.0 |
| A stat raise at lead entry counts on turn 1 | `MEDI_TURN1_SNAP_AFTER_ENTRY=1` | 6.52.0 |
| Helping Hand fails at an ally that already moved, unless the ally switched in this turn | `MEDI_HELPINGHAND_MOVED_ALLY=1` | 6.52.0 |
| The next queued Round is promoted and doubled | `MEDI_ROUND_UNPROMOTED=1` | 6.52.0 |
| Magic Bounce reflects each reflectable move that can be staged | `MEDI_BOUNCE_KIND_BLIND=1` | 6.52.0 |
| The residual stops at the handler that decides the game | `MEDI_RESIDUAL_STOP_GROUP_ONLY=1` | 6.52.0 |
| A Future Sight payout charges Life Orb | `MEDI_ORB_TOLL_SKIPS_PAYOUT=1` | 6.52.0 |
| Confusion at the end of a lock waits for the next update | `MEDI_FATIGUE_BERRY_INLINE=1` | 6.52.0 |
| Pressure charges PP from the body that took a redirected move | `MEDI_PP_PRESSURE_PRE_REDIRECT=1` | 6.52.0 |
| Healer and Shed Skin roll only when a target has a status | `MEDI_CURE_ROLL_UNGATED=1` | 6.52.0 |
| Spicy Spray rolls no die | `MEDI_REACTION_DIE_ALWAYS=1` | 6.52.0 |
| Life Dew addresses its rolls at a drawn foe | `MEDI_ALLIES_ADDR_AT_USER=1` | 6.52.0 |
| Darkest Lariat ignores evasion, read from its tag | `MEDI_MOVE_EVASION_COUNTED=1` | 6.52.0 |
| Protean does not convert on a called or delayed move, or on a status click whose own Try refuses | `MEDI_PROTEAN_IGNORES_TRY=1` | 6.54.0 |
| Clear Smog clears the stat changes of the target, but not through a Substitute | `MEDI_CLEAR_SMOG_KEEPS_BOOSTS=1` | 6.54.0 |
| A Mold Breaker move lands its stat change on a Contrary target as written | `MEDI_CONTRARY_UNBROKEN=1` | 6.54.0 |
| The pending berry of Cud Chew clears on switch-out | `MEDI_CUD_SURVIVES_SWITCH=1` | 6.54.0 |
| Copycat refuses the moves that carry `failcopycat` | `MEDI_COPYCAT_IGNORES_FAILCOPYCAT=1` | 6.54.0 |
| An on-hit chance ability rolls once for each hit that lands | `MEDI_REACT_LATE_ONCE=1` | 6.54.0 |
| Triple Axel draws its accuracy at the start of each hit | `MEDI_MULTIACC_UPFRONT=1` | 6.54.0 |
| A user that falls asleep during a multi-hit move stops | `MEDI_VOLLEY_IGNORES_SLEEP=1` | 6.54.0 |
| Effect Spore rolls no die against a Grass or Overcoat attacker | `MEDI_SPORE_DIE_UNGATED=1` | 6.54.0 |
| Sucker Punch reads the queued move, which Encore rewrites | `MEDI_SUCKER_READS_PRE_ENCORE=1` | 6.57.0 |
| Sucker Punch reads the category of the move, not the kind of action | `MEDI_SUCKER_READS_ACTION_KIND=1` | 6.57.0 |
| Focus Sash and Sturdy apply to a Future Sight payout | `MEDI_DELAYED_HIT_NO_SURVIVAL=1` | 6.57.0 |
| Focus Sash and Sturdy apply to a confusion self-hit | `MEDI_SELFHIT_NO_SURVIVAL=1` | 6.57.0 |
| Roost announces `-singleturn` on each body | `MEDI_ROOST_ANNOUNCE_FLYING_ONLY=1` | 6.57.0 |
| A spread move with only a partner left targets the partner and prints no `-fail` | `MEDI_SPREAD_NOFOE_FAILS=1` | 6.57.0 |
| Corrosive Gas with no target left prints `-fail` | `MEDI_ITEMMOVE_NOTARGET_SILENT=1` | 6.57.0 |
| Synchronize prints its refusal line | `MEDI_SYNC_IMMUNE_SILENT=1` | 6.57.0 |
| Coaching and Aromatic Mist fail aloud with no ally | `MEDI_COACHING_NOALLY_SILENT=1` | 6.57.0 |
| Life Dew prints each refusal before any heal | `MEDI_ALLIES_HEAL_INTERLEAVED=1` | 6.57.0 |
| The mega phase waits for the switches, not for a priority bracket | `MEDI_MEGA_GATE_ON_PRIORITY=1` | 6.57.0 |
| The replacement order after a faint reads the Speed that Speed Swap wrote | `MEDI_CORPSE_SPEED_KEEPS_REWIRE=1` | 6.57.0 |
| A broken Protect clears the stall counter | `MEDI_BREAK_KEEPS_STALL_FRESH=1` | 6.57.0 |
| A lock ends in confusion after a blocked last turn | `MEDI_LOCK_END_NEEDS_HIT=1` | 6.57.0 |
| White Herb acts between the switch of the pivot and the switch-in of the entrant | `MEDI_PIVOT_HERB_AFTER_ENTRY=1` | 6.57.0 |
| Berserk acts after recoil | `MEDI_HP_THRESHOLD_BOOST_ABOVE_RECOIL=1` | 6.57.0 |
| Dragon Darts into a semi-invulnerable body prints the lines of the authority | `MEDI_SMART_INVULN_MISS_LINE=1` | 6.57.0 |
| Volatiles act in the order that they were added | `MEDI_VOL_ARTIFACT_ORDER=1` | 6.57.0 |
| The weather step skips fainted bodies when it sorts | `MEDI_WEATHER_SORT_KEEPS_CORPSES=1` | 6.57.0 |
| Berry passes sort on the saved Speed that the authority refreshes at set points | `MEDI_UPDATE_LIVE_SPEED=1` | 6.57.0 |
| Electro Shot writes its zero-size charge boost | `MEDI_CHARGE_BOOST_ZERO_SILENT=1` | 6.57.0 |
| Forewarn announces the move that it finds and draws its tie die | `MEDI_FOREWARN_SILENT=1` | 6.59.0 |
| Harvest throws its coin at each end of turn, with a berry or without | `MEDI_HARVEST_COIN_GATED=1` | 6.59.0 |
| Chilly Reception prints `-fail` with snow up and no bench | `MEDI_CHILLY_NOBENCH_SILENT=1` | 6.59.0 |
| A held status refuses before Misty Terrain, Electric Terrain or Safeguard | `MEDI_STATUS_HELD_AFTER_FIELD=1` | 6.59.0 |
| Rapid Spin, Mortal Spin and Defog attribute their removal lines | `MEDI_SWEEP_UNATTRIBUTED=1` | 6.59.0 |
| Magician prints one `-item` line | `MEDI_MAGICIAN_ENDITEM_LINE=1` | 6.59.0 |
| A Magician user at 0 HP takes nothing | `MEDI_MAGICIAN_DEAD_THIEF_TAKES=1` | 6.59.0 |
| A partial stat-drop refusal prints above the drops that it lets through | `MEDI_DROP_REFUSAL_AFTER_TABLE=1` | 6.59.0 |
| Unburden is engine state and a compared board leaf; a carrier that stole an item and then lost it doubles its Speed | `MEDI_UNBURDEN_BREAK=1` | 6.63.0 |
| Gastro Acid suppresses the target's ability for every reader and prints `-endability` | `MEDI_GASTRO_SUPPRESSES_NOTHING=1` | 6.63.0 |
| Knock Off's multiplier and Poltergeist read the held item under Magic Room and Klutz | `MEDI_TARGET_ITEM_READS_SLOT=1` | 6.63.0 |
| Ripen takes its second halving after a resist berry | `MEDI_RIPEN_NO_RESIST_WEAKEN=1` | 6.63.0 |
| Anticipation announces, and draws no die | `MEDI_ANTICIPATION_SILENT=1` | 6.63.0 |
| Screen Cleaner announces before the screens end | `MEDI_SCREENCLEAN_ACTIVATE_LAST=1` | 6.63.0 |
| Pressure, Mold Breaker and Unnerve write their switch-in `-ability` line | `MEDI_START_ANNOUNCE_SILENT=1` | 6.63.0 |
| Tidy Up removes, announces, then boosts | `MEDI_TIDYUP_BOOST_FIRST=1` | 6.63.0 |
| A drop at −6 prints `-unboost\|0` | `MEDI_FLOOR_DROP_REFUSED=1` | 6.63.0 |
| Anger Point runs at the `Hit` event, before the move's own secondary | `MEDI_HIT_BUFF_AT_DAMAGING_HIT=1` | 6.66.0 |
| A Thief, Covet or Knock Off strip that Sticky Hold refuses writes `-activate` | `MEDI_STICKYHOLD_SILENT=1` | 6.68.0 |
| Sticky Hold does not refuse for a holder that the same hit brought to 0 HP | `MEDI_STICKYHOLD_REFUSES_AT_ZERO=1` | 6.68.0 |

CHANGELOG 6.7.0 records fifteen more corrections, each with its own knob, found at the corner arms.
CHANGELOG 6.18.0 records eight more. In those eight, Oblivious refuses Taunt, a bounced Spite takes
PP from the body that clicked it, a holder of Super Luck or of Scope Lens keeps the sure critical hit
after Focus Energy, and a mega stone under Klutz still evolves. `engine/game_differential.js` pins
the `range` stream to 0 in each arm, so a length that is drawn cannot move a comparison.

The rows for 6.63.0 to 6.68.0 were checked present in the release copy `data/releases/6180c4712761/engine/medicham2-browser.js`. Three more knobs are not engine knobs, and the release does not hold them. The empirical driver in `engine/game_differential.js` has `MEDI_DRIVER_HIDDEN_DISABLE_UNREAD=1` and `MEDI_DRIVER_FALLBACK_FIRST_SLOT=1` (6.63.0): each restores a click that the authority refuses. The staging instruments have `STAGE_PLANNER_FIRST_PASSING_CONTROL=1` (6.66.0), `AMF_LEGACY_FIRST_CONTROL=1` (6.67.0) and `STAGE_PLANNER_KEEP_LOUD_BEARER=1` (6.68.0): each restores a control that is not measured quiet. `MEDI_WISH_NO_PAYOUT=1` and `MEDI_STAT_INVERT_NOOP=1` are red plants for `tests/probe_state_credit_red.js` (6.66.0), not corrections.

### 3.8 Instruments added since 6.0.0
...
| `tests/probe_roster_learnset_refusals.js` | does each roster fixture body learn the move it clicks |
| `tests/probe_corner_mechanisms.js` | do the corner mechanisms agree with the official engine |
| `tests/probe_ability_flag_refusal.js` | does a move refused by an ability flag fail here also |
| `tests/probe_reopen_partings.js` | do the partings that the planner found stay closed |
| `tests/probe_volley_collapse_clamp.js` | does a survival item answer each hit of a volley |
| `engine/fixture_legality.js` | is each fixture body legal, with a legal status and a legal gender (report only) |
| `engine/register_reality.js` | does each register row agree with the instrument that it names |
| `tests/probe_control_self_name.js` | does the control arm of a roster row count only its own mechanic |
| `tests/probe_electric_charge_abort.js` | is an Electric charge spent after an aborted or status click |
| `tests/probe_mega_ends_absorb_gift.js` | does a mega evolution end the boost of the ability that it replaces |
| `tests/probe_beatup_set_species.js` | does Beat Up read the set species of each ally |
| `tests/probe_future_sight_doll.js` | does a Future Sight payout meet a Substitute |
| `tests/probe_harvest_nonberry.js` | does Harvest refuse an item that is not a berry |
| `tests/probe_signature_tags.js` | do the accuracy and protection decisions read their tags |
| `tests/probe_moldbreaker_refusals.js` | does Mold Breaker get through the refusals that it breaks |
| `tests/probe_gooey_boost_road.js` | does the Gooey drop go through the shared boost road |
| `tests/probe_contact_rewrite_flags.js` | do Mummy and Wandering Spirit obey the refusing flags |
| `tests/probe_electric_charge_paths.js` | is Charge spent on each path of an Electric move |
| `tests/probe_bench_private_counters.js` | does a switch-out clear the counters that a benched body must not keep |
| `tests/probe_transform_copied_start.js` | does Transform run the start handler of the copied ability |
| `tests/probe_bond_reactor_ko.js` | does Parental Bond stop at a fainted target |
| `tests/probe_move_effect_leads.js` | do the move-effect corrections of 6.52.0 hold |
| `tests/probe_unburden_leaf.js` | does Unburden hold, compare and end on every road (6.63.0) |
| `tests/probe_gastro_acid.js` | does Gastro Acid suppress the ability for every reader (6.63.0) |
| `tests/probe_room_target_item.js` | do Knock Off and Poltergeist read the held item under Magic Room and Klutz (6.63.0) |
| `tests/probe_start_announce.js` | do the switch-in announcements print (6.63.0) |
| `tests/probe_driver_refused_click.js` | does the empirical driver avoid a click that the authority refuses (6.63.0) |
| `tests/probe_narration_e.js` | do the Tidy Up order and the `-unboost\|0` line hold (6.63.0) |
| `tests/probe_uncompared_leaves.js` | can any leaf stand at a turn boundary uncompared; the board-leaf clause reads it (6.64.0) |
| `tests/probe_hit_event_buff_order.js` | does a hit-event buff run before the move's own secondary (6.66.0) |
| `tests/probe_state_credit_red.js` | is a move that prints no line credited only on the leaves the authority moves (6.66.0) |
| `tests/test-release-pin-no-cut.js` | can a pinned run cut a release (6.65.1) |

## 4. Explanation

### 4.-3 What a search can rely on in the simulator (added 7.0.0)

**This section explains what was measured about the simulator as a component of a search. It does not
give instructions.**

A tree search keeps many positions alive at the same time. Three properties decide whether that is
safe, and each one was measured.

**A copy of a position is independent.** MEDICHAM builds a copy from the board. A copy and its
original do not share state. CHANGELOG 6.9.2 records the measurement and the cost of a copy against
the cost of a turn. Playing turns limits a search. Copying positions does not.

**Two live positions that are played one after the other stay independent.** CHANGELOG 6.10.2 records
the bench. It also records the one variable that leaked between battles: after a later battle was
built, the result of a double wipe was read wrongly. Nothing in use at that time was affected,
because each caller reads the result of a battle before it builds the next one.

**The faint epoch now belongs to the battle.** CHANGELOG 6.14.1 records the correction. The winner of
a double wipe survives the construction of a later battle. The knob `MEDI_FAINT_EPOCH_GLOBAL=1`
restores the earlier rule, which was global to the process.

**A mode that skipped narration would save nothing.** The protocol trace is off in a rollout already.
CHANGELOG 6.12.2 records the profile that shows this, and names where the time is spent instead.
...
### Procedure — pin the three inputs

A release freezes the code. It does not freeze the store, the census or the artifacts. All three
move. Pin three things, and not one.
...
| `--census <path>` | the census that selects the sample |
| `--team-store data/team-pool-frozen` | the pool of teams, which the live store changes every hour |
...
**Then prove that two samples are the same.** Compare the number of games, the list of first
divergences, and the coverage block. `node engine/arms_comparable.js <before.json> <after.json>`
gives that answer and refuses a pair that is not comparable.

`engine/all_mechanics_fire.js` takes `--release` from the same argument list, and `--kind` with the
default `all`. **Without `--release` it cuts a release over the live tree and says so. That is a
smoke run. It is not a measurement.**

### Procedure — measure the three lattices

The gate reads three samples. Run the differential three times on one release, with the same pins.
Change only `--games` and the output path. `latticeRerun` in `engine/quarantine.js` prints this command:

```bash
node engine/game_differential.js --steering empirical --release <id> --arm middle --end-state   --census data/mechanics-census.json --team-store data/team-pool-frozen   --games 1200 --write --out data/game-differential.json
# then --games 1350 --out data/game-differential.g1350.json
# then --games 1950 --out data/game-differential.g1950.json
```

Then run `node engine/quarantine.js`. **RULE.** Do not read an artifact while another process writes
it. Read the mtime first. A torn read gives a plausible answer that is false. **RULE.** Before the run,
write down which games each fix should remove from which lattice. After the run, compare. A game that
joins a lattice is a new finding. A predicted game that stays is a fix that did not work.

### Procedure — check that a release can still serve a caller

```bash
node engine/engine_release.js compat <file-in-the-release> [symbol ...]
```

A release freezes the engine and not the reader. Each symbol that a caller adds to its list of needs
strands every release that was cut before it. The release still verifies and stops being usable. Ask
this question before a run finds the answer deep inside itself. A stranded artifact is a figure to
withhold and to measure again. It is not a thing to repair.

### Rule — a figure may cite only a release that is in the repository

`engine/provenance.js` holds this as RULE 5. A published figure may not rest on a release whose bytes
are not tracked. The set of published documents is derived. The check asks the manifest of the
release, and not the directory, so a release that is tracked in part fails also. Run
`node engine/provenance.js`. CHANGELOG 6.2.0 records the rule and the artifacts that it found.

### Note — a second cut appends

A cut is an event, appended to `cuts.jsonl` in the directory of the release. The fields `cut` and
`why` in `release.json` mean the first freeze. They are never rewritten. `node engine/engine_release.js
rerender <id>` draws `release.json` again from that log. A cut stops if the official engine is not at
the pinned commit. `--allow-authority-drift` is the only way past it, and it must be deliberate.
