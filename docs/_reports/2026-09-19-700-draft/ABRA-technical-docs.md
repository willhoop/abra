...
**7.0.0 - THE FOLD-IN. EVERY NOTES ROW FROM 6.0.1 TO <<GATE: the last notes row before the 7.0.0 entry>> IS IN THIS DOCUMENT.**
...
**GATE.** Run `node engine/quarantine.js`. The verified reading at this release is: <<GATE: the first line that the command prints on the 7.0.0 release, and the release id that it names>>. The dated readings below this block describe earlier releases. Do not quote them as the reading of today. Run the command.
...
4. The deliberate roster, one clause for each stage: `data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`. A stage passes when FIRED-AND-BOARDS-DIFFER is 0, DID-NOT-FIRE is 0, each red demonstration behaves as its rule predicts, and no owner deferral passes on its own. A COULD-NOT-STAGE row does not fail the stage. The stage counts it.
...
<<GATE: confirm this list against the clause names that the command prints on the 7.0.0 release; correct the list if a clause was added or removed>>.
...
**THE ENGINE THAT THE FIGURES BELOW WERE MEASURED ON.** The release is <<GATE: the 7.0.0 release id>>. Read `engine_release` in `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json`, `data/engine-diff.json`, `data/all-mechanics-fire.json`, `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`. All eight artifacts must record the same release. **RULE.** The release must be tracked. Run `git ls-files data/releases/<id>`. If the output is empty, RULE 5 of `engine/provenance.js` forbids you to publish a figure from it.

**WHOLE GAMES, MIDDLE ARM, THREE LATTICES.** For each lattice artifact, read `state.games` and `state.games_board_never_diverged`. Subtract the second from the first. That is the board-material count. <<GATE: the board-material count for `--games 1200`, `1350` and `1950`, with both operands for each; each must be 0>>. <<GATE: the undeclared narration-only count for each lattice, which must be 0, and each declared row, by name>>. <<GATE: for each lattice, `state.games_void_excluded`, `state.games_cut_off_by_the_turn_cap` and `turns_cap`>>.

**THE SAMPLE.** Read `steering` in each lattice artifact. <<GATE: `team_pool_teams`, `team_pool_picked` and `team_pool_digest` for each lattice, and `input_digest` (the census pin), which must be the same on all three>>. **RULE.** Record every flag with the result.

**CORNER ARMS.** Two arms hold the damage roll at an extreme. They are not part of the gate. <<GATE: either the two corner readings on the 7.0.0 release, or this sentence: "They were not measured again on the 7.0.0 release. The last reading is on release `b42b81899631`; CHANGELOG 6.22.0 records it.">> **RULE.** A corner reading is true of the release that it names. Do not write it as a reading of another release. The games that stay apart are end-of-battle clock cards. ROADMAP #584 declares them.

**DAMAGE.** Read `data/engine-diff.json`. <<GATE: `requested`, `compared`, `agreed`, `disagreed` (0), `seed`, `skipped_multihit` (0), `skipped_ability_multihit` (0), `volley.move_rows`, `volley.bond_rows`, `volley.no_arrival`>>. **RULE.** The volley loop is compared now. CHANGELOG 6.1.0 records the change. Do not repeat the older sentence that says the tool never applies a multi-hit move.

**THE DELIBERATE ROSTER.** `engine/legal_scope.js` decides which mechanics are in scope. <<GATE: for each of `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`, the `scope` block — `total`, `out_of_scope`, `in_scope`, `tested`, `differ` (0), `silent` (0), `could_not_stage_in_scope`, `unattributable`, `deferred` — and `plant_anchors` with `dead` empty and `reds_ran` true>>. **RULE.** Do not write "unaccounted". Write the category. **RULE.** An entity that no legal body can carry is out of scope. It is not a staging gap (CHANGELOG 6.28.0).
...
**DEFERRED BY THE OWNER.** Will set some rows aside. The roster reports them as DEFERRED-BY-OWNER. They are named, not hidden. The abilities are Anticipation, Forewarn and Frisk. Will, 2026-09-18: *"set frisk aside."* (CHANGELOG 6.49.0).

**CENSUS.** Read `data/mechanics-census.json`. <<GATE: `probed`, `live`, `armed`, `missing` (0), `unarmed` (0), `hollow` (0), `threw` (0), `run_ok` (true)>>.

**STAGED MECHANICS.** Read `data/all-mechanics-fire.json`. <<GATE: `games_played`, `games_threw` (0), `sheets_unassembled` (0), `planner.played.games`, and the diverging rows by kind with the sentence that the mechanics clause prints>>. The harness plays the fixtures that `engine/stage_planner.js` builds. CHANGELOG 6.14.0 records the integration.

**OPEN DEFECTS.** Run `node engine/open_work.js`. Do not type a list of what is open. <<GATE: the open-defect clause line on the 7.0.0 release>>. At CHANGELOG 6.52.0, seven engine leads were known and not fixed: Cursed Body, Triple Axel, Clear Smog, stat-raising items under Contrary, Mold Breaker against Contrary on the boost road, Cud Chew after a switch, and Protean on a status move that fails. <<GATE: the state of each of the seven at 7.0.0>>.

**LIMITS OF THE VERDICT.** Run `node engine/coverage.js`. It prints what each verdict does not cover. Each count comes from an artifact. <<GATE: the rows "differential bodies on a REAL spread", "entities exercised in a real game", "ranged mechanics fully staged", "move leaves whose EFFECT was exercised" and "driver policies the gate quotes", as printed on the 7.0.0 artifacts>>. **RULE.** A clean verdict is a statement about its sample, its driver and its invented spread. Do not write it as a statement about the ladder.

**PROTOCOL EVENTS.** Read `emittedCount` in `data/protocol-events.json`. <<GATE: `emittedCount`, and the CHANGELOG entry of the last change to it>>.
...
| <<GATE: each engine mechanism after 6.52.0 that the 7.0.0 release carries, with its knob and version>> | | |
