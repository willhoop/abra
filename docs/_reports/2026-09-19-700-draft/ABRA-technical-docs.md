...
**7.0.0 - THE FOLD-IN. EVERY NOTES ROW FROM 6.0.1 TO 6.61.0 IS IN THIS DOCUMENT.**
...
**GATE.** Run `node engine/quarantine.js`. The verified reading at this release is: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`, on release `4c9b0cc4a4da` (CHANGELOG 6.61.0). The dated readings below this block describe earlier releases. Do not quote them as the reading of today. Run the command.

**HOW THE GATE OPENED.** CHANGELOG 6.53.0 to 6.61.0 record it. Board-material on the three lattices was 0 / 1 / 2 on `a1c7dcd5696b` (6.53.0), 0 / 1 / 1 on `8a4140de3eaa` (6.55.0) and 0 / 0 / 0 on `1a6550ea5ec6` (6.58.0). No game joined at any step. The narration clause then gated. Undeclared narration was 0 / 2 / 6 on `1a6550ea5ec6` and 0 / 0 / 0 on `4c9b0cc4a4da` (6.60.0). On `4c9b0cc4a4da` the gate first read CLOSED, 1 of 9, because the Harvest fix of 6.59.0 rewrote the line that a roster red plant patched. CHANGELOG 6.61.0 re-aimed the plant, the plant was CAUGHT, and the gate read OPEN. No engine byte changed in 6.60.0 or 6.61.0.
...
1. The whole-game BOARD-MATERIAL clause, on each of three team lattices. See **LATTICES** below.
...
4. The deliberate roster, one clause for each stage: `data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`. A stage passes when FIRED-AND-BOARDS-DIFFER is 0, DID-NOT-FIRE is 0, each red demonstration behaves as its rule predicts, and no owner deferral passes on its own. An in-scope COULD-NOT-STAGE row fails the stage (CHANGELOG 6.56.0). A CONTROL-NOT-QUIET row does not fail the stage. The stage counts it.
5. The staged mechanics, `data/all-mechanics-fire.json`.
...
The command prints nine clause lines on release `4c9b0cc4a4da`. Items 1, 2, 3, 5, 6 and 7 are one line each. Item 4 is three lines, one for each stage. The list is correct. Each of the nine lines reads PASS.
...
**WHAT AN OPEN GATE DOES NOT DO.** An open gate makes an artifact below MEDICHAM re-runnable. It does not make that artifact true. Versions 6.15.0 to 6.25.0 removed from this document every figure that was read from such an artifact. This version restores none of them. Will gave that order: these artifacts stay stale until he has done the work on MAG and MILTANK again. The re-run list is ROADMAP #57. Leaf calibration stays absent. It needs an open gate and a new run of `node engine/backtest_winrate.js`. **RULE.** Do not put a caption on a withheld figure. Remove the figure and name the generator. **RULE.** An open gate does not stop the documents check. `engine/docs_scan.js` continues to charge a figure that is read from a downstream artifact that does not name the current release (CHANGELOG 6.30.0).

**THE ENGINE THAT THE FIGURES BELOW WERE MEASURED ON.** The release is `4c9b0cc4a4da`. `git ls-files data/releases/4c9b0cc4a4da` lists 29 files. Read `engine_release` in `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json`, `data/engine-diff.json`, `data/all-mechanics-fire.json`, `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`. All eight artifacts must record the same release. **RULE.** The release must be tracked. Run `git ls-files data/releases/<id>`. If the output is empty, RULE 5 of `engine/provenance.js` forbids you to publish a figure from it.

**WHOLE GAMES, MIDDLE ARM, THREE LATTICES.** For each lattice artifact, read `state.games` and `state.games_board_never_diverged`. Subtract the second from the first. That is the board-material count. `--games 1200`: 961 − 961 = 0 (`data/game-differential.json`). `--games 1350`: 1069 − 1069 = 0 (`data/game-differential.g1350.json`). `--games 1950`: 1497 − 1497 = 0 (`data/game-differential.g1950.json`). The undeclared narration-only count is 0 on each lattice. The raw count is 0, 1 and 2. The declared row is Supreme Overlord `fallenundefined`. The clause subtracts it 0, 1 and 2 times. `state.games_void_excluded` is 0 on each lattice. `state.games_cut_off_by_the_turn_cap` is 0 on each lattice. `turns_cap` is 50. `end_state[0].summary.verdicts` shows THREW 1, 1 and 2. In these four games, the authority refused a click of a disabled move that the driver drew, and the game stopped at that click. The boards agreed at each boundary before the stop. This is a defect of the driver, not of the engine.

**THE SAMPLE.** Read `steering` in each lattice artifact. `--games 1200` (`data/game-differential.json`): `team_pool_teams` 8778, `team_pool_picked` 1968, `team_pool_digest` `0d103fb9fa87`. `--games 1350` (`data/game-differential.g1350.json`): 8778, 2206, `7e7a37ded7fc`. `--games 1950` (`data/game-differential.g1950.json`): 8778, 3069, `a5ce76242f8d`. `input_digest` is `1298f25115e3` on all three. **RULE.** Record every flag with the result.

**CORNER ARMS.** Two arms hold the damage roll at an extreme. They are not part of the gate. They were not measured again on the 7.0.0 release. The last reading is on release `b42b81899631`; CHANGELOG 6.22.0 records it. On that release, board-material was 1 of 961 at top-tie-first and 2 of 961 at bottom-tie-first, and the protocol parted in 12 and 15 games (`data/verification/game-differential-top-tie-first.json`, `data/verification/game-differential-bottom-tie-first.json`) **RULE.** A corner reading is true of the release that it names. Do not write it as a reading of another release. The games that stay apart are end-of-battle clock cards. ROADMAP #584 declares them.

**DAMAGE.** Read `data/engine-diff.json`. `requested` 6000, `compared` 6000, `agreed` 6000, `disagreed` 0, `seed` 20260804, `skipped_multihit` 0, `skipped_ability_multihit` 0, `volley.move_rows` 130, `volley.bond_rows` 12, `volley.no_arrival` 4. Each of the 16 entries in `arms` shows `compared` 6000 and `disagreed` 0. **RULE.** The volley loop is compared now. CHANGELOG 6.1.0 records the change. Do not repeat the older sentence that says the tool never applies a multi-hit move.

**THE DELIBERATE ROSTER.** `engine/legal_scope.js` decides which mechanics are in scope. Read `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`. Items: `total` 148, `out_of_scope` 0, `in_scope` 148, `tested` 148, `differ` 0, `silent` 0, `could_not_stage_in_scope` 0, `unattributable` 0, `deferred` 0. Abilities: `total` 316, `out_of_scope` 116, `in_scope` 200, `tested` 194, `differ` 0, `silent` 0, `could_not_stage_in_scope` 0, `unattributable` 0, `deferred` 6. Moves: `total` 500, `out_of_scope` 3, `in_scope` 497, `tested` 494, `differ` 0, `silent` 0, `could_not_stage_in_scope` 0, `unattributable` 0, `deferred` 3. `plant_anchors`: 22, 63 and 37 checked, `dead` empty, `reds_ran` true. Each red demonstration is CAUGHT. **RULE.** Do not write "unaccounted". Write the category. **RULE.** An entity that no legal body can carry is out of scope. It is not a staging gap (CHANGELOG 6.28.0).

**THE FIXTURES ARE LEGAL NOW.** CHANGELOG 6.24.0 gave the roster a rule: a fixture body must learn the move that it clicks, and a move must go to a body that learns it. CHANGELOG 6.50.1 applied the same rule to the idle click: see *Stage a roster row* in section 2.

**DEFERRED BY THE OWNER.** Will set some rows aside. The roster reports them as DEFERRED-BY-OWNER. They are named, not hidden. Read `deferred` on each row of the roster artifacts. The abilities are Anticipation, Forewarn and Frisk (the effect is a message), Illusion (ROADMAP #160), Pickup (the cost of the fixture against its usage) and Stall (ROADMAP #195). The moves are Axe Kick and Electrify (below the usage shelf of 25) and Copycat. Will, 2026-09-18: *"set frisk aside."* (CHANGELOG 6.49.0).

**CENSUS.** Read `data/mechanics-census.json`. `probed` 955, `live` 955, `armed` 955, `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true.

**STAGED MECHANICS.** Read `data/all-mechanics-fire.json`. `games_played` 4700, `games_threw` 0, `sheets_unassembled` 0, `planner.played.games` 3836. Diverging rows: moves 1 (Gastro Acid, below the reach shelf), abilities 1 (Supreme Overlord, declared), items 0. The mechanics clause prints: *2 diverge, 1 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0*. **CAUTION.** The Axe Kick row shows `board.verdict` STATE: the confusion counter of the target is 1 in MEDICHAM and 2 in the authority. The row has no protocol divergence, so the clause does not count it. Axe Kick is DEFERRED-BY-OWNER in the roster. The harness plays the fixtures that `engine/stage_planner.js` builds. CHANGELOG 6.14.0 records the integration.

**OPEN DEFECTS.** Run `node engine/open_work.js`. Do not type a list of what is open. PASS — *clean: no open row names an instrument that is RED — no open defect is backed by a failing measurement (194 verdict(s) read)*. At HEAD `e85e48c8`, `node engine/open_work.js` shows 201 open rows. 20 of them assert breakage. 18 of those 20 have no instrument that decides them. The clause also shows that `data/register-reality.json` (2026-09-12) is older than `docs/ROADMAP.md`. At CHANGELOG 6.52.0, seven engine leads were known and not fixed: Cursed Body, Triple Axel, Clear Smog, stat-raising items under Contrary, Mold Breaker against Contrary on the boost road, Cud Chew after a switch, and Protean on a status move that fails. All seven are closed. CHANGELOG 6.54.0 fixes six of them. The seventh, stat-raising items under Contrary, was not a defect: no legal item raises a stat (CHANGELOG 6.54.0). **STILL OPEN.** The notes rows record three leads that are not fixed. No register row names them, so the open-defect clause cannot see them. Coaching prints `-fail` after `[notarget]` in one Perish Song game (notes row 6.57.0). Tidy Up has the wrong boost and removal order and no `-activate` line (notes row 6.59.0). A stat refused at −6 must print `-unboost|0` (notes row 6.59.0). None of them parts a lattice game on this release.

**LIMITS OF THE VERDICT.** Run `node engine/coverage.js`. It prints what each verdict does not cover. Each count comes from an artifact. Differential bodies on a REAL spread: none. `declared_gaps` in `data/game-differential.json` shows 17,440 bodies with the nature of the sheet and 96 with Serious. Entities exercised in a real game: 852 of 887, with 68 unmeasurable and 35 measurable but not exercised (`coverage` in `data/game-differential.json`). Ranged mechanics fully staged: 0 of 8. Move leaves whose EFFECT was exercised: 7 of 11, and 76 more leaves print nothing when they fire (`summary.moves.leaf_effect` in `data/all-mechanics-fire.json`). Driver policies the gate quotes: 1, `empirical-click/v1`; 960 of 961 games reached a result (`data/game-differential.json`). **RULE.** A clean verdict is a statement about its sample, its driver and its invented spread. Do not write it as a statement about the ladder.

**PROTOCOL EVENTS.** Read `emittedCount` in `data/protocol-events.json`. `emittedCount` is 45. The last change to it is in CHANGELOG 6.18.0 (commit `d2e86cf1`).
