...
**7.0.0 - THE FOLD-IN OF THE 6.x SERIES. THE MEDICHAM GATE NOW READS THREE TEAM LATTICES AND READS `<<GATE: the first line of `node engine/quarantine.js` on the 7.0.0 release>>`, AND EVERY FIGURE DOWNSTREAM OF THE SIMULATOR IS STILL ABSENT FROM THIS DOCUMENT: RE-RUNNABLE IS NOT TRUE, AND NOTHING WAS RE-RUN.**

| question | artifact | answer |
|---|---|---|
| the gate | `node engine/quarantine.js`, release <<GATE: the 7.0.0 release id, tracked under `data/releases/`>> | `<<GATE: first line of the gate output>>` |
| whole-game / **BOARD-MATERIAL**, the gating clause, on every lattice | `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json` — `--games` `1200`, `1350`, `1950`, middle arm, same pins (ROADMAP #619) | <<GATE: per lattice, `state.games` less `state.games_board_never_diverged` — all three must be 0 — with `state.turn_boundaries_compared` equal to `state.turn_boundaries_identical`, and `state.games_void_excluded` and `state.games_cut_off_by_the_turn_cap` both 0>> |
| whole-game / **NARRATION**, the second gate Will named on 2026-08-22, which starts gating automatically once board-material reads zero on every lattice | the same three artifacts | <<GATE: per lattice, the narration clause's undeclared narration-only count — must be 0 — and every declared row subtracted, by name>> |
...
| the two speed-tie corner arms — published beside the gate, not part of it | `data/verification/game-differential-top-tie-first.json` and `data/verification/game-differential-bottom-tie-first.json` | <<GATE: either the corners re-measured on the 7.0.0 release (board-material and protocol first divergence for each), or "not re-measured on this release; the last reading is on release `b42b81899631`, CHANGELOG 6.22.0">>. The games a corner still parts are the declared end-of-battle clock cards, ROADMAP #584 |
| the damage differential | `data/engine-diff.json` | <<GATE: `compared` and `disagreed` at the midpoint and every arm in `arms` — must be 0 — with `skipped_multihit` and `skipped_ability_multihit` both 0 and `volley.move_rows` / `volley.bond_rows`>> |
| the deliberate roster, items | `data/roster.items.json` | <<GATE: FIRED-AND-BOARDS-MATCH of `scope.in_scope`; FIRED-AND-BOARDS-DIFFER and DID-NOT-FIRE, both 0; `scope.could_not_stage_in_scope`; red demonstrations caught>> |
| the deliberate roster, abilities | `data/roster.abilities.json` | <<GATE: the same fields, plus `scope.out_of_scope` by code, CONTROL-NOT-QUIET and DEFERRED-BY-OWNER>>. Deferred by the owner and named: Anticipation, Forewarn and Frisk (Will, 2026-09-18: *"set frisk aside."*, CHANGELOG 6.49.0) |
| the deliberate roster, moves | `data/roster.moves.json` | <<GATE: the same fields, plus the moves with no legal carrier and the deferred rows>> |
| mechanics live in the census | `data/mechanics-census.json` | <<GATE: `live` of `probed`, `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true>> |
| the staged harness | `data/all-mechanics-fire.json` | <<GATE: `games_played`, `games_threw` 0, `planner.played.games`, and the mechanics clause's sentence on how each diverging row is cleared>> |
| open, known engine defects | `node engine/open_work.js`; the gate's open-defect clause | <<GATE: the clause's PASS line, and the open rows asserting breakage on the publish commit>>. At the last notes row before this draft, seven were named and not fixed (CHANGELOG 6.52.0) — among them Protean converting on a status move that fails, and Clear Smog never clearing the target's boosts. <<GATE: which of the seven are closed at 7.0.0>> |
| what the verdict does not cover | printed by `node engine/coverage.js`, never typed | no differential body plays a real spread (both engines get the same invented one), some entities never occur in a real game, the interior of a hit-count range is reached only by draw, one driver plays every game. <<GATE: the counts `node engine/coverage.js` prints for each of those four rows on the 7.0.0 artifacts>> |
| what is withheld, and what a lift would mean | `node engine/quarantine.js`; ROADMAP #57 | read the gate in the first row. **No figure downstream of the simulator is quoted here either way**: while the gate is shut they are withheld by it, and when it opens they become RE-RUNNABLE rather than true. **None was re-run for 7.0.0** — Will's instruction is that the downstream set stays stale until MAG and MILTANK are reworked. The documents' own quarantine check keeps charging a downstream figure that does not name the current release even with the gate open (CHANGELOG 6.30.0) |
...
STILL THE POINT.** `docs/RUNNING-NOTES.md` rows 6.0.1 through <<GATE: the last notes row before the
7.0.0 entry>> are folded in here; each figure lands beside the artifact or the CHANGELOG entry that
...
game-end residuals and die addresses (6.52.0). <<GATE: any engine fix after 6.52.0 that the 7.0.0
release carries, one line each with its CHANGELOG version>>.
...
describes; what is true now is the first row of the 7.0.0 block at the head of this document: `<<GATE: the first line of `node engine/quarantine.js` on the 7.0.0 release, identical to the head block's>>`.
