...
**7.0.0 - THE FOLD-IN OF THE 6.x SERIES. THE MEDICHAM GATE NOW READS THREE TEAM LATTICES AND READS `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`: EVERY GATING CLAUSE PASSES, INCLUDING THE BLIND-SPOT CLAUSES. THAT IS NOT A CLAIM THAT MEDICHAM IS CORRECT — WHAT IT DOES NOT COVER IS THE SECOND TABLE BELOW — AND EVERY FIGURE DOWNSTREAM OF THE SIMULATOR IS STILL ABSENT FROM THIS DOCUMENT: RE-RUNNABLE IS NOT TRUE, AND NOTHING WAS RE-RUN.**

| question | artifact | answer |
|---|---|---|
| the gate | `node engine/quarantine.js`, release `6180c4712761` (CHANGELOG 6.69.0), tracked under `data/releases/` | `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`. Ten clauses, all PASS, including the blind-spot clauses added 2026-09-19 (CHANGELOG 6.64.0, 6.66.1, 6.67.1) |
| the blind-spot clauses | `engine/quarantine.js`; CHANGELOG 6.64.0, 6.66.1, 6.67.1 | Will, 2026-09-19: *"stop saying medicham is done when all these blind spots remain"*. The gate now also fails on: an in-scope mechanic not proven to fire (or resolve on both engines); a board that parts while the protocol agrees; a leaf that can stand at a turn boundary uncompared (a new, tenth clause); a lattice game that threw; a board that parts in a staged row's control arm; and a FIRED credit on a live control that the subject's own receipt does not earn. Under these, the 6.61.0 open reading on `4c9b0cc4a4da` would not have opened; `d92bdfb50d88` read CLOSED, 2 of 10 (CHANGELOG 6.65.0) |
| whole-game / **BOARD-MATERIAL**, the gating clause, on every lattice | `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json` — `--games` `1200`, `1350`, `1950`, middle arm, same pins (ROADMAP #619) | `--games 1200`: 961 − 961 = **0**; `1350`: 1069 − 1069 = **0**; `1950`: 1497 − 1497 = **0**. `threw` 0 / 0 / 0. Turn boundaries 10,716 = 10,716, 11,856 = 11,856, 16,725 = 16,725; `state.games_void_excluded` 0 and `state.games_cut_off_by_the_turn_cap` 0 on all three. Pins: census `57deebd09273`, driver `faf70ecbca71`, pools `0d103fb9fa87` / `7e7a37ded7fc` / `a5ce76242f8d`. The four games that threw on `4c9b0cc4a4da` now play to the end: the driver no longer clicks a move the authority refuses (CHANGELOG 6.63.0) |
| whole-game / **NARRATION**, the second gate Will named on 2026-08-22, which starts gating automatically once board-material reads zero on every lattice | the same three artifacts | **0 / 0 / 0** undeclared; raw 0 / 1 / 2, every one the declared Supreme Overlord `fallenundefined` row (the authority prints an undefined value on a `[silent]` line) |
| why three lattices | CHANGELOG 6.45.0 and 6.46.0 | `--games` picks WHICH teams play, by a stride computed from it, so one lattice is one sample. On one release the gate's `--games 1200` run parted 0 boards in 961 games while a `--games 12000` run from the same frozen pool parted 84 in 7178 (CHANGELOG 6.45.0). The gate now requires zero on three lattices chosen to share few teams, and a missing, stale or duplicate lattice is CANNOT-ANSWER, never a pass |
| the two speed-tie corner arms — published beside the gate, not part of it | `data/verification/game-differential-top-tie-first.json` and `data/verification/game-differential-bottom-tie-first.json` | not re-measured on this release; the last reading is on release `b42b81899631`, CHANGELOG 6.22.0: board-material 1 of 961 (top-tie-first) and 2 of 961 (bottom-tie-first), protocol divergence in 12 and 15 games. The games a corner parted were the declared end-of-battle clock cards, ROADMAP #584 |
| the damage differential | `data/engine-diff.json` | `compared` 6000, `disagreed` 0 at the midpoint and at each of the 16 arms (`top`, `bottom`, `idx01` to `idx14`); `skipped_multihit` 0, `skipped_ability_multihit` 0; `volley.move_rows` 130, `volley.bond_rows` 12 |
| the deliberate roster, items | `data/roster.items.json` | 148 of 148; DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0; 22 of 22 red demonstrations CAUGHT, 22 anchors, none dead |
| the deliberate roster, abilities | `data/roster.abilities.json` | 194 of 200; DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0; 63 of 63 CAUGHT; out of scope 116 (no legal carrier 114, validator-refused 1, no legal reader 1); CONTROL-NOT-QUIET 0; DEFERRED-BY-OWNER 6: Anticipation, Forewarn, Frisk (Will, 2026-09-18: *"set frisk aside."*, CHANGELOG 6.49.0), Illusion (ROADMAP #160), Pickup, and Stall (ROADMAP #195) |
| the deliberate roster, moves | `data/roster.moves.json` | 494 of 497; DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0; 37 of 37 CAUGHT; 3 with no legal carrier, out of scope; DEFERRED-BY-OWNER 3 — Axe Kick, Copycat and Electrify |
| mechanics live in the census | `data/mechanics-census.json` | 969 of 969; `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true |
| the staged harness | `data/all-mechanics-fire.json` | 4638 games, 0 threw, `red_ok` true, `planner.played.games` 3936. Proof in scope: abilities FIRED 198 of 200 (Frisk DID-NOT-FIRE and Illusion UNPROVEN-UNCONTROLLED, both owner-excused), moves 497 of 497 resolved on both engines, items 148 of 148. Control watch: every FIRED row on a live control is earned by the subject's own receipt; 0 are not. Control arms: 2 of 841 parted, 0 on a board. *0 diverge (+1 board-only), 0 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0* — the board-only row is Axe Kick |
| open, known engine defects | `node engine/open_work.js`; the gate's open-defect clause | PASS — no open row names an instrument that is red (194 verdicts read). `node engine/open_work.js` at HEAD `165b338c`: 585 register rows, 201 open, 20 asserting breakage, 18 of them with no deciding instrument; the clause warns that `data/register-reality.json` (2026-09-12) is older than the register. The seven leads of CHANGELOG 6.52.0 are closed (6.54.0); Tidy Up's order and `-unboost\|0` at −6 are fixed (6.63.0). Still open, named by no register row, and parting no lattice game: Coaching's `-fail` after `[notarget]` (notes row 6.57.0) |
| what is withheld, and what a lift means | `node engine/quarantine.js`; ROADMAP #57 | the gate is open, so every artifact `node engine/quarantine.js` lists as downstream of MEDICHAM is RE-RUNNABLE — no longer withheld, and not true. **No figure downstream of the simulator is quoted here**. **None was re-run for 7.0.0** — Will's instruction is that the downstream set stays stale until MAG and MILTANK are reworked. The documents' own quarantine check keeps charging a downstream figure that does not name the current release even with the gate open (CHANGELOG 6.30.0, 6.62.0) |
| leaf calibration — MEASURE's one number | `data/winrate-backtest.json` | **not re-run.** No sample size, no reliability curve, no calibration gap and no verdict — and no direction may be inferred from the absence. It becomes quotable only when `node engine/backtest_winrate.js` has been re-run |

**WHAT THE GATE DOES NOT COVER.** Every gating clause passes, including the blind-spot clauses; each row below survives that, and each is why this document does not call MEDICHAM correct or done.

| limit | where it is read | what stands |
|---|---|---|
| Illusion | ROADMAP #160; `closet.teams_dropped` in `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json` | closeted and unmodelled; Illusion teams dropped from the pool (43, 63 and 71 teams on the three lattices); the staged row is UNPROVEN-UNCONTROLLED |
| Frisk — **OPEN, being fixed** | `data/all-mechanics-fire.json` | DID-NOT-FIRE; excused only by the owner's deferral. The authority announces Frisk only for a foe that holds an item, and the fixture's foes hold none |
| Axe Kick — **OPEN, being fixed** | `data/all-mechanics-fire.json` | the staged board parts with no line difference: the target's confusion counter reads 2 on the authority and 1 here. Judged below the reach shelf, and owner-deferred in the roster, so no clause fails on it |
| the sample | `state.games` and `steering.team_pool_teams` in `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json` | three lattices of 961, 1,069 and 1,497 games from ONE frozen open-sheet pool of 8,778 teams, `turns_cap` 50, one driver (`empirical-click/v1`). Rule-of-three bound on the largest lattice: about 3 / 1497 ≈ 0.20% of games (Hanley and Lippman-Hand, 1983), generous because the lattices share teams |
| closed sheets and best-of-one | — | not sampled; the pool is open-team-sheet play only |
| spreads | `node engine/coverage.js`; `declared_gaps` in `data/game-differential.json` | no differential body on a real spread; both engines get the same invented one (natures real: 17,440 from the sheet, 96 fallen back to Serious) |
| effects that print nothing | `summary.moves.leaf_effect`, `data/all-mechanics-fire.json` | 76 leaves print nothing when they fire; of the 11 that print, 7 had their effect exercised and 4 resolved on the announcement alone |
| uncomparable leaves | `node engine/coverage.js` | every leaf that can stand at a turn boundary is compared; the leaves the board comparator can never see are covered only because another instrument fired on them, and one, `volatile:powershift`, by none |
| entities never exercised in a real game | `coverage` in `data/game-differential.json` | 860 of 901 exercised; 68 unmeasurable, 41 measurable and not exercised |
| ranged mechanics | `node engine/coverage.js` | 0 of 8 fully staged by an arm; the interior is reached by draw |
| tags | `node engine/coverage.js` | two in-scope tags, `needsUntrackedState` and `readsOwnItem`, have no engine consumer |
| declared divergences | `DECLARED_DIVERGENCE`, `engine/quarantine.js` | one row, Supreme Overlord `fallenundefined` |
| downstream models | ROADMAP #57 | re-runnable, **not re-run**, by Will's ruling |

**WHY THIS IS A MAJOR, IN THE FORM THE RULE ASKS FOR.** Four things a reader can no longer be told.
(1) That the whole-game zero is the same series as 6.0.0's: 6.0.0 read one team lattice, and 7.0.0
requires zero on three that share few teams (CHANGELOG 6.45.0, 6.46.0). The `--games 1200` reading
links; the other two extend it into teams the 6.0.0 gate never played. (2) That a gate reading
describes the same question 6.0.0 answered in the lab: a staging planner derives a legal fixture, a
one-variable control and a failure plant for each mechanic instead of the harness's hand-picked bodies,
the harness plays those fixtures, scope is decided by one module, and twenty register rows that asserted
a defect with nothing deciding them gained a probe (CHANGELOG 6.9.1, 6.10.0, 6.13.0, 6.14.0).
(3) That the roster's and the harness's denominators and greens mean what they meant: unifying scope
moved which entities are counted, a could-not-stage row now counts only when the entity is in scope
(CHANGELOG 6.28.0), and the roster's control arm was found three times to be measuring something other
than the subject, so greens that rested on it were refused (CHANGELOG 6.34.0, 6.36.0, 6.39.0). The
tested-of-in-scope pairs above therefore cannot be lined up against the pairs in the 6.0.0 block by
arithmetic. (4) That a PASS means what it meant at 6.61.0: the gate grew from nine clauses to ten, and
its mechanics clause now requires proof of every in-scope row, an earned credit on a live control and a
quiet control arm (CHANGELOG 6.64.0, 6.66.1, 6.67.1). That is the test for a major revision rather than a routine one, and it is why the whole
document set is rewritten here instead of restamped.

**THE FOLD-IN COVERS EVERY NOTES ROW FROM 6.0.0 TO THE RELEASE, AND THE ROWS THAT CARRY NO FIGURE ARE
STILL THE POINT.** `docs/RUNNING-NOTES.md` rows 6.0.1 through 6.69.0 are folded in here; each figure lands beside the artifact or the CHANGELOG entry that
holds it. Most of the series is instrument work, which moves no published figure and is recorded in
*Measurement validity* below rather than in this table.

**WHAT THE ENGINE FIXED SINCE 6.0.0, IN ONE LINE PER RELEASE.** Each was shown red against the official
simulator first, each sits behind its own `MEDI_*` switch, and each is named in the CHANGELOG entry
beside it: Disguise's crit refusal, three accuracy roads and a Flash Fire absorb (6.5.0); Encore ending
when the encored move runs out of PP (6.6.0); thirteen speed-tie corner mechanisms (6.7.0); Simple
conferred by Simple Beam, Magnet Rise's clock, a second Psychic Noise, Parental Bond's per-hit
secondaries and a rampage's length (6.11.0); five moves that fail on an ability flag (6.12.0); the
per-battle faint epoch (6.14.1); the eight partings the planner found — Oblivious refusing Taunt, Magic
Bounce sending Spite back, Focus Energy with Super Luck or a Scope Lens, a mega stone under Klutz, and
the narration of Cute Charm, Own Tempo, Sweet Veil and Covet (6.18.0); a survival clamp answering each
hit of a volley, for Endure, Focus Sash, Sturdy and Focus Band (6.22.0); a residual sleep from Yawn
never offered to its Chesto Berry (6.24.0); a perish-zeroed body running its later residual handlers
(6.26.0); the Early Bird sleep counter (6.41.0); Heal Bell, Corrosive Gas's order and Reflect Type's
turn-boundary line (6.44.0); an Electric charge surviving an aborted or status click, and a mega
evolution skipping the outgoing ability's end (6.45.0); Beat Up's set species (6.47.0); Future Sight
against a Substitute, Harvest on a non-berry, and a target-aware Prankster (6.48.0); Mold Breaker
through Good as Gold, Magic Bounce and Sticky Hold (6.49.0); Gooey's drop road, Mummy and Wandering
Spirit's flags, Charge on an early exit, the Ally Switch counter on the bench, Transform's copied start
and Parental Bond after a KO (6.50.0); and fifteen single-game leads across move effects, turn order,
game-end residuals and die addresses (6.52.0). Then the last leads (6.54.0): Protean on a refused Try, Clear Smog, Mold Breaker against Contrary, Cud Chew on switch-out, Copycat and `failcopycat`, and on-hit chance abilities rolling per landed hit with Triple Axel's accuracy per hit; Sucker Punch reading the queued move after Encore, Focus Sash and Sturdy against a Future Sight payout and a confusion self-hit, thirteen narration mechanisms and four turn-order mechanisms (6.57.0); Forewarn's announcement and die, Harvest's coin every turn, and six more narration lines (6.59.0). Board-material on the three lattices read 0 / 1 / 2 (6.53.0), 0 / 1 / 1 (6.55.0) and 0 / 0 / 0 (6.58.0); undeclared narration reached 0 / 0 / 0 (6.60.0); the gate opened at 6.61.0 on release `4c9b0cc4a4da` under nine clauses. Then the blind-spot clauses (6.64.0, 6.66.1, 6.67.1) and the fixes they aimed: Unburden as a compared leaf and a carrier that never doubled after losing a stolen item, Gastro Acid suppressing nothing, Knock Off and Poltergeist under Magic Room and Klutz, Ripen, Anticipation, Screen Cleaner, the switch-in lines of Pressure, Mold Breaker and Unnerve, Tidy Up's order, `-unboost|0` at −6, and a driver that clicked moves the authority refuses (6.63.0); Anger Point before the hit's secondary (6.66.0); Sticky Hold's refusal line and a fainting holder's item (6.68.0). The gate read CLOSED, 2 of 10, on `d92bdfb50d88` (6.65.0) and OPEN, 10 of 10, on `6180c4712761` (6.69.0).

**CORRECTION, 2026-09-11 - THE GATE IS CLOSED AGAIN. THE 6.0.0 BLOCK BELOW IS DATED EVIDENCE; ITS "MEDICHAM IS CORRECT" AND ITS GATE ROW READING OPEN ARE NOT TRUE TODAY.**

| question | artifact | answer |
|---|---|---|
| the gate | `node engine/quarantine.js`, HEAD `bf2d594f`, `2026-09-11T12:21Z` | `GATE: CLOSED — 2 of 9 GATING clauses fail`. The other seven pass, both whole-game clauses among them |
| failing clause — no open, known engine defect | same | `10 OPEN roadmap row(s) name an instrument that is RED`: ROADMAP #535, #529, #318, #348, #375, #380, #412, #425, #467, #511 |
| failing clause — mechanics staged and compared against Showdown | same | `9 of 16 DIVERGING MECHANICS ARE PLAYED AND UNCLEARED`, all among the partings registered as ROADMAP #593–#600 |
| why it closed | CHANGELOG 6.13.0 and 6.14.0 | **better instruments, not a worse engine.** Twenty register rows gained a deciding probe and ten read RED; the staging planner went inside the harness and staged mechanics that fired rose from 740 to 783 of 845. **The defects are real**: they were in the engine at 6.0.0, which had no instrument that staged them |
| the new partings | same | board: Oblivious lets Taunt through; Magic Bounce does not bounce Spite; a Super Luck or Scope Lens holder misses the guaranteed critical hit after Focus Energy; a mega stone held under Klutz does not evolve. Narration only: Cute Charm, Own Tempo, Sweet Veil, Covet |
| the pinned real-game pool | `data/game-differential.json`, release `aefcb93baf14`, generated `2026-09-11T11:23:47Z` | **unmoved.** `state.games` 961 less `state.games_board_never_diverged` 961: board-material 0 of 961 on the middle arm |
...
**UPDATE, 2026-09-11 (6.22.0 to 6.26.0):** the two rows that clause named were closed — #511 by the
survival clamp (6.22.0), #318 by the roster's fixtures learning their moves by rule (6.24.0) — and #601
and #440 closed at 6.26.0. The reading that stands today is the first row of the 7.0.0 block above; the
2-of-9 and 1-of-9 readings here are kept as dated evidence of what was true when they were taken.
...
| question | artifact | answer |
|---|---|---|
| whole-game / **BOARD-MATERIAL**, the gating clause | `data/game-differential.json` | **0 of 961.** `state.games` **961** less `state.games_board_never_diverged` **961**. `state.games_void_excluded` 0; `state.games_cut_off_by_the_turn_cap` 0 at `turns_cap` **50**, longest game turn **36**; `state.turn_boundaries_compared` **10,705** = `state.turn_boundaries_identical`; `state.first_board_divergences` empty |
| whole-game / end state | same | `end_state[0].summary.verdicts`: SAME-END-STATE **960**, DIFFERENT-END-STATE 0, ENDED-APART 0, NO-COMPARABLE-BOARD 0, THREW 1 (a harness choice rejection in `errors[0]`) |
| whole-game / **NARRATION**, the second gate Will named on 2026-08-22 | same | **0 undeclared of 961.** `state.protocol_diverged_board_never_did` is 1 and the 1 declared row is the perish drain above `\|upkeep\|`, closeted 2026-08-28, ROADMAP #440. A lower bound: a game records its first divergence only |
| the bound on the word "board" | `tests/probe_uncompared_leaves.js`, `state.not_compared` | **54 of 80** leaves compared, and 54 is the `ceiling`: of the `hole` of **20**, **18** are duration-1 leaves ended in the residual and 2 remove themselves inside the action, so `standing_at_the_boundary` is 0. Beside them 8 board fields are declared uncompared with a reason each. Within-turn erasure measured **2 of 19** on the batch V population |
| the sample, which is part of the claim | `data/game-differential.json` `steering` | 961 real open-sheet games, driver `empirical-click/v1` over real recorded clicks, pool `data/team-pool-frozen` — `steering.team_pool_teams` **8,778**, `steering.team_pool_picked` **1,968**, digest `0d103fb9fa87`; mode A pins `de38d17e15a2`; census pin `ab219c68f165`; flags `--games 1200 --turns 50 --arm middle`. **43** teams dropped for Illusion; spreads synthetic, natures real |
...
| the gate | `node engine/status.js` | **9 of 9 clauses PASS — OPEN**, release `cbd510bc2b13`, authority commit `20ad99ff…` |
| what becomes re-runnable | `node engine/major_readiness.js` | **40** artifacts, leaf calibration (`data/winrate-backtest.json`) among them. **None was re-run.** Will, 2026-09-09: leave them stale until MAG and MILTANK are reworked |
| what stays withheld, by decision | same | **24** — every generator that writes or reads `data/policy-weights.json`, or is MILTANK: the exploitability triple, both exploit-step probes, the opponent model, `recall-at-k`, the ladder, the scoreboard, `partial-label-em`, `brood`, `censoring-value`, `collinearity-audit`, `seed-source-audit`, `sheet-channel-value`, `weight-multiplicity`, `rollout-r3`, the search decision profile and the three weight files |
| leaf calibration — MEASURE's one number | `data/winrate-backtest.json` | **Not stated in this version.** It lifts on a re-run and was not re-run. No replication verdict, no bucket share, no calibration gap, no sample size — and no direction may be inferred from the absence |

**WHY THIS IS A MAJOR, IN THE FORM THE RULE ASKS FOR.** Three things a reader can no longer be told.
(1) That MEDICHAM is not correct — the sentence that has stood under every model figure in this project
since 3.79.0. The gate is open, and 40 withheld artifacts stop being withheld and become re-runnable at
once, so nothing published downstream survives unrewritten. (2) That a board-material figure measured at
a turn cap of 20 covers whole games: 35 of 961 games were still running when that instrument stopped
watching, and the cap-50 series is not the cap-20 series without a sample-identity check. (3) That a
level measured before the 2026-09-07 dice-addressing fix belongs to the same series as one after it.
The old series is back-cast beside the new in this release's `docs/RUNNING-NOTES.md` row, so the two
can be linked.

**AND IT IS A PARTIAL LIFT, WHICH IS SAID HERE RATHER THAN FOOTNOTED.** Announcing "the quarantine
lifted" while 24 artifacts are still withheld — and while all 40 of the others are stale by the owner's
instruction — would be the PRE-CHANGE caption in a new costume, and this repository has paid for that
twice. Every figure that is absent from this document is absent with its generator named.
...
**AND A HASH WAS PASSING FOR A MEASUREMENT.** The figure lexer now derives its exclusion from ROLE — a digit run glued to a letter is part of a token — which stopped `data/policy-weights.json`'s integer `featureHashes` matching published figures by arithmetic coincidence, and **unmasked four documents restating `data/protocol-events.json` with the **stale** pair 38 / 56 where the artifact reads `emittedCount` 44 and `notEmittedCount` 50**. Corrected in the white paper, `docs/GAME-DIFFERENTIAL-DESIGN.md`, `docs/MODELS.md` and this file.
...
artifact was produced by a simulator the gate does not certify. **At 6.0.0 the gate is OPEN, so this
artifact is RE-RUNNABLE - and it was not re-run**, by the owner's instruction of 2026-09-09 that the
downstream set stays stale until MAG and MILTANK are reworked. The table that stood here carried the sample size,
...
on one command, when the owner asks for it: `node engine/leaf_engine_contrast.js`.

**CORRECTED 2026-09-11: THE GATE IS CLOSED AGAIN, SO THIS ARTIFACT IS WITHHELD BY THE GATE, NOT
MERELY UNRE-RUN.** `data/leaf-engine-contrast.json` is on the withheld list `node engine/quarantine.js`
prints. It becomes quotable only when the gate opens again AND `node engine/leaf_engine_contrast.js` is
re-run. Why the gate closed is at the head of this document.

**7.0.0: WHICHEVER WAY THE GATE READS, THIS ARTIFACT CARRIES NO FIGURE HERE — IT WAS NOT
RE-RUN.** The gate reading is the first row at the head of this document. An open gate makes this
measurement re-runnable and does not make it true.
...
neighbour. **Read every figure from `f038cdb3:data/wire-ladder.json`**, the eleven-arm replay (the file on disk was replayed again that day and holds other counts) — the figures below moved when ROADMAP
...
**7.0.0 — phase 1 was reopened, was worked, and its state today is the gate rather than this
box.** Instruments built after 6.0.0 found real defects in an engine 6.0.0 had already called correct
(CHANGELOG 6.13.0, 6.14.0). The line in the box is left as written, because it was true of the release it
describes; what is true now is the first row of the 7.0.0 block at the head of this document: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`.
...
|---|---|---|---|
| **MEDICHAM** | Hand-written doubles battle simulator. **Its justification is now falsifiable (ADR-003, 3.62.2): it exists so per-turn re-solving is affordable, so the engine work is justified if and only if search pays — gated by ROADMAP #62.** The speed ratio that originally justified it is corrected in the section above | **7.0.0: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`, release `6180c4712761`; every gating clause passes, including the blind-spot clauses — not a claim that it is correct, and its limits are the second table at the head of this document.** Read on three team lattices of the pinned pool rather than one (CHANGELOG 6.46.0), beside the damage differential, the three deliberate-roster stages, the census and the staged harness; every reading is in the 7.0.0 block at the head of this document and is not restated in this cell. Everything after this sentence in this cell is dated evidence. **CLOSED AGAIN, 2026-09-11: `GATE: CLOSED — 2 of 9 GATING clauses fail`** — the open-defect clause and the staged-mechanics clause, both failing on instruments added after 6.0.0 that see real defects; board-material on the pinned pool is unmoved. See the correction at the head of this document. The 6.0.0 reading below is dated evidence. **The MEDICHAM gate reads OPEN at 6.0.0 — 9 of 9 clauses PASS on release `cbd510bc2b13`.** Board-material 0 of 961 with none excluded; narration 0 undeclared of 961 with one declared row; 54 of 80 board leaves compared and 8 fields declared uncompared; one driver, real teams, synthetic spreads. The two speed-tie corner arms part 16 and 15 of 961 and are published beside it | **EVERY COUNT IN THIS CELL WAS SUPERSEDED AND IS NOW CUT (2026-08-22).** It stated a mechanics census, an interaction-matrix agreement and coverage fraction, a damage-differential ratio, a scenario count, a win-probability gap against the official engine, a two-rulebook collision ratchet, a DEAD-tag ratchet and a mutation-tier count. Every one of those instruments has been re-run since the cell was written and every artifact now reads something else. This is a CURRENT-STATE table, so a figure in it that is a release old reads as a claim about tonight — which is the failure the whole document set exists to prevent. State is printed, not typed: run **`node engine/status.js`** for the gate, the census, the differentials and the withheld set, and **`node engine/quarantine.js`** for the full derivation. ADR-001 stands: MEDICHAM becomes a lookup over precomputed tables. The win-probability gap against the official Champions engine is a rollout figure: it lifts on a re-run, it was NOT re-run for 6.0.0, and it is therefore absent rather than restated. **CORRECTED 2026-09-11:** with the gate closed again it is withheld by the gate, and a re-run alone does not make it quotable. **7.0.0: still absent, and now for the plainer reason — it is a rollout figure and it was NOT re-run** (ROADMAP #57). |
| **GURU** | Meta matchup matrix from real outcomes | ⚠️ **No decisive cells that survive multiplicity** | `data/guru-matchups.json`, 2026-07-31, **5,265 clean games / 12 archetypes**, a 12 x 12 `matrix`. **6 directed = 3 distinct** matchups clear a 95% test one at a time, and **ZERO survive FDR at q=0.05 or Bonferroni** — 66 pairs, 3.3 expected by chance, 3 observed, smallest exact p 6.1e-3 against a BH threshold of 7.6e-4. Predictive test **0.7124** vs a coin 0.6931 over 1,053 held-out games — **worse than a coin**. Descriptive structure only. (This row read *1,124 clean games, 11 archetypes, 0.735* until 2026-08-04, from a superseded run; the verdict is unchanged.) |
| **XATU** | Opponent set + next-move belief | ✅ Built | Behaviour-clone top-1 **0.293** / top-3 **0.6459** on 118,274 held-out human clicks, cross-entropy 2.3365 (`data/policy-eval.json:species_only_clone`) against `data/policy-eval.json:baselines` 4.7124 and 3.6978 — beats both ( re-run 2026-09-09 under quality filter 1.3.0; the rounded pair this row carried until 2026-09-09 was in no artifact) |
...
| **ROLES** | Multi-label team composition (52 roles, `data/pokemon-roles.json:roles`; the "26" here until 2026-09-09 was typed and stale) | ✅ Built | Role-pair matrix pools data to median cell **n=20** across 1,051 cells (vs old single-label n=11–18) — the 7,971 once published was retracted in 2.7.0; preview roles tie a coin (honest null) |
...
| **NMF** | Emergent roles / archetypes | ⚠️ **Rank read, not yet defended** | **Rank 4 ships**, read by `engine/nmf_roles.py` from `data/nmf-rank-selection.json:most_reproducible.rank` since 2026-09-09 (`data/nmf-roles.json:archetype_rank` 4, `archetype_recon_error` 0.738, `n_team_sides_role_matrix` 63,882 × `n_roles_role_matrix` 52). The hand-set rank 6 that shipped before scored **−0.107** excess over a shuffled null on the project's own criterion (`engine/nmf_rank.py`, bootstrap factor stability, cf. Brunet et al. 2004); rank 4 scores **+0.0775** — but the selection is from 2026-07-28 at `bootstrap_pairs` 6, so `python engine/nmf_rank.py 50` is owed before the rank counts as defended. Reconstruction error **cannot select a rank** (it falls monotonically by construction). A team is a *blend*, learned not hand-labelled |
| **MILTANK, PORYGON2, DODUO, GARY** | the models downstream of MEDICHAM that read a rollout or the fitted weights — each described in the living [model ledger](MODELS.md) | **Not re-run** | **No figure appears here for any of them.** They are re-runnable rather than true, and Will's instruction of 2026-09-09 is that the downstream set stays stale until MAG and MILTANK are reworked. The re-run list is ROADMAP #57. A figure absent from this row is absent with its generator named in that ledger, not softened |
...
**7.0.0 — THE FIVE DIVISIONS, AND WHAT THE 6.x ROWS LEAVE EACH ONE OWING.** Work is cut five
ways — ENGINE, MEASURE, SEARCH, OPS, WEB — along the invalidation graph, so a change in one does not
silently invalidate the others (`docs/DIVISIONS.md`). ENGINE owns the simulator and the optimization pass,
which is planned from a profile and lands nothing until a before/after harness proves the boards
byte-identical (CHANGELOG 6.12.2). MEASURE owns the rulers and the re-run list, ROADMAP #57, which stays
untouched by the owner's instruction, and the MAG refit, which is a REFIT and not a restamp. SEARCH may
now assume the copy cost and the position independence recorded under *Measurement validity*, and nothing
beyond them. OPS owns the store and owes the hash-bound correction mechanism named above. WEB renders and
never authors a number. **This is the shape of the work, not its state** — `node engine/status.js` and
`node engine/open_work.js` print the state, and a list typed here would be stale within a day.
...
**7.0.0 — THE STORE, FOLDED IN FROM THE 6.x ROWS.** The store is sharded and neither
monolith is tracked: 30 parsed shards and 26 raw, a routine shard about 4 MB, and the largest tracked
blob under `data/` is the 58.7 MB recovery shard carrying the 15,862 recovered games. A dated shard is
written once and does not grow, so nothing is on a trajectory to GitHub's 100 MB limit — the limit itself
is unchanged and still binding, only the deadline is gone (CHANGELOG 6.3.0). The quality filter is at
version 1.4.0 (`data/quality-filter.json`), which adds `exclude_corrupt_winner`: a DECLARATION naming the
two ladder rows whose recorded winner is neither player, not a check that detects them, so it cannot
outlive the defect it names. The ladder store was not rewritten to do it — 92,594 rows, no store byte
written (CHANGELOG 6.6.1). A hash-bound store-correction mechanism is owed to OPS, and until it exists a
corrupted row can be excluded but not corrected.
...
Measured 2026-07-25 against Smogon's whole-ladder statistics for the same format and month, whose
**previously** published battle count for that month was 1,163,315, set against our own count as it is
generated into `data/live.js`:
...
| exploitability | **no figure.** The earlier figure is retracted on its own merits; the 2026-08-04 re-run is `void: true`. |
| mirror control | **withheld** (withdrawn 2026-09-11). It was published here as surviving the void run; the artifact holding it is withheld by the gate, so the seat-asymmetry worry is open again rather than retired. |
| MAG refit | ran. **Its measured effect is withdrawn** (2026-09-11): the paired table traces to no artifact and compared two MAG vectors the gate withholds, so no effect, interval, noise floor or verdict is carried. |
| open, needs a decision | the fit sees `{nature, item}`; the player sees `{nature, item, ability, moves}`. The share of trained decisions this touches is not carried: no artifact backs it, and it is a count over the withheld MAG fit. |
| click censoring | **CORRECTED 2026-09-11: WITHHELD BY THE GATE, WHICH IS CLOSED AGAIN; "lifts on a re-run" in the next sentence no longer holds.** **FIGURES ABSENT — the artifact lifts on a re-run and was not re-run for 6.0.0.** `data/click-censoring-census.json` is downstream of MEDICHAM (`engine/click_census.js` reaches the simulator through `require`) and was measured on an engine that no longer exists. This cell used to state the count of recorded actions that were never clicks, the redirected-attack share and the paired held-out effect; those are cut rather than captioned. The QUALITATIVE finding does not depend on them and stands: actions the game FORCED — Encore, `\|drag\|` — were being fitted as if a human had chosen them, and a redirected attack now enters the fit as a two-member candidate set instead of a certainty on the redirector. `data/click-censoring-census.json`, `data/censoring-value.json` |
| the staging planner (7.0.0) | a legal fixture, a one-variable control and a failure plant are DERIVED for every mechanic instead of hand-picked — 841 of the 847 with a legal carrier — and the hand-picked bodies had been hiding mechanics (CHANGELOG 6.9.1). The staged harness plays them (CHANGELOG 6.14.0) |
| one scope (7.0.0) | which mechanics exist in Reg M-B is decided by `engine/legal_scope.js` for the harness, the deliberate roster and the tag builder. Two derivations had disagreed on three rows in both directions, hidden inside totals that differed by two (CHANGELOG 6.10.0) |
| the coverage counters (7.0.0) | a board is counted only where the mechanic actually fired, over the in-scope denominator rather than over mechanics that cannot occur in the regulation (CHANGELOG 6.9.0) |
| the register (7.0.0) | twenty rows that asserted a defect with nothing deciding them now name the probe that decides them, pinned to an engine release; three of those claims were refuted by their own probe (CHANGELOG 6.13.0). What is open is PRINTED — `node engine/open_work.js` — and is not typed into this document |
| exit codes (7.0.0) | seven checks exited 0, which reads as agreement, after failing to stage their board; the readers of exit codes now share one classifier, `engine/exit_codes.js` (CHANGELOG 6.12.1) |
| release tracking (7.0.0) | RULE 5 in `engine/provenance.js`: a published figure may not rest on an engine release whose bytes are not in this repository. The published set is DERIVED from the living documents and the division ledgers rather than listed, and the check asks the release manifest rather than the directory (CHANGELOG 6.2.0) |
| figure traceability (7.0.0) | a figure is traced only by a trace bound to it — an artifact its own paragraph cites and that holds the value, or the CHANGELOG entry its block names. The old rule accepted a match anywhere in `data/`, which almost nothing could fail. Existing unbound figures, 2,258 across 24 living documents, are grandfathered by a generated list that may only shrink (CHANGELOG 6.6.2) |
| the docs gate (7.0.0) | the pre-commit gate judges the commit being made rather than the working tree, so a staged figure that is not also on disk can no longer pass (CHANGELOG 6.12.3) |
| conformance (7.0.0) | ratchet regressions 107 to 21 (CHANGELOG 6.4.1), then 21 to 5 (CHANGELOG 6.6.0), none of it by typing a value into the baseline |
| what a search may assume (7.0.0) | a position copy costs about a third of one turn in MEDICHAM and its copies are independent, so copying is not what limits search (CHANGELOG 6.9.2). Two positions played alternately stay independent, and the faint counter that had been process-wide is now per battle: a double wipe read as a draw on 43 of 43 constructed cases before the fix and 0 of 43 after (CHANGELOG 6.14.1). Skipping narration inside rollouts would save nothing, because rollouts already run with it off (CHANGELOG 6.12.2) |
| three team lattices (7.0.0) | the whole-game clauses read every sample in `LATTICE_SAMPLES` in `engine/quarantine.js` — `--games` `1200`, `1350` and `1950` — and open only on zero on all three; `engine/game_differential.js` stamps `games_requested` so the sample is recorded rather than inferred. A missing, stale, unpinned or duplicate lattice is CANNOT-ANSWER (CHANGELOG 6.46.0, ROADMAP #619) |
| narration gates on its own clock (7.0.0) | the narration clause reports without gating while board-material is non-zero on any lattice, and holds the gate shut the moment board-material reads zero everywhere. Nothing switches it on by hand: its `gates` flag is the board clause's own verdict (`engine/quarantine.js`, `narrationClause`) |
| an honest control (7.0.0) | a roster row is a one-variable experiment only if its control differs from the subject in the one ability. The control was found measuring the swapper or itself three times, and the greens that rested on it were refused rather than kept (CHANGELOG 6.34.0, 6.36.0, 6.39.0); CONTROL-NOT-QUIET reached zero only after that (CHANGELOG 6.42.0) |
| scope in the counts (7.0.0) | an entity no legal body can carry is out of scope with a code and is not counted as a staging gap, in the verdict buckets, the scope block and the written rows alike (CHANGELOG 6.28.0) |
| legal fixtures at run time (7.0.0) | `engine/fixture_legality.js` checks status codes, genders against the species and every body a test builds through the shared builder, report-only; a directed game hashes identically with it on and off (CHANGELOG 6.48.0). The roster's idle click is now a move each fixture body can learn (CHANGELOG 6.50.1) |
| the gate's own parser and the open-gate docs check (7.0.0) | the gate's register parser read a red marker case-sensitively and cut a status cell that quoted a pipe (CHANGELOG 6.29.0); the documents' quarantine check had gone quiet when the gate opened and now keeps charging a downstream figure that does not name the current release (CHANGELOG 6.30.0) |
| register hygiene (7.0.0) | closed rows whose VERIFIED BY named an instrument nobody can run were re-pointed at instruments that were run green, and `engine/register_reality.js` stopped counting CANNOT-ANSWER as a disagreement (CHANGELOG 6.29.0, 6.47.1) |
| proof of every in-scope mechanic (7.0.0) | the mechanics clause fails on an in-scope ability or item that does not read FIRED with a control arm, a move that does not resolve on both engines, or a missing row; only the owner's `tests/roster.js DEFERRED` map and the Illusion closet excuse one. A board that parts while the protocol agrees is judged as a divergence (CHANGELOG 6.64.0) |
| no uncompared leaf at a boundary (7.0.0) | a tenth gate clause fails when `tests/probe_uncompared_leaves.js` finds a leaf a legal mechanic writes that can stand at a turn boundary uncompared, and cannot answer if `BS.snapshot` has a second caller (CHANGELOG 6.64.0). Unburden was such a leaf and is now compared (CHANGELOG 6.63.0) |
| a thrown game is a finding (7.0.0) | the board-material clause fails on a lattice game that threw, reading `threw` and `errors[]` and taking the larger (CHANGELOG 6.64.0) |
| the control arm and the credit (7.0.0) | a board that parts in a staged row's control arm fails the mechanics clause, excused only when the control mechanic itself is shelved (CHANGELOG 6.66.1); a FIRED row on a live control fails unless the subject's own receipt earns it (CHANGELOG 6.67.1); the planner and the older test path prefer a control measured quiet (CHANGELOG 6.66.0, 6.67.0) |
| a pinned run cannot cut a release (7.0.0) | `engine/game_differential.js` lost its `--release` flag under `node -e` and cut on every re-load; it now reads the right offset, and a child inherits the pin (CHANGELOG 6.65.1, `tests/test-release-pin-no-cut.js`) |
