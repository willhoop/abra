...
**7.0.0 - THE FOLD-IN OF THE 6.x SERIES. THE MEDICHAM GATE NOW READS THREE TEAM LATTICES AND READS `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`, AND EVERY FIGURE DOWNSTREAM OF THE SIMULATOR IS STILL ABSENT FROM THIS DOCUMENT: RE-RUNNABLE IS NOT TRUE, AND NOTHING WAS RE-RUN.**

| question | artifact | answer |
|---|---|---|
| the gate | `node engine/quarantine.js`, release `4c9b0cc4a4da`, tracked under `data/releases/` | `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld` |
| whole-game / **BOARD-MATERIAL**, the gating clause, on every lattice | `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json` — `--games` `1200`, `1350`, `1950`, middle arm, same pins (ROADMAP #619) | `--games 1200`: 961 − 961 = **0**; `1350`: 1069 − 1069 = **0**; `1950`: 1497 − 1497 = **0**. Turn boundaries 10,705 = 10,705, 11,842 = 11,842, 16,713 = 16,713; `state.games_void_excluded` 0 and `state.games_cut_off_by_the_turn_cap` 0 on all three. Four games stopped when the authority refused a disabled move the driver drew (THREW 1 / 1 / 2 in `end_state[0].summary.verdicts`); their boards agreed up to the stop |
...
| the deliberate roster, items | `data/roster.items.json` | 148 of 148; DIFFER 0, DID-NOT-FIRE 0; could-not-stage in scope 0; 22 of 22 red demonstrations CAUGHT |
| the deliberate roster, abilities | `data/roster.abilities.json` | 194 of 200; DIFFER 0, DID-NOT-FIRE 0; could-not-stage in scope 0; 63 of 63 CAUGHT; out of scope 116 (no legal carrier 114, validator-refused 1, no legal reader 1); CONTROL-NOT-QUIET 0; DEFERRED-BY-OWNER 6. Deferred by the owner and named: Anticipation, Forewarn and Frisk, whose effect is a message (Will, 2026-09-18: *"set frisk aside."*, CHANGELOG 6.49.0), Illusion (ROADMAP #160), Pickup, and Stall (ROADMAP #195) |
| the deliberate roster, moves | `data/roster.moves.json` | 494 of 497; DIFFER 0, DID-NOT-FIRE 0; could-not-stage in scope 0; 37 of 37 CAUGHT; 3 with no legal carrier, out of scope; DEFERRED-BY-OWNER 3 — Axe Kick and Electrify, below the usage shelf of 25, and Copycat |
| mechanics live in the census | `data/mechanics-census.json` | 955 of 955; `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true |
| the staged harness | `data/all-mechanics-fire.json` | 4700 games, 0 threw, `planner.played.games` 3836; *2 diverge, 1 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0* (Supreme Overlord declared; Gastro Acid below the shelf). Axe Kick's fixture parts a board with no protocol line and is not counted; the move is owner-deferred |
| open, known engine defects | `node engine/open_work.js`; the gate's open-defect clause | PASS — no open row names an instrument that is red (194 verdicts read). `node engine/open_work.js` at HEAD `e85e48c8`: 201 open rows, 20 asserting breakage, 18 of them with no deciding instrument; the clause warns that `data/register-reality.json` is older than the register. At the last notes row before this draft, seven were named and not fixed (CHANGELOG 6.52.0) — among them Protean converting on a status move that fails, and Clear Smog never clearing the target's boosts. All seven are closed at 7.0.0 — six fixed (CHANGELOG 6.54.0), and the item lead was false because no legal item raises a stat. Still open, named by no register row, and parting no lattice game: Coaching's `-fail` after `[notarget]` (notes row 6.57.0); Tidy Up's order and missing `-activate`, and `-unboost|0` at −6 (notes row 6.59.0) |
| what the verdict does not cover | printed by `node engine/coverage.js`, never typed | no differential body plays a real spread (both engines get the same invented one), some entities never occur in a real game, the interior of a hit-count range is reached only by draw, one driver plays every game. no body on a real spread (`declared_gaps` in `data/game-differential.json`: 17,440 with the sheet's nature, 96 with Serious); 852 of 887 entities exercised, 68 unmeasurable and 35 measurable and not exercised (`coverage` in `data/game-differential.json`); 0 of 8 ranged mechanics fully staged; 1 driver policy, `empirical-click/v1`, with 960 of 961 games reaching a result |
| what is withheld, and what a lift would mean | `node engine/quarantine.js`; ROADMAP #57 | the gate is open, so every artifact `node engine/quarantine.js` lists as downstream of MEDICHAM is RE-RUNNABLE — no longer withheld, and not true. **No figure downstream of the simulator is quoted here**. **None was re-run for 7.0.0** — Will's instruction is that the downstream set stays stale until MAG and MILTANK are reworked. The documents' own quarantine check keeps charging a downstream figure that does not name the current release even with the gate open (CHANGELOG 6.30.0) |
| leaf calibration — MEASURE's one number | `data/winrate-backtest.json` | **not re-run.** No sample size, no reliability curve, no calibration gap and no verdict — and no direction may be inferred from the absence. It becomes quotable when the gate is open AND `node engine/backtest_winrate.js` has been re-run |
...
arithmetic. That is the test for a major revision rather than a routine one, and it is why the whole
...
STILL THE POINT.** `docs/RUNNING-NOTES.md` rows 6.0.1 through 6.61.0 are folded in here; each figure lands beside the artifact or the CHANGELOG entry that
