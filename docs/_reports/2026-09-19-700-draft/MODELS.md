...
**7.0.0 — THE GATE READS <<GATE: the first line of `node engine/quarantine.js` on the 7.0.0 release>>, AND THIS LEDGER STILL CARRIES NO MODEL FIGURE.** Every model in this file sits downstream of MEDICHAM, so the only quantity it may carry is the state of the engine underneath them. That state is folded in below from the notes rows written since 6.0.0. **No model figure returns in this version**, and none may be inferred from an absence: the artifacts that would carry them were measured through a simulator that has since been rewritten, so they are RE-RUNNABLE rather than true, and they have not been re-run — Will, 2026-08-11: *"Dont re run the artifacts weve bene over this"*. The re-run list is ROADMAP #57.

**THE PINNED POOL, READ ON THREE TEAM LATTICES — REAL LADDER TEAMS, PLAYED AGAINST THE OFFICIAL SIMULATOR.** `--games` decides which teams play, not only how many: `engine/diff_swarm.js` picks them by a stride computed from it, so one run is one sample of the pool. On one release the gate's `--games 1200` run parted 0 boards in 961 games while a `--games 12000` run from the same frozen pool parted 84 in 7178 (`CHANGELOG.md` 6.45.0), and the gate now requires zero on three lattices chosen to share few teams (`CHANGELOG.md` 6.46.0, ROADMAP #619). On release <<GATE: the 7.0.0 release id, tracked>>: <<GATE: for `data/game-differential.json` (`--games 1200`), `data/game-differential.g1350.json` and `data/game-differential.g1950.json`, BOARD-MATERIAL as `state.games` less `state.games_board_never_diverged` (all three must be 0), the undeclared narration count (all three must be 0 once board-material is), `state.games_void_excluded`, `state.games_cut_off_by_the_turn_cap`, `state.turn_boundaries_compared` against `state.turn_boundaries_identical`, and `end_state[0].summary.verdicts`>>. The bound on the claim is `state.not_compared`: <<GATE: its field count on each lattice>>.

**THE FLAGS ARE PART OF THAT SAMPLE AND ARE RECORDED WITH IT.** Each lattice artifact records `games_requested`, arm `middle` (`pins.arms_run`, `pins.primary`), `steering.policy` `empirical-click/v1`, `steering.team_store_pinned_to` `data/team-pool-frozen`, and the census pin `steering.input_digest`, and `LATTICE_SAME` in `engine/quarantine.js` refuses the three as one question unless those agree. <<GATE: `pins.digest`, `steering.input_digest` and each lattice's `steering.team_pool_digest`>>. A run at a different `--games`, a different arm or a different driver is a different question wearing the same artifact name.

**THE DAMAGE DIFFERENTIAL, WITH THE VOLLEY LOOP INSIDE IT.** `data/engine-diff.json`: <<GATE: `compared`, `agreed`, `disagreed` (0), `skipped_multihit` (0), `skipped_ability_multihit` (0), `volley.move_rows`, `volley.bond_rows`, and what stays out — `volley.unstageable` with the move it names, and `volley.no_arrival`>>.

**THE DELIBERATE ROSTER — ONE STAGED MECHANIC AT A TIME.** <<GATE: for `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`: FIRED-AND-BOARDS-MATCH of `scope.in_scope`, `scope.out_of_scope` by code, FIRED-AND-BOARDS-DIFFER (0), DID-NOT-FIRE (0), COULD-NOT-STAGE, CONTROL-NOT-QUIET, DEFERRED-BY-OWNER, and red demonstrations caught>>. The DEFERRED-BY-OWNER abilities are Will's decisions, named: Anticipation, Forewarn and Frisk (*"set frisk aside."*, 2026-09-18, `CHANGELOG.md` 6.49.0). The tail this leaves is printed rather than described.

**THE CENSUS — EVERY MECHANIC WITH A PROBE THAT FIRES.** `data/mechanics-census.json`: <<GATE: `live` of `probed`, `missing`, `unarmed`, `hollow`, `threw` — the last four 0>>.

**THE STAGED HARNESS.** `data/all-mechanics-fire.json`: <<GATE: `games_played`, `games_threw` (0), and `summary.{moves,abilities,items}.diverged` with the mechanics clause's clearing sentence>>.

**THE TWO SPEED-TIE CORNERS ARE PUBLISHED BESIDE THE GATE AND ARE NOT PART OF IT.** <<GATE: either both corners re-measured on the 7.0.0 release, or "neither corner was re-measured; the last reading is on release `b42b81899631`, `CHANGELOG.md` 6.22.0">>.
...
**MECHANICS STATE, 7.0.0 — READ FROM THE ARTIFACTS, WITH THE GATE VERDICT AT <<GATE: the first line of `node engine/quarantine.js` on the 7.0.0 release, identical to the head block's>>.** The current readings are in the 7.0.0 block at the head of this ledger and are not restated here: board-material and narration on the three team lattices of the pinned pool, the damage differential, the three roster stages, the census and the staged harness, all on the 7.0.0 release. What follows is what moved between 6.0.0 and this major, folded in from the notes rows. Everything dated below it stands as dated evidence and is not rewritten in place.
