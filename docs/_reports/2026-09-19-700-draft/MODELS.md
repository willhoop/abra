...
**7.0.0 — THE GATE READS `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`, AND THIS LEDGER STILL CARRIES NO MODEL FIGURE.** Every model in this file sits downstream of MEDICHAM, so the only quantity it may carry is the state of the engine underneath them. That state is folded in below from the notes rows written since 6.0.0, to 6.69.0. **Every gating clause passes, including the blind-spot clauses added on 2026-09-19** (`CHANGELOG.md` 6.64.0, 6.66.1, 6.67.1); that is not a claim that MEDICHAM is correct, and what the gate does not cover is listed below. **No model figure returns in this version**, and none may be inferred from an absence: the artifacts `node engine/quarantine.js` lists as downstream were measured through a simulator that has since been rewritten, so they are RE-RUNNABLE rather than true, and they have not been re-run — Will, 2026-08-11: *"Dont re run the artifacts weve bene over this"*. The re-run list is ROADMAP #57.

**THE PINNED POOL, READ ON THREE TEAM LATTICES — REAL LADDER TEAMS, PLAYED AGAINST THE OFFICIAL SIMULATOR.** `--games` decides which teams play, not only how many: `engine/diff_swarm.js` picks them by a stride computed from it, so one run is one sample of the pool. On one release the gate's `--games 1200` run parted 0 boards in 961 games while a `--games 12000` run from the same frozen pool parted 84 in 7178 (`CHANGELOG.md` 6.45.0), and the gate now requires zero on three lattices chosen to share few teams (`CHANGELOG.md` 6.46.0, ROADMAP #619). On release `6180c4712761` (tracked under `data/releases/6180c4712761`, `CHANGELOG.md` 6.69.0), reading `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json` in that order: BOARD-MATERIAL 961 − 961 = 0, 1069 − 1069 = 0 and 1497 − 1497 = 0; `threw` 0, 0 and 0; undeclared narration 0, 0 and 0 (raw 0, 1 and 2, the difference being the declared Supreme Overlord `fallenundefined` row); `state.games_void_excluded` 0 and `state.games_cut_off_by_the_turn_cap` 0 on all three; turn boundaries 10,716 = 10,716, 11,856 = 11,856 and 16,725 = 16,725; `end_state[0].summary.verdicts` SAME-END-STATE 961, 1069 and 1497 and every other verdict 0. The four games that threw on `4c9b0cc4a4da` now play to the end, because the empirical driver no longer clicks a move the authority refuses (`CHANGELOG.md` 6.63.0). The bound on the claim is `state.not_compared`: 7 fields on each lattice, among them ability trapping, which MEDICHAM evaluates when a switch is attempted rather than storing; Unburden left that list when it became a compared leaf (`CHANGELOG.md` 6.63.0).

**THE FLAGS ARE PART OF THAT SAMPLE AND ARE RECORDED WITH IT.** Each lattice artifact records `games_requested`, arm `middle` (`pins.arms_run`, `pins.primary`), `steering.policy` `empirical-click/v1`, `steering.team_store_pinned_to` `data/team-pool-frozen`, and the census pin `steering.input_digest`, and `LATTICE_SAME` in `engine/quarantine.js` refuses the three as one question unless those agree. `pins.digest` `de38d17e15a2`, `steering.input_digest` `57deebd09273` and `steering.driver_code.digest` `faf70ecbca71` on all three, and pools `0d103fb9fa87`, `7e7a37ded7fc` and `a5ce76242f8d`. The driver digest moved at `CHANGELOG.md` 6.63.0 and 6.65.1, so no earlier lattice is comparable to these. A run at a different `--games`, a different arm or a different driver is a different question wearing the same artifact name.

**THE DAMAGE DIFFERENTIAL, WITH THE VOLLEY LOOP INSIDE IT.** `data/engine-diff.json`: `compared` 6000, `agreed` 6000, `disagreed` 0 (and 0 at each of the 16 other indices in `arms`), `skipped_multihit` 0, `skipped_ability_multihit` 0, `volley.move_rows` 130, `volley.bond_rows` 12; out — `volley.unstageable` one move, Dragon Darts, whose own hit loop in the authority throws in a one-against-one harness, and `volley.no_arrival` 4.

**THE DELIBERATE ROSTER — ONE STAGED MECHANIC AT A TIME.** From `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`: items 148 of 148 in scope, none out of scope; abilities 194 of 200 in scope, 116 out of scope (no legal carrier 114, validator-refused 1, no legal reader 1); moves 494 of 497 in scope, 3 out of scope with no legal carrier. On all three: FIRED-AND-BOARDS-DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0; DEFERRED-BY-OWNER 0, 6 and 3; red demonstrations 22 of 22, 63 of 63 and 37 of 37 CAUGHT, with no dead anchor. The DEFERRED-BY-OWNER rows are Will's decisions, named: the abilities Anticipation, Forewarn and Frisk (*"set frisk aside."*, 2026-09-18, `CHANGELOG.md` 6.49.0), Illusion, Pickup and Stall, and the moves Axe Kick, Electrify and Copycat. A deferral is a decision, not a pass: Frisk and Axe Kick carry open items, below.

**THE CENSUS — EVERY MECHANIC WITH A PROBE THAT FIRES.** `data/mechanics-census.json`: `live` 969 of `probed` 969; `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0.

**THE STAGED HARNESS.** `data/all-mechanics-fire.json`: `games_played` 4638, `games_threw` 0; `summary.moves.diverged` 0, `summary.abilities.diverged` 0, `summary.items.diverged` 0. Proof in scope: abilities FIRED 198 of 200, moves resolved 497 of 497 on both engines, items FIRED 148 of 148; the two unproven ability rows, Frisk (DID-NOT-FIRE) and Illusion (UNPROVEN-UNCONTROLLED), are excused by the owner. Control watch: every FIRED row that rests on a live control is earned by the subject's own receipt. Control arms: 2 of 841 parted, 0 on a board, both announcement-only on rows shelved with Illusion. The clause reads *0 diverge (+1 board-only), 0 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0*; the board-only row is Axe Kick.

**WHAT THE GATE DOES NOT COVER.** Every gating clause passes, including the blind-spot clauses; these limits stand beside that. (1) Illusion is closeted and unmodelled, and its teams are dropped from the pool (ROADMAP #160). (2) **Frisk — open, being fixed:** DID-NOT-FIRE, excused only by the owner's deferral; the authority announces it only for a foe that holds an item, and the fixture's foes hold none. (3) **Axe Kick — open, being fixed:** its staged board parts with no line difference, the target's confusion counter 2 on the authority and 1 here; judged below the reach shelf and owner-deferred, so no clause fails on it. (4) The sample: three lattices of 961, 1,069 and 1,497 games (`state.games` in `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json`) from one frozen open-sheet pool, capped at 50 turns, one driver; a zero bounds the per-game board-material rate at about 3 / 1497 ≈ 0.20% by the rule of three (Hanley and Lippman-Hand, 1983), and generously. (5) Closed-sheet and best-of-one play is not sampled. (6) No body plays a real spread (`node engine/coverage.js`). (7) 76 effects print nothing when they fire (`summary.moves.leaf_effect`, `data/all-mechanics-fire.json`). (8) 24 leaves cannot be compared on the board and are covered only by other instruments — 23 of them, with `volatile:powershift` covered by none. (9) 860 of 901 entities are exercised in a real game (`coverage` in `data/game-differential.json`). (10) The corner arms were not re-measured. (11) The downstream models in this ledger are re-runnable and were not re-run.

**BASIS — CHANGED.** A reader can no longer be told (1) that the whole-game zero is the series 6.0.0 published: 6.0.0 read one team lattice and this version reads three (`CHANGELOG.md` 6.45.0, 6.46.0); (2) that the lab's denominators mean what they meant at 6.0.0: which mechanics exist in this regulation is decided by one module for every instrument, a could-not-stage row counts only when the entity is in scope, and the staged denominator moved with it (`CHANGELOG.md` 6.10.0, 6.14.0, 6.28.0); (3) that a mechanic is staged on a hand-picked body — the harness and the roster play fixtures derived from each mechanic's own requirements, on bodies that learn the moves they click (`CHANGELOG.md` 6.14.0, 6.24.0, 6.50.1); (4) that a roster green means what it meant — its control arm was found measuring itself or the swapper three times and the greens resting on it were refused (`CHANGELOG.md` 6.34.0, 6.36.0, 6.39.0); (5) that a gate PASS means what it meant at 6.61.0 — the gate grew from nine clauses to ten and its mechanics clause now requires proof, an earned credit and a quiet control arm (`CHANGELOG.md` 6.64.0, 6.66.1, 6.67.1). The old and new series therefore cannot be linked, which is what makes this a MAJOR rather than a better number.

**LEAF CALIBRATION — MEASURE'S ONE NUMBER — IS NOT IN THIS VERSION.** `data/winrate-backtest.json` was not re-run, so no Brier figure, no reliability curve and no decisive-pair count is carried here, and none may be inferred. When it is run, what gets published is the reliability curve, the bucket table and the decisive-pair count — never a verdict string — and if the leaf does not beat a coin, that is what this ledger will say.

**THE MAG REFIT STAYS OWED, AND IT IS A REFIT RATHER THAN A RESTAMP.** `data/policy-weights.json` was not touched in this release. The damage table underneath the fitted vector has moved since the vector was fitted, so the feature FUNCTION's input changed; a restamp would answer the fixture gate and write over the evidence for the refit instead of answering it. `engine/feature_fixture.js --check` is the gate that says so.
...
**5.257.0 - NO MODEL CHANGED, NO FITTED VECTOR WAS WRITTEN AND NO QUARANTINED FIGURE BECOMES QUOTABLE. THE ITEM THAT REACHES THIS LEDGER IS THAT A STAND-IN PLAYER IS A MODEL, AND THIS ONE WAS BEING JUDGED ON A CHOICE IT WAS NEVER OFFERED.** Every model here sits downstream of MEDICHAM, so the only thing this ledger records is the distance to the gate. The empirical driver's protect rate was read as a POLICY fault - the model likes the move too much - and it was an ACTION SET fault: `engine/game_differential.js`'s `prefer` axis was a hard narrowing, **22.2% of decisions reached the sampler with exactly one candidate and 60% of those were Protect**, and **where the body kept all four moves the arm already realised 15.3%, the human rate.** The rule for this ledger is the general one: **before blaming a policy's weights, check what was in its candidate set** - a model cannot be scored on options that were deleted before it looked.
...
**THE REFIT THIS LEDGER OWES IS STILL A REFIT AND STILL NOT A RESTAMP.** No fit was started, no policy weight was written and MAG stays paused by its owner's decision. `node engine/status.js` prints REFIT OWED and names the moved inputs, and the simulator moved again this version, so a restamp would write over the evidence for the refit rather than answer it. Nothing in this ledger was re-measured on any release cut in this session, so nothing here is retracted by the engine movement - the exposure is stated so it is not discovered later.
...
**5.253.0 - NO MODEL CHANGED, NO FITTED VECTOR WAS WRITTEN, NO QUARANTINED FIGURE BECOMES QUOTABLE, AND THE CLAUSE THAT DECIDES WHEN THIS LEDGER'S QUARANTINE LIFTS DID NOT MOVE.** Every model here sits downstream of MEDICHAM. No mechanic changed, no differential ran and no whole-game figure was re-derived, so board-material stands where the previous version published it and is not re-derived in this block. Leaf calibration, the leaf-cost figure, the divergence and head-to-head results, the joint layer's numbers and every model report that reads a rollout stay WITHHELD rather than annotated.
...
**THE METHOD RESULT THIS LEDGER SHOULD KEEP, BECAUSE IT IS ABOUT HOW EVERY MODEL HERE WILL BE CERTIFIED.** A cause count and a game count are different quantities. Thirteen causes closed and the game figure improved by seven, because six of those games carry a second divergence that was already counted. A model report that closes N mechanisms and claims N games of improvement is making exactly this error. Say which unit the number is in, and say it in the first sentence.
...
`require`, so it was measured on a simulator the gate did not certify. Whether MEDICHAM passes its gate today
is a gate verdict, and a pass is not a claim that it is correct — `node engine/status.js` names any failing clause — and an open gate makes this
re-runnable, not true. So no sample
...
- **MILTANK untimed vs MILTANK on the clock** is unanswered. A share of move decisions was measured as
  deferred under time pressure; **that share is withdrawn (2026-09-11)** — it is in
  `data/miltank-timing-r6.json`, which stamps the `engine/medicham2-browser.js` of 2026-08-04 and has
  no discoverable writer, so it describes a simulator that no longer exists and cannot be re-run by
  name. "As many rollouts as time allows" is load-bearing either way.
...
horizon. So MILTANK can play to actual wins and losses, and the learned leaf evaluator is load-bearing
only on the 0.2% tail. It is not a blocker for this design. **The evaluator's accuracy and the
count-bodies-and-HP baseline beside it are withdrawn (2026-09-11)**: the pair was scored against the
rollout leaf in `data/leaf-comparison.json`, which declares itself a one-shot with no committed
generator and says in its own words *"do not quote it after the value net or the leaf moves"* — and the
leaf has moved. Not re-run (ROADMAP #57).

---

## DODUO — Doubles Optimiser: Decisions United, One turn (named 2026-07-28)

**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** Every figure this section would carry was produced through `engine/medicham2-browser.js` before the fixes folded in at the head of this ledger, so it describes a simulator that no longer exists. It becomes re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and it has NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. The joint layer's vector is `data/policy-weights-joint.json` and its generator is `node --max-old-space-size=4096 engine/fit_joint.js`, which is sequenced after the MAG refit. Nothing is restored here, and no direction may be read out of an absence.
...
**I retired this earlier and was wrong.** The retirement rested on the double-target rate, MAG's
against the humans'. **Both rates are withdrawn (2026-09-11)** — MAG's is a self-play output on
superseded engine bytes and neither is in any artifact. The argument does not need them: that metric
touches **2 of the 18 features**, and judging a team-coordination model by a targeting statistic is
judging a ninth of it.
...
section and naming them mid-section put the retired double-target figure, which no artifact
...
those eighteen to zero and the model scores as the same player. **The two held-out figures behind that
sentence are withdrawn (2026-09-11)**: they are outputs of the joint fit, which `node engine/quarantine.js`
holds downstream of MEDICHAM, and they have not been re-run (ROADMAP #57). MAG is
...
denominator contains actions that were never available. Both DODUO arms inherit that error today.
**The share of items affected is withdrawn (2026-09-11)**: it is a count over the MAG fit corpus, no
artifact holds it, and the fit has not been re-run (ROADMAP #57).
...
actions** per side, which is arithmetic over the action space and stands. **How many of the 72 carried
a non-zero joint vector is withdrawn (2026-09-11)** — a feature-firing count taken through
`engine/board.js` on superseded engine bytes, held by no artifact. The structural point survives: a
pair whose coordination terms are all zero scores exactly as the sum of two singles already computed. With two Pokémon the coordination graph is a single edge, so
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** MACHAMP hill-climbs MAG's weights by playing games through `engine/medicham2-browser.js`, so every figure it would carry describes a simulator that no longer exists. It becomes re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and it has NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. Its exploitability artifact is `data/exploitability-machamp.json`, and the climb starts from a MAG vector that is itself owed a refit. Nothing is restored here, and no direction may be read out of an absence.
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** Every exploitability figure is produced by training a best response through `engine/medicham2-browser.js` against a frozen MAG vector, so it describes a simulator that no longer exists. It becomes re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and it has NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. The artifacts are `data/exploitability.json` and `data/exploitability-mag.json` (`engine/exploit.js`), and the step-rule probe's figures are withheld on the same grounds. **This is the project's headline metric and it has no current value; that absence is the state, not a caveat on a number.**
...
**MECHANICS STATE, 7.0.0 — READ FROM THE ARTIFACTS, WITH THE GATE VERDICT AT `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`.** The current readings are in the 7.0.0 block at the head of this ledger and are not restated here: board-material and narration on the three team lattices of the pinned pool, the damage differential, the three roster stages, the census and the staged harness, all on the 7.0.0 release. What follows is what moved between 6.0.0 and this major, folded in from the notes rows. Everything dated below it stands as dated evidence and is not rewritten in place.

**THE ENGINE FIXES, EACH SHOWN RED AGAINST THE OFFICIAL SIMULATOR BEFORE IT WAS MADE GREEN, AND EACH BEHIND ITS OWN `MEDI_*` SWITCH.** Disguise's crit refusal, three accuracy roads and a Flash Fire absorb (CHANGELOG 6.5.0); Encore ending in the residual when the encored move reaches 0 PP (CHANGELOG 6.6.0); the speed-tie corner mechanisms, fifteen of them, from Encore's same-turn `changeAction` to Reflect Type (CHANGELOG 6.7.0); Simple conferred by Simple Beam, Magnet Rise's clock, a second Psychic Noise, Parental Bond's per-hit secondaries and the three-turn rampage (CHANGELOG 6.11.0); five moves that fail on an ability flag now asking the flag (CHANGELOG 6.12.0); a per-battle faint epoch (CHANGELOG 6.14.1); the eight partings the staging planner caught — Oblivious against Taunt, Magic Bounce against Spite, the crit-stage volatiles, a mega stone under Klutz, and the Cute Charm, Own Tempo, veil and Covet lines (CHANGELOG 6.18.0); the survival clamp answering each hit of a volley instead of its total (CHANGELOG 6.22.0); a sleep the residual walk sets being offered to its `onUpdate` cure before the turn ends (CHANGELOG 6.24.0); and a perish-zeroed body running its later residual handlers, with a corpse no longer counted as a residual follower (CHANGELOG 6.26.0).

**AND THE ENGINE FIXES OF THE SECOND HALF OF THE SERIES, AIMED BY THE LAB AND THEN BY THE THREE LATTICES.** The first fixture that ever put an Early Bird body to sleep found the sleep counter counting turns where the authority spends two ticks (CHANGELOG 6.41.0). Heal Bell reached no action at all and now cures the party, Corrosive Gas no longer interleaves its refusals with its effect, and Reflect Type writes its real-type line at the turn boundary (CHANGELOG 6.44.0). An Electric charge is spent after an aborted or a status click, and a mega evolution ends the ability it replaces so a Flash Fire boost does not survive it (CHANGELOG 6.45.0). Beat Up prices each hit off the ally's set species (CHANGELOG 6.47.0). A Future Sight payout meets a Substitute, Harvest restores only a berry, Prankster's refusal reads its target, and accuracy and protection read their tags rather than a table of names (CHANGELOG 6.48.0). Mold Breaker pierces Good as Gold, Magic Bounce and Sticky Hold (CHANGELOG 6.49.0). Gooey's drop travels the boost road, Mummy and Wandering Spirit honour the flags that refuse them, Charge is spent on an early exit, a switch-out clears the Ally Switch counter, Transform runs the copied ability's start, and Parental Bond does not open its second hit on a fainted target (CHANGELOG 6.50.0). Fifteen single-game leads across move effects, turn order, game-end residuals and die addresses close at CHANGELOG 6.52.0. The last named leads close at CHANGELOG 6.54.0: Protean honours a refused Try, Clear Smog clears, a Mold Breaker move breaks Contrary on the boost road, Cud Chew's pending berry clears on switch-out, Copycat refuses the `failcopycat` moves, and on-hit chance abilities roll once per landed hit with Triple Axel's accuracy drawn per hit. Sucker Punch reads the queued move that Encore rewrites, Focus Sash and Sturdy answer a Future Sight payout and a confusion self-hit, thirteen narration mechanisms print as the authority prints, and four turn-order mechanisms that looked like coin flips were order defects (CHANGELOG 6.57.0). Forewarn announces with its die, Harvest throws its coin every turn, and six more narration lines match (CHANGELOG 6.59.0). The three lattices read board-material 0 / 0 / 0 for the first time on `1a6550ea5ec6` (6.58.0), undeclared narration 0 / 0 / 0 on `4c9b0cc4a4da` (6.60.0), and the gate opened on that release under nine clauses (6.61.0). The blind-spot clauses then made it stricter (6.64.0, 6.66.1, 6.67.1), and the fixes they aimed followed: Unburden as a compared leaf, Gastro Acid, Knock Off and Poltergeist under Magic Room and Klutz, Ripen, Anticipation, Screen Cleaner, three switch-in announcements, and a driver that clicked moves the authority refuses (6.63.0); Anger Point before the hit's secondary (6.66.0); Sticky Hold's refusal line and a fainting holder's item (6.68.0). The gate read CLOSED, 2 of 10, on `d92bdfb50d88` (6.65.0) and OPEN, 10 of 10, on `6180c4712761` (6.69.0). **Every one sits behind its own `MEDI_*` switch and was measured on all three lattices, not on the one that had read zero.** Known and not fixed at CHANGELOG 6.52.0, and owed to ENGINE: Cursed Body's roll count, Triple Axel's per-hit accuracy, Clear Smog, stat-raising items under Contrary, Mold Breaker against Contrary on the boost road, Cud Chew after a switch, and Protean on a failed status move. All seven close by 7.0.0: six are fixed at CHANGELOG 6.54.0, and the stat-raising-item lead was false, because no legal item raises a stat. Found after them: Tidy Up's order and a stat refused at −6 that should print `-unboost|0` (notes row 6.59.0), both fixed at CHANGELOG 6.63.0; and Coaching's `-fail` after `[notarget]` (notes row 6.57.0), still open, named by no register row and parting no lattice game.

**THE LAB'S CONTROL WAS MADE HONEST BEFORE ITS COUNT WAS TRUSTED.** A roster row is a one-variable experiment only if its control arm differs from the subject in the one ability. The control was found measuring the swapper (a lent crit blocker; CHANGELOG 6.34.0), measuring itself (the Skill Swap bookkeeping, which made 39 ability greens vacuous; CHANGELOG 6.36.0), and restoring the subject's own ability through a null control (CHANGELOG 6.39.0). The greens that rested on those were refused rather than kept, CONTROL-NOT-QUIET then reached zero (CHANGELOG 6.42.0), and the roster's idle click became a move each fixture body can learn (CHANGELOG 6.50.1). **For this ledger that matters directly: every model here will one day be re-measured on an engine whose lab reads green, and a green that measured the control would have certified the wrong engine.**

**THE LAST PROTOCOL DIVERGENCE ON ONE LATTICE OF THE PINNED POOL CLOSED AT 6.26.0 — AND THE OTHER TWO LATTICES LATER SHOWED THAT ONE LATTICE WAS NOT THE POOL (CHANGELOG 6.46.0).** CHANGELOG 6.26.0 records it as that lattice's one remaining protocol divergence: **1 of 961** games became **0 of 961** on the same sample when `residualFollowerRuns` stopped counting a body whose `|faint|` was already written. The narration clause had been 0, then 1, then 0 of 961 across the three releases of CHANGELOG 6.18.0, which is the shape to expect: claiming a line the authority writes exposes the next thing that was refused silently.

**THE INSTRUMENTS CHANGED MORE THAN THE ENGINE DID, AND THAT IS THE HONEST SUMMARY OF THIS MAJOR.** The damage differential now enters the authority's own hit loop, so multi-hit moves and Parental Bond clicks are compared rather than skipped by construction (CHANGELOG 6.1.0). A published figure may no longer cite an engine release whose bytes are not in this repository (CHANGELOG 6.2.0). The staged harness stages the 75 mega stones as a mega evolution and gives the abilities with no A/B control a board-only arm (CHANGELOG 6.8.0). A staging planner derives a legal fixture, a one-variable control and a failure plant for each mechanic — **841** of the **847** with a legal carrier (CHANGELOG 6.9.1) — and the harness plays those fixtures, which took staged mechanics that fired from **740 to 783 of 845** (CHANGELOG 6.14.0). Scope is decided by one module for every instrument (CHANGELOG 6.10.0, 6.14.0). Gender reaches a real game, so Attract, Cute Charm and Rivalry are exercised for the first time (CHANGELOG 6.14.0). The roster's fixture bodies learn the moves they use: refused pairs **420 → 43** (CHANGELOG 6.22.0) and then **0** on release `9ec2ab9ad0ef`, against **420** under `ROSTER_LEARNSET_UNREPAIRED=1` (CHANGELOG 6.24.0). Twenty owed instruments were registered against open register rows that had asserted a defect with nothing deciding it (CHANGELOG 6.13.0).

**THE PROTOCOL TRACE NOW CLAIMS 46 EVENTS.** `data/protocol-events.json` reads `showdownEvents` **91**, `emittedCount` **46**, `notEmittedCount` **48**, `partialCount` **10**; the engine claims the `-block` line since CHANGELOG 6.18.0 and Gastro Acid's `-endability` since CHANGELOG 6.63.0.

**WHAT A ROLLOUT COSTS, MEASURED FOR THE FIRST TIME — AND IT IS PLAYING TURNS, NOT COPYING POSITIONS.** A position copy costs about a third of one turn in MEDICHAM and slightly under half in Showdown, roughly **6%** of a full rollout in both (CHANGELOG 6.9.2); MEDICHAM's rebuild-from-board copy is independent, Showdown's in-memory JSON copy shares the original's log and is not. Where the time actually goes was profiled rather than guessed (CHANGELOG 6.12.2): `data/verification/opt-plan-2026-09-11/profile-summary.json` `phases.A1`, on release `13257c8bc397`, reports `self_top[0]` as `battleTurn`, with tag-id normalisation (`norm @ engine/tags.js`) next. Its shares are not restated here, because no living-document citation can bind a figure to a file under `data/verification/`; the run was contended, so its absolute times are not figures of record either. Skipping narration would save nothing, because rollouts already run with the trace off.

**TWO LIVE POSITIONS STEPPED ALTERNATELY STAY INDEPENDENT, WHICH IS A PRECONDITION FOR ANY TREE SEARCH OVER THIS ENGINE.** The interleave bench found one module variable that leaked between builds: a double wipe's result read as a draw after any later battle was built, on **43 of 43** constructed double wipes, **0 of 43** after the per-battle faint epoch, and **43 of 43** again with the old rule switched back on (CHANGELOG 6.14.1; artifact `data/verification/interleave-2026-09-11/interleave-aefcb93baf14.json`, release `aefcb93baf14`). No caller today was affected; a search that keeps positions alive would have been.
...
**MECHANICS STATE, 6.0.0, READ FROM THE ARTIFACTS RATHER THAN TYPED.** `data/mechanics-census.json`: `live` **835**, `probed` **835**, `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true. `data/engine-diff.json`: `compared` **6,000**, `agreed` **6,000**, `disagreed` 0, with the two multi-hit skip counts this paragraph carried RETRACTED at 6.1.0 — the volley loop was wired into the damage differential and both counters now read 0; the superseded figures are recorded in `docs/RUNNING-NOTES.md` and CHANGELOG 6.1.0, and the 6.1.0 paragraph above carries the replacement. `data/roster.items.json` **142 tested** of 148 in scope; `data/roster.abilities.json` **139 tested** of 201 in scope, with **115** of 316 abilities carrying no legal carrier in this regulation; `data/roster.moves.json` **492 tested** of 498 in scope — `differ` 0 and DID-NOT-FIRE 0 on all three. `data/all-mechanics-fire.json`: `games_played` **1,313**, `games_threw` 0. `data/game-differential.json`: board-material **0 of 961**, narration **0 undeclared of 961**, **10,705** of **10,705** boundaries identical, on release `cbd510bc2b13` against authority commit `20ad99ffc9a5a4a4e8fb56ab04ad8e4255b3f2b4`. The bound: 54 of 80 leaves compared, 8 fields declared uncompared, one driver, real teams, synthetic spreads, and two corner arms that part **16 of 961** and **15 of 961**. Everything dated below this paragraph is history.
...
`data/protocol-events.json`: **showdownEvents 91, emittedCount 46, notEmittedCount 48, partialCount
...
mechanics work TOGETHER. Since 3.43.0 the generator **asserts its own arithmetic** —
`theoretical = staged + dropped`, per axis and per `(key, reactor)` on the flag axis, throwing rather
than printing. That assertion is what moved both the denominator and the emitted count off their
3.42.0 values; the superseded figures are in `CHANGELOG.md` 3.43.0 rather than here, because a prior
number quoted beside a live artifact reads as a claim about that artifact.

Read from `data/interaction-matrix.json`: theoretical **7103**, emitted **2250**,
`staged_pct_of_theoretical` **31.7**, `live` **1642**, `agree` **1642**, `agreement_pct` **100**,
`part` **0**, `off_gate` **12**. The multi-turn field axis — every ordered pair of persistent field
effects, run eight turns to expiry — is staged and theoretical **156** on both counts.

**CORRECTED 2026-08-22, AND THE PRIOR PARAGRAPH IS THE POINT RATHER THAN THE FIGURES.** This block
read *"a theoretical cross product of 8,795 … 2,300 emitted … 26.2% … 1,634 LIVE … 98.8% (1,614/1,634)"*
while naming the artifact on the same line — every one of those six figures had moved. It also carried
the heading **"THE AGREEMENT FIGURE HAS FALLEN TWICE AND THE SIMULATOR DID NOT CHANGE EITHER TIME"**
over the series 100.0% → 99.6% → 98.8%, and that series has since gone back up. The reasons the block
gave for the two falls remain true and are worth keeping: 3.43.0 found that 5,090 pairs were being
dropped without ever reaching the ledger; 3.45.0 found that the 902 pairs dropped for "having a
...
had been MISSING in the reference engine while medicham2 hit it. Both movements were the denominator
becoming honest, in both directions. Read the coverage fraction beside the agreement, always: a
percentage over a denominator nobody checked is a statement about where nobody looked — which is
exactly why the staged fraction is quoted above and not only the agreement. Ten engine bugs were
found by the field axis in one pass (WIRE 72–81); none was reachable from a single-mechanic probe.
...
**Win% backtest — EVERY FIGURE WITHHELD. QUARANTINED, 2026-08-22.** This block stated a Brier
difference, a reliability curve, a decisive-call rate, a noise-floor margin and an extreme-bucket
share, all read from `data/winrate-backtest.json`. That artifact is downstream of MEDICHAM — its
generator `engine/backtest_winrate.js` reaches `engine/medicham2-browser.js` through `require` — and
`node engine/status.js` reports it as **QUARANTINED: the figure is withheld, not annotated.** CLAUDE.md
is explicit that a caption is not a quarantine and that printing the figure with a caveat is the bug,
so the numbers are cut rather than captioned. They are not retracted as false; they are unquotable
until the MEDICHAM gate opens **and** `node engine/backtest_winrate.js` is re-run. Leaf calibration is
MEASURE's one number and this is the standing open item.

What survives the withholding, because it does not rest on any of those values: the 2026-07-23 and
2026-08-02 readings below both scored `winProb2`, which **no live decision calls**, so neither should
be quoted for a different reason. MILTANK's team-preview leaf is a greedy playout at
`maxTurns=60 / seeded:true`; its in-game leaf is `rollout_leaf.rolloutWinProb` at
`explore=1.0 / foePolicy=uniform / maxTurns=60`. Those are configuration facts read from the code, not
measurements. Harness: `engine/backtest_winrate.js`; report `data/winrate-backtest.json` (which stamps
...
**Honest status:** big improvement over the old 1v1 chain (which gave 0%/100%); results carry a 95% CI on the site. **The mirror-match value, the rollout count-and-latency pair and the spread description are withdrawn (2026-09-11)** — all three are outputs of the rollout on engine bytes that have since been rewritten, no artifact records them, and they have not been re-run (ROADMAP #57). **The damage math is now VALIDATED** against the Smogon damage calculator (MIT ground truth). Read from `data/damage-validation.json`: **36 scenarios compared, within 5% on 100% of them, worst 0%**. See `engine/validate_damage.js`. The remaining caveat is the *policy* (behaviour-cloned; over-credits speed control), not the damage numbers.

**Corrected 2026-08-22.** This line read *"matches the calc to the integer on 18/22 meta scenarios … within 5% on 100% of scenarios, median error 0% (worst 3% = 16-roll rounding)"*. None of those four figures is in the shipped artifact: the 18/22 stage predates the Ruin/Solar Power/Guts additions, and the current run reports worst 0% with no median and no 22-scenario stage. Superseded by the artifact, not withdrawn as wrong when written.
...
invariants — mirror symmetry, range, antisymmetry; **the mirror value that stood here is withdrawn
2026-09-11**, a rollout output on superseded bytes that no artifact holds).
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** DITTO's hill-climb optimises the grounded MEDICHAM value, as its own method line below says, so every matchup bar and every swap verdict it would carry rests on a simulator that no longer exists. It becomes re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and it has NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. Nothing is restored here, and no direction may be read out of an absence.
...
engine; verified running (`medichamRank` returns a value rather than null — **the value itself is
withdrawn 2026-09-11**, a rollout output on superseded bytes recorded in no artifact).
...
**7.0.0 — THE WIN-% CHIP IT RENDERS IS MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** KADABRA itself parses a replay and needs no simulator, but the value chip it displays comes from the PORYGON2 family, which does, so that chip's numbers describe a simulator that no longer exists. They become re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and have NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. The coaching walkthrough, which reads the log rather than a model, is unaffected.
...
**7.0.0 — EVERY FIGURE THAT READS A ROLLOUT IS MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** SLOWKING's leaf and its continual re-solve play positions forward through `engine/medicham2-browser.js`, so those figures describe a simulator that no longer exists. They become re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and have NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. The preview-Nash half that solves GURU's archetype matrix reads real game outcomes rather than a rollout and is not downstream of the simulator. Nothing is restored here, and no direction may be read out of an absence.
...
**The question it answers:** given a LIVE POSITION — both actives, HP, field, and everything revealed
so far — **which action, right now, maximises win probability, and by how much?** It is specified to
answer as a MIXED strategy over the legal actions rather than a single click, because a Pokemon turn
is simultaneous-move, and to carry the expected win-% delta of each option and a plain-English reason
beside it (`docs/ALAKAZAM-v2-spec.md`, "The capstone — ALAKAZAM (built last)"; `docs/ABRA-whitepaper.md`
§7). It is the assembly of parts this ledger already documents separately — a belief over the
opponent's hidden sets (XATU), depth-limited search over the validated damage engine with each
simultaneous turn solved as a matrix game (MILTANK, with GARY as the opponent inside it), a learned
value at the leaves (the value nets), human-anchoring to the behaviour clone (MOVE PRIORS), and exact
answers at the small end (DUSK).

**NO FIGURE IS QUOTED HERE AND NONE MAY BE.** Nothing has been fitted, run or measured under this
name. This heading states the QUESTION and stops there; a result in this entry would be an invented
number in the one file whose job is to hold none.

**What it is judged on, stated in advance:** decision quality against held-out human play, and
self-play / ladder win-rate with intervals — **never** on predicting the winner from team preview,
which this project measures as close to a coin. Its search layer is kept only if it beats the
no-search policy; the published literature gives real reason to think it may not, and that is a
result about the game rather than a defeat.

**Gating, and why nothing here is startable.** It is phase 3 of the four-phase sequence in
`docs/ABRA-whitepaper.md` §7: reached only through phase 1 (MEDICHAM correct — a search needs an
engine that is fast AND correct) and phase 2 (does compute buy anything at all). MEDICHAM's gate is
open, so every part above is either quarantined or unbuilt.

**Why this heading exists at all.** `tests/test-model-map.js` compares this ledger against the
project's own model map and reported the gap for weeks: `web/models.html` carries an ALAKAZAM box
while this file documented only its PARTS, which is the shape CLAUDE.md names — two files carrying
...
> not tell these value nets apart. And the destination is unchanged: the paired accuracy above is still well under the
> in-sample ceiling for this feature class (whose value no artifact records, so it is not restated), and under the live leaf — **whose accuracy is
> withdrawn 2026-09-11**, being a rollout figure measured on engine bytes that have since been
> rewritten, recorded in no artifact and not re-run (ROADMAP #57). A correctness fix worth making on
> its own terms; not a capability change.
>
...
**DONE (2026-07-23): the engine's damage math is validated** against the Smogon damage calculator (`engine/validate_damage.js`, `data/damage-validation.json`). MEDICHAM/DITTO no longer rest on unverified numbers. The scenario count is read from the artifact and is not repeated here — it has moved once already (this line said 31 until 2026-08-22, against a file that has read 36 since 2026-08-08).
...
**7.0.0 — THE HARNESS PLAYS THE OFFICIAL ENGINE; ANY RUN SCORED THROUGH A POLICY IS DOWNSTREAM, RE-RUNNABLE AND NOT RE-RUN.** MEW's battles are played by the official Showdown Champions engine, so the harness itself is not a MEDICHAM consumer; but `mew.js --policy score` prices clicks with MAG, and `data/quarantine-stamp.json` (generated `2026-09-10T23:44:15Z`) lists `mew.js` among the artifacts classified downstream of the simulator. Any policy-scored MEW figure therefore describes a simulator and a weight vector that have both moved, becomes re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and has NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57.
...
**Honest status:** built and gated. `engine/validate_selfplay.js` runs 13 checks — store shape (S7), set realism, determinism, and a mirror-symmetry check that plays identical teams against each other and asks whether either seat wins more often. **ITS RATE, ITS INTERVAL, ITS BATTLE COUNT AND THE BATCH SIZE BESIDE THEM ARE WITHDRAWN (2026-09-11).** What was measured: p1's win share over a few hundred identical-team battles, with a 95% interval, as a test for side bias in the harness. `engine/validate_selfplay.js` **writes no artifact** — it prints — so no file on disk records the run, the engine bytes it ran on cannot be identified, and `data/mew.js` is on the list `node engine/quarantine.js` prints as downstream of MEDICHAM. Not re-run (ROADMAP #57); no direction may be read out of the absence.
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN, AND BEING REWORKED.** `data/policy-weights.json` was not touched in this release. Every corpus count, weight, standard error and held-out figure this section would carry was fitted on features computed through `engine/medicham2-browser.js` before the fixes folded in at the head of this ledger, so each describes a simulator that no longer exists. They become re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and have NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*, because he is reworking MAG and does not want the work done twice; the re-run list is ROADMAP #57. **What is owed is a REFIT, not a restamp** — the damage table underneath the fitted vector has moved, so the feature function's input changed, and a restamp would write over the evidence for the refit (`engine/feature_fixture.js --check`). The sequence, only when he asks: `node --max-old-space-size=4096 engine/fit_policy.js`, then `engine/fit_joint.js`.
**Job:** decide a move by looking at the other side of the field, instead of by how popular the move is.
**Why:** the behaviour clone answers only *what does this species usually click?* Two gaps followed and no prior-tuning could close them — it clicked far fewer super-effective moves than real players and far more moves that outright failed. **The four rates behind that sentence are withdrawn (2026-09-11)**: they were measured in games played through `engine/medicham2-browser.js` on bytes that have since been rewritten, no artifact records them, and they have not been re-run (ROADMAP #57). It also made every `build_lab` number a measurement of what beats **bad** play.
...
and reach `engine/medicham2-browser.js` through `require`. Whether MEDICHAM passes its gate is a
gate verdict, and a pass is not a claim that it is correct, — `node engine/status.js` names any failing clause — and an open gate makes these re-runnable, not true. So no turn count, no coverage counter, no held-out
...
`require`. Whether MEDICHAM passes its gate is a gate verdict, and a pass is not a claim that it is correct — `node engine/status.js` names any failing clause — and an open gate makes this re-runnable, not true. No action count,
...
ability never reached the estimate at all: across a large matchup sweep, declaring `intimidate`,
`drizzle` or `drought` moved the vector in **none** of them while a `levitate` control did move it —
so MAG weighed bringing Incineroar in against the foe's full Attack. All three now fire. **The sweep
size, the control's count and the three post-fix counts are withdrawn (2026-09-11)**: they are
`engine/board.js` feature-firing counts on engine bytes that have since been rewritten, held by no
artifact, and not re-run (ROADMAP #57). The zero-against-a-live-control shape is the finding and it
does not need the counts. Modelling the drop
...
**It samples, it does not take the best move** — a greedy bot overshoots the human super-effective rate and is *less* human. **The rate it overshoots is withdrawn (2026-09-11)**: a self-play output on superseded engine bytes, recorded in no artifact. Same argument as DEFENSE §2.
...
**THE THREE FITTED WEIGHTS AND THEIR INTERVALS ARE WITHDRAWN (2026-09-11).** What was measured: each
was estimated by `engine/fit_policy.js` as a conditional-logit coefficient with a standard error, on
features computed through `engine/medicham2-browser.js`. `data/policy-weights.json` carries **no engine
release id at all** (it stamps `fitEnvironment` and `featureHashes` and nothing that names a release),
the simulator has been rewritten many times since, and
`node engine/quarantine.js` lists the artifact among those downstream of MEDICHAM. The fit has **not**
been re-run (Will, 2026-08-11: *"Dont re run the artifacts weve bene over this"*; ROADMAP #57), so no
value, interval or ranking is carried here and no direction may be read out of the absence. The values
printed here until 2026-09-11 did not match the artifact on disk either.
...
in the conditional logit. Live play was never affected (the request marks the rest `disabled`).
**THE SHARE OF ITEMS AFFECTED, THE SWITCH-FEATURE COUNT AND THE SWITCH-EVENT COUNTS ARE WITHDRAWN
(2026-09-11).** What was measured: the share of fitted items carrying the wrong denominator, how many
of the eight switch features cleared zero after the refit, and the number of switch events in greedy
self-play before and against after it. All three are outputs of `engine/fit_policy.js` and of games
played through `engine/medicham2-browser.js`; no artifact on disk holds any of them, and the simulator
has been rewritten since. Not re-run (ROADMAP #57).

**THE OPPONENT MODEL — job 2 of ALAKAZAM, off by default (3.29.0).** `incomingThreat` took a MAX, the
foe's hardest available hit, and nine features are built on it. MAG assumed the foe's lead always
clicks a damaging move AND that it was the nastiest one available. It is now an expectation weighted
by P(their action), from the same weights — `candidates` and `featuresFor` already take `side`.
**EVERY FIGURE THIS PARAGRAPH CARRIED IS WITHDRAWN (2026-09-11).** What was measured: how often a
foe's lead really clicks a damaging move, and how far `protectThreatened` and `diesBeforeMoving` fell
across a sample of boards once the expectation replaced the maximum. Both are board-feature readings
taken through `engine/board.js` over `engine/medicham2-browser.js`; `data/opponent-calibration.json`
holds none of them, no other artifact does, and that artifact is itself on the downstream list. Not
re-run (ROADMAP #57). Needs a refit before shipping, since nine features now mean something different.
...
base so the cosmetic fallback correctly refuses to substitute. `board.dmgMon.unknownSpecies` fell once the rows existed; **its before and after shares of candidate
scorings are withdrawn (2026-09-11)**, board-counter readings on superseded engine bytes that no
artifact holds. What survives is the mechanism: the counter had read clean for weeks only because its
workload was 120 replayed games, in which an in-battle forme change never reaches a scored position.
**Covariate shift, corrected automatically:** open-sheet TEAMS differ from closed-sheet teams in species composition (`engine/corpus_shift.js`), while measured behaviour given a board differs far less. Every refit re-estimates on a sample reweighted to the closed-sheet species mix and reports the shift **in standard errors**, and the **reweighted vector ships** (`shipped: "reweighted_to_closed"`), since MEW draws its teams from the ladder store. **THE SIZE OF THE TEAM GAP, THE SIZE OF THE BEHAVIOUR GAP, THE COUNT OF WEIGHTS THAT MOVE MATERIALLY AND THE PER-WEIGHT SE MOVEMENTS ARE WITHDRAWN (2026-09-11).** What was measured: total absolute species difference between the two corpora, the largest behavioural gap given a board, and how far each fitted weight moved under the reweighting, in standard errors. `engine/corpus_shift.js` reaches `engine/board.js` and the numbers live inside `data/policy-weights.json`, which carries no release id and is downstream of a simulator that has since been rewritten; the per-weight SE movements are in no artifact at all. Not re-run (ROADMAP #57). Which weights are board-reading and which are prior-reading is a property of the feature set and stands.
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN, AND BEING REWORKED.** MILTANK decides by playing the position forward through `engine/medicham2-browser.js` with MAG's weights at the leaf, so every figure below that reads a MILTANK run describes a simulator and a vector that have both moved. `data/search-decision-profile.json` (`engine/miltank.js`) was not re-run. It becomes re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and MILTANK is paused beside the MAG rework, so it is sequenced after the refit: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. Nothing is restored here, and no direction may be read out of an absence.
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** GARY plays the foe inside MILTANK's imagined games, so every calibration and recall figure it would carry was produced through `engine/medicham2-browser.js` and describes a simulator that no longer exists; `data/opponent-calibration.json` and `data/opponent-recall.json` were not re-run. They become re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and have NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. Nothing is restored here, and no direction may be read out of an absence.
...
**7.0.0 — MEASURED UNDER AN ENGINE THAT HAS SINCE CHANGED; RE-RUNNABLE, NOT RE-RUN.** PORYGON2 and PORY-NN are trained and scored on positions dumped from games MEDICHAM played, so every accuracy, separation and calibration figure they would carry describes a simulator that no longer exists. They become re-runnable when the gate reads OPEN — the verdict is at the head of this ledger — and have NOT been re-run: Will, 2026-08-11, *"Dont re run the artifacts weve bene over this"*; the re-run list is ROADMAP #57. The generator is `engine/porygon2.py`, and the value chip KADABRA renders comes from here. Nothing is restored here, and no direction may be read out of an absence.
...
- **A measurement reads a frozen release, not the live tree.** `engine/engine_release.js`. A number
  produced without a release stamp cannot be reproduced, because the tree it was scored against is
  not recoverable. The current release id and the frozen file set are printed by
  `node engine/engine_release.js list`; this bullet named a specific id "over 12 files" until
