...
**7.0.0 — THE GATE READS `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`, AND THIS LEDGER STILL CARRIES NO MODEL FIGURE.** Every model in this file sits downstream of MEDICHAM, so the only quantity it may carry is the state of the engine underneath them. That state is folded in below from the notes rows written since 6.0.0. **No model figure returns in this version**, and none may be inferred from an absence: the artifacts that would carry them were measured through a simulator that has since been rewritten, so they are RE-RUNNABLE rather than true, and they have not been re-run — Will, 2026-08-11: *"Dont re run the artifacts weve bene over this"*. The re-run list is ROADMAP #57.

**THE PINNED POOL, READ ON THREE TEAM LATTICES — REAL LADDER TEAMS, PLAYED AGAINST THE OFFICIAL SIMULATOR.** `--games` decides which teams play, not only how many: `engine/diff_swarm.js` picks them by a stride computed from it, so one run is one sample of the pool. On one release the gate's `--games 1200` run parted 0 boards in 961 games while a `--games 12000` run from the same frozen pool parted 84 in 7178 (`CHANGELOG.md` 6.45.0), and the gate now requires zero on three lattices chosen to share few teams (`CHANGELOG.md` 6.46.0, ROADMAP #619). On release `4c9b0cc4a4da` (tracked under `data/releases/4c9b0cc4a4da`), reading `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json` in that order: BOARD-MATERIAL 961 − 961 = 0, 1069 − 1069 = 0 and 1497 − 1497 = 0; undeclared narration 0, 0 and 0 (raw 0, 1 and 2, the difference being the declared Supreme Overlord `fallenundefined` row); `state.games_void_excluded` 0 and `state.games_cut_off_by_the_turn_cap` 0 on all three; turn boundaries 10,705 = 10,705, 11,842 = 11,842 and 16,713 = 16,713; `end_state[0].summary.verdicts` SAME-END-STATE 960, 1068 and 1495 with THREW 1, 1 and 2 and every other verdict 0 — the four THREW games stopped when the authority refused a disabled move the empirical driver drew, and their boards agreed up to the stop. The bound on the claim is `state.not_compared`: 8 fields on each lattice, among them ability trapping, which MEDICHAM evaluates when a switch is attempted rather than storing.

**THE FLAGS ARE PART OF THAT SAMPLE AND ARE RECORDED WITH IT.** Each lattice artifact records `games_requested`, arm `middle` (`pins.arms_run`, `pins.primary`), `steering.policy` `empirical-click/v1`, `steering.team_store_pinned_to` `data/team-pool-frozen`, and the census pin `steering.input_digest`, and `LATTICE_SAME` in `engine/quarantine.js` refuses the three as one question unless those agree. `pins.digest` `de38d17e15a2`, `steering.input_digest` `1298f25115e3` on all three, and pools `0d103fb9fa87`, `7e7a37ded7fc` and `a5ce76242f8d`. A run at a different `--games`, a different arm or a different driver is a different question wearing the same artifact name.
...
`require`, so it was measured on a simulator the gate did not certify. Whether MEDICHAM is correct today
is a gate verdict — `node engine/status.js` names any failing clause — and an open gate makes this
...
**THE PROTOCOL TRACE NOW CLAIMS 45 EVENTS.** `data/protocol-events.json` reads `showdownEvents` **91**, `emittedCount` **45**, `notEmittedCount` **49**, `partialCount` **10**; the engine claims the `-block` line since CHANGELOG 6.18.0, which is the one that moved.
...
and reach `engine/medicham2-browser.js` through `require`. Whether MEDICHAM is correct is a
gate verdict — `node engine/status.js` names any failing clause — and an open gate makes these re-runnable, not true. So no turn count, no coverage counter, no held-out
...
`require`. Whether MEDICHAM is correct is a gate verdict — `node engine/status.js` names any failing clause — and an open gate makes this re-runnable, not true. No action count,
