...
**7.0.0 - WE NOW TEST THE SIMULATOR ON THREE DIFFERENT SAMPLES OF REAL GAMES INSTEAD OF ONE, AND ON ALL THREE IT AGREES WITH THE OFFICIAL ONE ON EVERY BOARD. OUR RELEASE CHECK READS: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`**

**WHY THIS IS A NEW MAJOR VERSION AND NOT ANOTHER UPDATE.** A major version here does not mean a better number. It means the question changed. For weeks this page has had to say that everything built on top of the simulator was blocked, and that the simulator was the reason. The release check reads open, so that reason is gone. Every page below this block is left as it was written - including the corrections directly beneath it, which said the release check had started failing again. They were true on the day they were written, and this block is what replaces them.
...
**THE HEADLINE TEST, ON THE THREE SAMPLES.** The first sample (1,200 asked for) played 961 games: the boards differed in none of them, and all 10,705 end-of-turn board checks matched. The second (1,350) played 1,069 games: none differed, and 11,842 of 11,842 checks matched. The third (1,950) played 1,497 games: none differed, and 16,713 of 16,713 matched (`data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json`, all on the same frozen copy of the simulator). Nothing is thrown away to get there and no game is cut short by the turn limit; each sample records that on its own. Four games did stop early for another reason: the program that clicks for both players picked a move the official game had switched off that turn, and the official game refused it. Up to that point their boards matched. That is a fault in our clicking program, not in the simulator.

**THE COMMENTARY TEST, WHICH NOW COUNTS AUTOMATICALLY.** Both simulators also write a running commentary of what happened. Will set matching it as a second bar, to come after the boards. That "after" is now built in: the moment the boards stop differing on all three samples, the release check starts requiring the commentary to match too, with nobody having to remember to switch it on. Zero on all three samples. The only commentary differences left are one line on the second sample and two on the third, and they are all the same thing: after Supreme Overlord, the official simulator prints the word `fallenundefined` - a typo in its own code, on a hidden line players never see. We wrote that down as a named permission instead of copying the typo.

**DAMAGE.** Thousands of damage comparisons against the official game, including attacks that hit more than once. 6,000 comparisons at each of 17 dice settings - the middle, both extremes and 14 in between - and none disagree (`data/engine-diff.json`).

**THE LABORATORY, WHERE EVERY MECHANIC IS STAGED ON ITS OWN.** Each legal item, ability and move is set up deliberately, played with it and without it, and compared against the official engine. Items: all 148 fire and leave the same board. Abilities: 194 of the 200 in the game do, and Will has set the other 6 aside. Moves: 494 of 497, with the other 3 set aside. None leave a different board, none fail to happen, and none could not be set up. We also plant deliberate faults to prove the lab can see one - 22 among the items, 63 among the abilities and 37 among the moves - and every one was caught (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`). The separate list of mechanics we probe for: 955 of 955 fire, and none is missing (`data/mechanics-census.json`). Six abilities and three moves are set aside on purpose by Will and are named rather than hidden: Anticipation, Forewarn and Frisk, whose whole effect is a message ("set frisk aside", 2026-09-18); Illusion and Stall, which are in the closet; Pickup, which almost nobody uses; and the moves Axe Kick, Electrify and Copycat.

**WHAT FINISHING THE LABORATORY TAUGHT US.** Will told us to stop trying to close the simulator out while mechanics were still untested, so we tested them: every item, and nearly every ability and move that could be set up at all. Along the way we found that the lab's "control" - the version of each test without the mechanic - was sometimes measuring the wrong thing. Once, 39 abilities we had marked as passing were passing on the control's own paperwork rather than on the ability, so we took them back and rebuilt them properly (CHANGELOG 6.36.0). When fixing the ruler makes your score go down, that is the ruler working.

**WHAT WE FIXED IN THE SIMULATOR SINCE 6.0.0, IN PLAIN TERMS.** Among many others: a Pokemon with Oblivious now ignores Taunt, and Magic Bounce now reflects Spite. A Focus Sash, Focus Band, Sturdy or Endure now answers each hit of a multi-hit attack. Heal Bell now actually cures the team; before, clicking it did nothing at all (CHANGELOG 6.44.0). A mega evolution now switches off the ability it replaces, so a Flash Fire boost no longer survives it (6.45.0). Beat Up now uses the right Pokemon's Attack (6.47.0). Future Sight now hits a Substitute instead of going through it (6.48.0). Mold Breaker now gets through Good as Gold, Magic Bounce and Sticky Hold (6.49.0). Gooey's Speed drop now triggers Contrary, Defiant and Mirror Armor the way the real game does (6.50.0). Helping Hand now fails if its partner has already moved, and a second Round now goes straight after the first (6.52.0). Clear Smog now resets the target's stat changes, Protean no longer changes type on a status move that fails, and abilities like Cursed Body now get their chance on every hit of a multi-hit attack (6.54.0). Sucker Punch now checks the move its target will really use after Encore changes it, and a Focus Sash or Sturdy now saves a Pokemon from a Future Sight hit or from hitting itself in confusion (6.57.0). Forewarn now announces the move it spots, and Harvest now flips its coin every turn (6.59.0). Many more fixes made the commentary match the official game line for line (6.57.0, 6.59.0). Every fix was shown to be wrong against the official game first, and every one can be switched back off to prove the test can still see it.

**WHAT WE KNOW IS STILL NOT RIGHT, BECAUSE A CLEAN RELEASE CHECK IS NOT THE SAME AS "NOTHING IS WRONG".** At our last notes entry before this page was drafted, seven problems were known and not yet fixed - for example, Protean changing its user's type on a status move that fails, and Clear Smog not resetting the target's stat changes (CHANGELOG 6.52.0). All seven are fixed in this release (CHANGELOG 6.54.0) - except that one turned out not to be a problem at all: no legal item raises a stat, so there was nothing for Contrary to reverse. Three smaller problems found since are still open, and each is about what the commentary prints: Coaching complaining in the wrong place in one game, Tidy Up announcing its effects in the wrong order, and a stat already at its lowest printing the wrong message when something tries to lower it again. None of them shows up in any of the three samples, and none is on our written list of known problems yet, which is why the release check cannot see them.

**WHAT THE TEST DOES NOT COVER, AND THIS IS THE PART READERS SKIP.** Real players' stat spreads are hidden on an open team sheet, so both simulators are handed the same made-up spread: the test proves the two agree, not that the damage matches what real teams roll. Some mechanics never come up in real games at all, so only the laboratory tests them. Attacks that hit a variable number of times are tested at their extremes on purpose and in the middle only when the dice happen to land there. And every game is played by one "driver" that copies what real players click. All of these limits are printed by `node engine/coverage.js` every time it runs, with counts, and the white paper lists them.

**THE TWO DELIBERATELY EXTREME DICE SETTINGS, WHICH WE STILL PUBLISH BESIDE THE HEADLINE AND NOT UNDER IT.** They are not part of the release check. They were not re-run for this release. The last reading belongs to an older copy of the simulator (CHANGELOG 6.22.0): out of 961 games, the boards split in 1 at one extreme and in 2 at the other. The games they still split are all the same thing: a clock that runs out at the very end of a battle, traced through the official simulator's own code and written down as a known difference with the reason attached.
...
because nobody has run them on the corrected simulator.
...
— the release check at the top of this page says whether that is done — then he rebuilds the bot's weights and the search player, and only then does everything on
