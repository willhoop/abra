...
**7.0.0 - WE NOW TEST THE SIMULATOR ON THREE DIFFERENT SAMPLES OF REAL GAMES INSTEAD OF ONE, AND ON ALL THREE IT AGREES WITH THE OFFICIAL ONE ON EVERY BOARD. OUR RELEASE CHECK READS: <<GATE: the first line printed by `node engine/quarantine.js` on the 7.0.0 release; this page is published only if it reads `GATE: OPEN`>>**

**WHY THIS IS A NEW MAJOR VERSION AND NOT ANOTHER UPDATE.** A major version here does not mean a better number. It means the question changed. For weeks this page has had to say that everything built on top of the simulator was blocked, and that the simulator was the reason. If the release check reads open, that reason is gone. Every page below this block is left as it was written - including the corrections directly beneath it, which said the release check had started failing again. They were true on the day they were written, and this block is what replaces them.
...
**THE HEADLINE TEST, ON THE THREE SAMPLES.** <<GATE: for each of the three samples - `data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json` - the games played, the games whose boards ever differ (must be 0), and the turn boundaries compared, in words a reader can follow>>. Nothing is thrown away to get there and no game is cut short; each sample records that on its own.

**THE COMMENTARY TEST, WHICH NOW COUNTS AUTOMATICALLY.** Both simulators also write a running commentary of what happened. Will set matching it as a second bar, to come after the boards. That "after" is now built in: the moment the boards stop differing on all three samples, the release check starts requiring the commentary to match too, with nobody having to remember to switch it on. <<GATE: the commentary differences left on each of the three samples that are not covered by a written, named permission - must be 0 - and any written permission in force, by name>>.

**DAMAGE.** Thousands of damage comparisons against the official game, including attacks that hit more than once. <<GATE: `data/engine-diff.json` - the comparisons made and how many disagree (must be 0)>>.

**THE LABORATORY, WHERE EVERY MECHANIC IS STAGED ON ITS OWN.** Each legal item, ability and move is set up deliberately, played with it and without it, and compared against the official engine. <<GATE: from `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json` - how many of each fire and leave the same board, how many differ (must be 0), how many fail to happen (must be 0), and how many could not be set up>>. The separate list of mechanics we probe for: <<GATE: `data/mechanics-census.json` - live of probed, missing (must be 0)>>. Three abilities are set aside on purpose by Will and are named rather than hidden: Anticipation, Forewarn and Frisk ("set frisk aside", 2026-09-18).
...
**THE TWO DELIBERATELY EXTREME DICE SETTINGS, WHICH WE STILL PUBLISH BESIDE THE HEADLINE AND NOT UNDER IT.** They are not part of the release check. <<GATE: either the two extreme-dice readings re-run on the 7.0.0 release, in words, or "they were not re-run for this release, and the last reading belongs to an older copy of the simulator, in the technical documentation">>. The games they still split are all the same thing: a clock that runs out at the very end of a battle, traced through the official simulator's own code and written down as a known difference with the reason attached.
...
**UPDATE (7.0.0): THE TWO PROBLEMS THAT WERE STILL ON THE LIST WERE FIXED - AND THEN THE CHECK FAILED AGAIN, FOR A BETTER REASON.** The test setups that used Pokemon which cannot legally learn the move they were testing were rebuilt, and a Focus Sash and its relatives now answer each hit of a multi-hit attack (CHANGELOG 6.22.0, 6.24.0). The next day we found that the headline test had only ever looked at one sample of teams, and when it was made to look at three it failed on two of them (CHANGELOG 6.46.0). The corrections above are kept exactly as they were written - each was true when it was written. The current reading is the 7.0.0 block at the top of this page, and the release check itself reads: <<GATE: the first line printed by `node engine/quarantine.js` on the 7.0.0 release, identical to the head block's>>
...
samples read is in the 7.0.0 block at the top of this page. Our release check reads: <<GATE: the first line printed by `node engine/quarantine.js` on the 7.0.0 release, identical to the head block's>>
