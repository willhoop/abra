...
**7.0.0 - WE NOW TEST THE SIMULATOR ON THREE DIFFERENT SAMPLES OF REAL GAMES INSTEAD OF ONE, AND ON ALL THREE ITS BOARDS MATCH THE OFFICIAL ONE. OUR RELEASE CHECK READS: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`. EVERY PART OF THAT CHECK PASSES, INCLUDING THE NEW PARTS ADDED TO CATCH OUR BLIND SPOTS. THAT DOES NOT MEAN THE SIMULATOR IS FINISHED: WHAT THE CHECK DOES NOT COVER IS LISTED BELOW.**

**WHY THIS IS A NEW MAJOR VERSION AND NOT ANOTHER UPDATE.** A major version here does not mean a better number. It means the question changed. For weeks this page has had to say that everything built on top of the simulator was blocked, and that the simulator was the reason. The release check reads open, so that block is lifted. Every page below this block is left as it was written - including the corrections directly beneath it, which said the release check had started failing again. They were true on the day they were written, and this block is what replaces them.

**THE BIG LESSON OF THIS VERSION: ONE SAMPLE WAS NOT ENOUGH.** Our headline test plays real ladder games through our simulator and through the official one, with both forced to roll the same dice, and compares the board after every turn. For 6.0.0 we ran it on one sample of teams, and on that sample the boards never differed. Then we found out that the way the test picks its teams depends on how many games you ask for - ask for a different number and you get different teams, not just more of them. On a different-sized sample from the same frozen set of teams, some boards did differ, and on a much bigger one more of them did (CHANGELOG 6.45.0). The zero we published was true, but it was true of the teams that one sample happened to pick. So the test now runs three samples chosen so they share as few teams as possible, and the release check passes only if all three read zero (CHANGELOG 6.46.0).

**THE SECOND LESSON: THE CHECK ITSELF HAD BLIND SPOTS, SO WE MADE IT STRICTER AND IT FAILED BEFORE IT PASSED.** The check first read open on an earlier copy of the simulator (CHANGELOG 6.61.0). Will then said: *"stop saying medicham is done when all these blind spots remain"*. So the same day we closed five holes in the check itself. It now fails if any mechanic that can appear in this format has not actually been shown to happen; if a board differs even when the commentary matches; if any piece of the board could be left standing at the end of a turn without being compared; if a test game crashes; if the "without it" half of a lab test changes the board; or if a mechanic is credited as happening only because its control did something (CHANGELOG 6.64.0, 6.66.1, 6.67.1). Under the stricter check the earlier copy would not have passed. The next measurement failed, on two causes (CHANGELOG 6.65.0); both were fixed, and on the current copy of the simulator every part of the check passes (CHANGELOG 6.69.0).

**THE HEADLINE TEST, ON THE THREE SAMPLES.** The first sample (1,200 asked for) played 961 games: the boards differed in none of them, and all 10,716 end-of-turn board checks matched. The second (1,350) played 1,069 games: none differed, and 11,856 of 11,856 checks matched. The third (1,950) played 1,497 games: none differed, and 16,725 of 16,725 matched (`data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json`, all on the same frozen copy of the simulator, `6180c4712761`). No game crashed, nothing is thrown away to get there, and no game is cut short by the turn limit; each sample records that on its own. The four games that stopped early last time, because the program that clicks for both players picked a move the official game had switched off, now play to the end: we fixed the clicking program (CHANGELOG 6.63.0).

**THE COMMENTARY TEST, WHICH NOW COUNTS AUTOMATICALLY.** Both simulators also write a running commentary of what happened. Will set matching it as a second bar, to come after the boards. That "after" is built in: the moment the boards stop differing on all three samples, the release check starts requiring the commentary to match too. Zero on all three samples. The only commentary differences left are one line on the second sample and two on the third, and they are all the same thing: after Supreme Overlord, the official simulator prints the word `fallenundefined` - a typo in its own code, on a hidden line players never see. We wrote that down as a named permission instead of copying the typo.

**DAMAGE.** 6,000 damage comparisons against the official game at each of 17 dice settings - the middle, both extremes and 14 in between - including attacks that hit more than once, and none disagree (`data/engine-diff.json`).

**THE LABORATORY, WHERE EVERY MECHANIC IS STAGED ON ITS OWN.** Each legal item, ability and move is set up deliberately, played with it and without it, and compared against the official engine. Items: all 148 fire and leave the same board. Abilities: 194 of the 200 in the game do, and Will has set the other 6 aside. Moves: 494 of 497, with the other 3 set aside. None leave a different board, none fail to happen, and none could not be set up. We also plant deliberate faults to prove the lab can see one - 22 among the items, 63 among the abilities and 37 among the moves - and every one was caught (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`). The separate list of mechanics we probe for: 969 of 969 fire, and none is missing (`data/mechanics-census.json`). A second lab plays 4,638 staged games with no crash (`data/all-mechanics-fire.json`): 198 of the 200 abilities, all 497 moves and all 148 items are shown to happen, and where a test's "without it" half was itself doing something, the credit had to come from the mechanic's own effect - it did, every time. Six abilities and three moves are set aside on purpose by Will and are named rather than hidden: Anticipation, Forewarn and Frisk ("set frisk aside", 2026-09-18); Illusion and Stall, which are in the closet; Pickup, which almost nobody uses; and the moves Axe Kick, Electrify and Copycat.

**WHAT FINISHING THE LABORATORY TAUGHT US.** Will told us to stop trying to close the simulator out while mechanics were still untested, so we tested them: every item, and nearly every ability and move that could be set up at all. Along the way we found that the lab's "control" - the version of each test without the mechanic - was sometimes measuring the wrong thing. Once, 39 abilities we had marked as passing were passing on the control's own paperwork rather than on the ability, so we took them back and rebuilt them properly (CHANGELOG 6.36.0). It happened again this week: some abilities were being credited because their control did something, not because they did. The lab now prefers a control it has measured to be quiet, and each of those abilities was re-tested on its own trigger (CHANGELOG 6.66.0 to 6.68.0). When fixing the ruler makes your score go down, that is the ruler working.

**WHAT WE FIXED IN THE SIMULATOR SINCE 6.0.0, IN PLAIN TERMS.** Among many others: a Pokemon with Oblivious now ignores Taunt, and Magic Bounce now reflects Spite. A Focus Sash, Focus Band, Sturdy or Endure now answers each hit of a multi-hit attack. Heal Bell now actually cures the team; before, clicking it did nothing at all (CHANGELOG 6.44.0). A mega evolution now switches off the ability it replaces, so a Flash Fire boost no longer survives it (6.45.0). Beat Up now uses the right Pokemon's Attack (6.47.0). Future Sight now hits a Substitute instead of going through it (6.48.0). Mold Breaker now gets through Good as Gold, Magic Bounce and Sticky Hold (6.49.0). Gooey's Speed drop now triggers Contrary, Defiant and Mirror Armor the way the real game does (6.50.0). Helping Hand now fails if its partner has already moved, and a second Round now goes straight after the first (6.52.0). Clear Smog now resets the target's stat changes, Protean no longer changes type on a status move that fails, and abilities like Cursed Body now get their chance on every hit of a multi-hit attack (6.54.0). Sucker Punch now checks the move its target will really use after Encore changes it, and a Focus Sash or Sturdy now saves a Pokemon from a Future Sight hit or from hitting itself in confusion (6.57.0). Forewarn now announces the move it spots, and Harvest now flips its coin every turn (6.59.0). Unburden now doubles Speed after a stolen item is lost, Gastro Acid now actually switches off an ability, and Tidy Up announces its effects in the right order (6.63.0). Anger Point now goes off before the rest of the hit's effects (6.66.0), and Sticky Hold no longer protects the item of a Pokemon the same hit knocks out (6.68.0). Many more fixes made the commentary match the official game line for line. Every fix was shown to be wrong against the official game first, and every one can be switched back off to prove the test can still see it.

**WHAT THE CHECK DOES NOT COVER, AND THIS IS THE PART READERS SKIP.** Every part of the release check passes, including the new blind-spot parts. These are the things it still does not cover, and they are why we do not call the simulator finished:
- **Illusion** is set aside and not modelled at all. Teams with it are left out of the samples.
- **Frisk and Axe Kick are still open, and are being fixed now.** Frisk did not go off in its lab test, because nobody on the other side was holding an item. Axe Kick's lab test ends with the target's confusion counter one different from the official game. Both are on Will's set-aside list, which is the only reason the check does not fail on them.
- **The samples are limited.** Three samples of 961, 1,069 and 1,497 games, all from one frozen set of 8,778 open-team-sheet teams, every game stopped at 50 turns (`data/game-differential.json`, `data/game-differential.g1350.json`, `data/game-differential.g1950.json`). Zero in about fifteen hundred games still leaves room for a rare problem - roughly up to one game in five hundred.
- **Closed team sheets and best-of-one games are not in the samples at all.** That is a different game.
- **Made-up stat spreads.** Real players' spreads are hidden on an open team sheet, so both simulators are handed the same made-up spread: the test proves the two agree, not that the damage matches what real teams roll.
- **76 effects print nothing when they happen**, so the commentary test cannot see whether they happened (`data/all-mechanics-fire.json`).
- **A few parts of the board cannot be compared at all**; they are covered only by other tests, and one of them by nothing that went off.
- **Some mechanics never come up in real games**, so only the laboratory tests them; attacks that hit a variable number of times are tested in the middle only when the dice happen to land there; and every game is played by one "driver" that copies what real players click.
All of these limits are printed by `node engine/coverage.js` every time it runs, with counts, and the white paper lists them.

**THE TWO DELIBERATELY EXTREME DICE SETTINGS, WHICH WE STILL PUBLISH BESIDE THE HEADLINE AND NOT UNDER IT.** They are not part of the release check. They were not re-run for this release. The last reading belongs to an older copy of the simulator (CHANGELOG 6.22.0): out of 961 games, the boards split in 1 at one extreme and in 2 at the other. The games they split were all the same thing: a clock that runs out at the very end of a battle, traced through the official simulator's own code and written down as a known difference with the reason attached.

**WHAT WE TOOK OUT OF THESE DOCUMENTS, AND WHY THAT IS NOT A LOSS.** In the 6.x versions we deleted from this deck and its companion documents every remaining number that came from a result built on the old simulator: the bot's fitted weights and what they were fitted on, the reliability of its "am I winning" estimate, every model-against-model score, the search player's step counts. The sentence saying **what was measured** stays in each case; only the number goes. A number with a warning label next to it is a number people quote, and this project has paid for that twice.

**WHAT DOES NOT COME BACK IN THIS VERSION - AND THIS IS THE OTHER PART READERS SKIP.** Nothing downstream is restored here. Will has told us not to re-run the model numbers yet: the bot's move-picker, the search player, the mid-game win estimate, the team side, and every head-to-head between them stay out of these documents while he rebuilds the weights and the search player. **A blocked number does not become true when the simulator passes its check. It becomes re-runnable, and somebody still has to run it.** Nobody has.

**SO OUR HEADLINE SCORE IS STILL BLANK, AND THE REASON HAS CHANGED.** Whether the model's confidence is honest - when it says it is nearly certain, does it win nearly that often - is the one number this project is judged on, and it is not printed anywhere in this deck. Until now the reason was that the simulator underneath it was wrong. Now the reason is that the measurement has not been re-run, by choice, and it is one command away on the day Will wants it.

**TWO HOUSEKEEPING DECISIONS, RECORDED BECAUSE THEY AFFECT NUMBERS.** Two stored ladder games whose recorded winner is corrupt are now excluded by a written declaration naming both games, rather than by a filter that guesses (Will's decision, 6.6.1). And a published figure may no longer point at a frozen copy of the simulator that is not actually saved in the project - four of them did (6.2.0).
...
**UPDATE, 2026-09-11 (6.18.0): THE RELEASE CHECK NOW FAILS ON ONE OF ITS NINE TESTS, NOT TWO.** The test that compares mechanics one at a time with the official simulator passes again. The one still failing is the list of known problems, and two remain on it: some of the lab's move tests were set up on Pokemon that cannot legally learn the move, and a survival effect such as a Focus Sash is applied to a multi-hit attack as a whole instead of hit by hit. We re-ran the check at 15:03 UTC on 2026-09-11 and it printed exactly that. The two-of-nine reading above is kept as it was written.
...
**UPDATE (7.0.0): THE TWO PROBLEMS THAT WERE STILL ON THE LIST WERE FIXED - AND THEN THE CHECK FAILED AGAIN, FOR A BETTER REASON.** The test setups that used Pokemon which cannot legally learn the move they were testing were rebuilt, and a Focus Sash and its relatives now answer each hit of a multi-hit attack (CHANGELOG 6.22.0, 6.24.0). The next day we found that the headline test had only ever looked at one sample of teams, and when it was made to look at three it failed on two of them (CHANGELOG 6.46.0). The corrections above are kept exactly as they were written - each was true when it was written. The current reading is the 7.0.0 block at the top of this page, and the release check itself reads: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`
...
**THE SCORE MOVED AGAIN, AND TWO REAL THINGS IN THE GAME ARE NOW RIGHT.** We replay 961 stored games through both simulators. The games where the two end up with a different board went from 37 to 35, and the games where they first say something different went from 122 to 120. Both games that closed were traced by name, and no new one opened. **The published file, `data/game-differential.json`, was again deliberately not rewritten and still reads 46 of 961** - stale as a description of tonight's engine. Say which file a score came from, every time.
...
**THE MORE IMPORTANT CHANGE IS WHAT KIND OF FAILURE IT IS NOW.** Our gate has eight questions that can hold it shut. Earlier in the day six of them failed, and five of those six failed for the worst possible reason: the evidence file could not prove which version of the engine it was measured on, so we knew nothing at all. All five were re-measured against one frozen copy of the engine. **Nothing is in that state any more.** One question still fails, and it fails for the good reason: a real instrument, run on the engine we have today, found real trouble in 46 real games. Going from "we do not know" to "we know, and it is bad" is progress even when the number itself is not pretty.
...
*(Updated for 7.0.0.)* The simulator has since had many more fixes, and whether it passes the
comparisons described at the top of this page is the release check printed there. If it does, this
paragraph's "run it again" is the only thing standing between us and an answer. It is still Will's
call, and it has not been run.
...
*(Updated for 7.0.0.)* The MEDICHAM line above is now tested on three samples of real games rather
than one, and once the boards agree on all three the commentary has to agree too. What the three
samples read is in the 7.0.0 block at the top of this page. Our release check reads: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`
...
*(Updated for 7.0.0.)* The simulator half of that sentence is the release check at the top of this page. The model half is
not: Will is rebuilding the weights and the search player himself, and has asked for none of this to
be re-measured until he has. So these results stay out of the deck — not because they are bad, but
because nobody has run them on the simulator as it now stands.
...
*(Updated for 7.0.0.)* That bullet is unchanged, and the reason it is unchanged is the point
of this release: if the release check at the top reads open, the measurement is **re-runnable**
rather than blocked — and it has not been re-run. Re-runnable is not the same as true. It comes back
on the day Will asks for it, with a fresh sample, and not before.
...
*(Updated for 7.0.0.)* The order of work is now settled and it is Will's: the simulator first
— the release check at the top of this page says whether it passes, and its limits are listed there — then he rebuilds the bot's weights and the search player, and only then does everything on
this list get measured again. Nothing on it was re-run for this release.
...
*(Updated for 7.0.0.)* Still true, and still by choice. Nothing in this release measures how
beatable the bot is, because that measurement reads the weights Will is replacing.
...
*(Updated for 7.0.0.)* One piece of that arrived. A search has to copy the position before it
can try a move, and nobody knew what a copy costs: it is about a third of a turn in our simulator and
slightly under half in the official one, a small fraction of a whole play-out in both. So copying
positions is not what limits a search — playing turns is. Our copies were also checked for
independence, and the official engine's in-memory copy is not independent while ours is (6.9.2).
