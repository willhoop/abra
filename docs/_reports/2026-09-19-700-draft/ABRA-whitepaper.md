...
**7.0.0 — THE MEDICHAM GATE IS READ ON THREE TEAM LATTICES AND NOT REMEMBERED. IT READS: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld` — RELEASE `4c9b0cc4a4da`, `CHANGELOG.md` 6.61.0. THE INSTRUMENTS THAT DECIDE IT ARE WIDER THAN THEY WERE AT 6.0.0, AND NOTHING DOWNSTREAM OF THE SIMULATOR WAS RE-RUN FOR THIS VERSION.**

**THE CURRENT VERDICT, WHICH IS THE ONLY ONE THAT GOVERNS THIS DOCUMENT.** `node engine/quarantine.js`, run at HEAD `e85e48c8` on release `4c9b0cc4a4da`, prints nine clauses and every one reads PASS: **game differential** — 0 of 6000 at the midpoint and at `top`, `bottom` and `idx01` to `idx14`, seed 20260804 (`data/engine-diff.json`); **deliberate roster / items**, **abilities** and **moves** — 148 of 148, 194 of 200 and 494 of 497 tested (`data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`); **coverage** — every move above the click shelf is measured by the roster or the census; **whole-game BOARD-MATERIAL** — zero on every lattice; **whole-game NARRATION** — zero undeclared on every lattice; **mechanics** — 2 diverge, 1 declared, 1 below the reach shelf, 0 left; **no open, known engine defect** — no open register row names an instrument that is red (194 verdicts read from `data/register-reality.json`). The gate's words *nothing is withheld* describe the gate, which withholds nothing. They do not describe this paper, which still carries no figure downstream of the simulator, by the owner's instruction (below). Every dated block below this one is true of the release it names and is not rewritten; none of them is a reading of today.

**WHAT "MEDICHAM IS CORRECT" MEANS IN THIS VERSION, STATED AS THE CONDITION THE CODE CHECKS.** Let `L = {1200, 1350, 1950}` be the three `--games` values in `LATTICE_SAMPLES` (`engine/quarantine.js`). For each lattice ℓ ∈ L, the whole-game differential plays the teams that `--games` selects from the frozen pool through MEDICHAM and through the official simulator under shared dice, and the gating quantity is

  B_ℓ = `state.games` − `state.games_board_never_diverged`,

read from `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json` respectively. The gate is open only when B_ℓ = 0 for every ℓ ∈ L **and** every other gating clause passes: the damage differential (`data/engine-diff.json`, 0 disagreed at every roll index), the three deliberate-roster stages (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`: 0 FIRED-AND-BOARDS-DIFFER, 0 DID-NOT-FIRE, every red demonstration behaving as its rule predicts, no owner deferral that now passes on its own, and no in-scope COULD-NOT-STAGE row — since `CHANGELOG.md` 6.56.0 an unstaged in-scope mechanic holds the stage shut, per Will's ruling of 2026-09-11, *"stop trying to close medicham out with all these untested mechanics"*; a CONTROL-NOT-QUIET row does not fail the clause and is carried and counted beside it), the staged-mechanics clause (`data/all-mechanics-fire.json`: every diverging mechanic cleared by declaration, by the reach shelf or on decision impact, leaving none), the open-defect clause (no open register row whose deciding instrument reads red) and the coverage clause. A lattice whose artifact is missing, stale, unpinned, mislabelled or a duplicate of another lattice is CANNOT-ANSWER, never a pass (ROADMAP #619, `CHANGELOG.md` 6.46.0). **And narration is the second gate Will named on 2026-08-22, which the code makes automatic:** the narration clause reports without gating while any B_ℓ is non-zero, and holds the gate shut the moment every B_ℓ reads zero, so an open gate also means the undeclared narration-only count N_ℓ is zero on every lattice. Nobody switched that on; it is the board clause's own verdict, read on the same run.

**WHY THREE LATTICES AND NOT ONE, WHICH IS THE CENTRAL FINDING OF THE 6.x SERIES.** `engine/diff_swarm.js` picks teams by a deterministic stride whose step is computed from `--games`, and pairs adjacent picks, so `--games` decides WHICH teams and WHICH matchups play, not only how many. The 6.0.0 headline read one such lattice. On one release (`8ad1ab5e1f86`), one census pin and one frozen pool, with only `--games` moving, the gate's `--games 1200` run played 961 games with board-material 0; `--games 1350` played 1069 games with board-material 10; and a `--games 12000` run played 7178 games with board-material 84 (`CHANGELOG.md` 6.45.0). The published zero was a correct reading of a lattice that happened to hold no divergence, and the gate had been certifying the engine off it. The wide draw was not adopted as the gate because it costs more wall-clock time per release than the three lattices together; the lattices were instead chosen by walking the shipping swarm builder so that they share few teams — 72.1% of the `--games 1950` picks fall in neither other lattice (`CHANGELOG.md` 6.46.0). **A zero on three lattices is a fact about three samples.** The gate reads open, so a per-game board-material rate is bounded above, at 95% confidence, by roughly 3/n by the rule of three for a zero numerator (Hanley and Lippman-Hand, *JAMA*, 1983), with n the number of distinct games. **No artifact records that number**: each lattice artifact carries its own totals and no list of its games, and the lattices share teams, so their union cannot be counted from them. The bound is therefore taken on the largest lattice alone, n = 1497 (`state.games`, `data/game-differential.g1950.json`), which gives 3/1497 ≈ 0.20% per game. Treating those games as independent draws is generous, because the lattices share teams and the pool is usage-weighted, so the bound is a floor on the honest uncertainty, not a ceiling.

**WHAT THE GATING ARTIFACTS READ ON THE 7.0.0 RELEASE, WITH BOTH OPERANDS OF EVERY SUBTRACTION NAMED, BECAUSE THE WRONG OPERAND WOULD READ GREENER AND LOOK RIGHT.** Release `4c9b0cc4a4da`, which all eight gating artifacts stamp as `engine_release` — the three lattices, `data/engine-diff.json`, `data/all-mechanics-fire.json` and the three roster stages. It is tracked: `git ls-files data/releases/4c9b0cc4a4da` lists 29 files. Pins, identical on the three lattices: arm `middle`, `steering.policy` `empirical-click/v1`, `--team-store data/team-pool-frozen`, `turns_cap` 50, `--end-state`, census pin `1298f25115e3` (`data/mechanics-census.json`; under empirical steering the census is credited only and selects nothing), pool digests `0d103fb9fa87`, `7e7a37ded7fc` and `a5ce76242f8d`.

| lattice | artifact | board-material B_ℓ | undeclared narration N_ℓ | turn boundaries compared = identical |
|---|---|---|---|---|
| `--games 1200` | `data/game-differential.json` | 961 − 961 = **0** | **0** (0 raw) | 10,705 = 10,705 |
| `--games 1350` | `data/game-differential.g1350.json` | 1069 − 1069 = **0** | **0** (1 raw, less 1 declared: Supreme Overlord `fallenundefined`) | 11,842 = 11,842 |
| `--games 1950` | `data/game-differential.g1950.json` | 1497 − 1497 = **0** | **0** (2 raw, less 2 declared: Supreme Overlord `fallenundefined`) | 16,713 = 16,713 |

Nothing may be dropped to reach a zero, and each lattice says so on its own fields in `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json`: `state.games_void_excluded` reads 0 against `state.games_before_void_exclusion` 961, 1069 and 1497, and `state.games_cut_off_by_the_turn_cap` reads 0 on all three at `turns_cap` 50. `end_state[0].summary.verdicts` reads SAME-END-STATE 960, 1068 and 1495, DIFFERENT-END-STATE 0, ENDED-APART 0, NO-COMPARABLE-BOARD 0, and THREW 1, 1 and 2. **The four THREW games are stated, not hidden.** In each, the authority refused a click that the empirical driver drew for a move disabled on that turn — a disabled Protect in three, a disabled Matcha Gotcha in one (`errors` in each artifact) — and the game stopped there. Every turn boundary compared before the stop agreed, which is why each counts in `state.games_board_never_diverged`. That is a defect of the driver, not of the engine, and the same games threw on the previous release (`docs/_reports/2026-09-19-4c9b-remeasure.md`).

**THE DAMAGE DIFFERENTIAL, WITH THE VOLLEY LOOP INSIDE IT.** `data/engine-diff.json`: `requested` 6000 and `compared` 6000 with `disagreed` 0 at the midpoint, and `compared` 6000 with `disagreed` 0 at each of the sixteen other indices in `arms` (`top`, `bottom`, `idx01` to `idx14`), seed 20260804; `skipped_multihit` 0 and `skipped_ability_multihit` 0; `volley.move_rows` 130 and `volley.bond_rows` 12; `volley.unstageable` one move, Dragon Darts, whose own hit loop in the authority throws in a one-against-one harness (ROADMAP #574); and `volley.no_arrival` 4, clicks the authority refused before the first arrival (Sucker Punch 3, Last Resort 1). The harness enters the authority's own volley loop and reads the arrival count back from it (`CHANGELOG.md` 6.1.0), so multi-hit moves and Parental Bond clicks are compared rather than skipped by construction.

**THE DELIBERATE ROSTER — ONE MECHANIC STAGED AT A TIME, WITH SCOPE DECIDED IN ONE PLACE.** From `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`: **items** — FIRED-AND-BOARDS-MATCH 148 of `scope.in_scope` 148, FIRED-AND-BOARDS-DIFFER 0, DID-NOT-FIRE 0, `scope.could_not_stage_in_scope` 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 0, `plant_anchors` 22 checked with `dead` empty, 22 of 22 red demonstrations CAUGHT; **abilities** — 194 of 200, DIFFER 0, DID-NOT-FIRE 0, could-not-stage in scope 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 6, 63 anchors checked with none dead, 63 of 63 CAUGHT, and 116 out of scope by code (no legal carrier 114, validator-refused 1, no legal reader 1); **moves** — 494 of 497, DIFFER 0, DID-NOT-FIRE 0, could-not-stage in scope 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 3, 37 anchors checked with none dead, 37 of 37 CAUGHT, and 3 out of scope with no legal carrier. The abilities stage is the 6.61.0 run, made after the Harvest red plant that the 6.59.0 coin fix had stranded was re-aimed (`CHANGELOG.md` 6.61.0). Scope is `engine/legal_scope.js`'s verdict for every instrument, so an entity no legal body can carry is out of scope with a code and is never counted as a staging gap (`CHANGELOG.md` 6.28.0). The rows the owner has set aside are DEFERRED-BY-OWNER by his decision and are named, not hidden (read `deferred` on each row): the abilities Anticipation, Forewarn and — since 2026-09-18, Will: *"set frisk aside."* — Frisk, whose whole effect is a message a board cannot see (`CHANGELOG.md` 6.49.0), Illusion (ROADMAP #160), Pickup, shelved on fixture cost against its usage, and Stall (ROADMAP #195); and the moves Axe Kick and Electrify, below the usage shelf of 25, and Copycat.

**THE CENSUS AND THE STAGED GAMES.** `data/mechanics-census.json`: `live` 955 of `probed` 955, `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true. `data/all-mechanics-fire.json`: `games_played` 4700, `games_threw` 0, `planner.played.games` 3836; `summary.moves.diverged` 1, `summary.abilities.diverged` 1, `summary.items.diverged` 0. The clause's own sentence: *2 diverge, 1 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0*. The declared row is Supreme Overlord `fallenundefined`; the reach-shelf row is Gastro Acid, below the reach shelf (ROADMAP #618, open); 21 ability rows never fired and the clause names them a harness gap; and three diverging rows sit on the owner's Illusion shelf and do not vote (ROADMAP #160). **One harness row parts a board and is not counted, because it parts no protocol line:** Axe Kick's fixture reads `board.verdict` STATE, the target's confusion counter one apart (`rows.moves`, `data/all-mechanics-fire.json`). Axe Kick is DEFERRED-BY-OWNER in the roster, below the usage shelf of 25, so no gating clause reads it.

**THE OPEN-DEFECT CLAUSE.** The clause prints PASS — *clean: no open row names an instrument that is RED — no open defect is backed by a failing measurement (194 verdict(s) read)*. `node engine/open_work.js` at HEAD `e85e48c8` reads 585 register rows, 201 open, 20 of them asserting breakage. The clause passes because none of the 20 is decided by an instrument that reads red; it prints 18 of them as asserting breakage with no instrument that decides them — debt, not evidence — and it warns that `data/register-reality.json` (2026-09-12) is older than `docs/ROADMAP.md`, so a row changed since is judged on an older verdict. Known and unfixed at the last notes row before this draft, each named in `CHANGELOG.md` 6.52.0 as owed to ENGINE: Cursed Body rolling once per move where the authority rolls per hit, Triple Axel's per-hit accuracy rolled up front, Clear Smog never clearing the target's boosts, stat-raising items ignoring Contrary, Mold Breaker not suppressing a target's Contrary on the boost road, Cud Chew's pending berry surviving a switch, and Protean converting its user on a status move whose `onTry` fails — the last exposed on the Greninjite row of the item stage once the roster's idle click became a legal one (`CHANGELOG.md` 6.51.0). All seven are closed on this release. Cursed Body and the derived class of on-hit chance abilities roll once per landed hit, Triple Axel draws its accuracy at the top of each hit, Clear Smog clears the target's boosts, a Mold Breaker move lands its stat change on a Contrary target as written, Cud Chew's pending berry clears on switch-out, and Protean honours a refused Try, which moved the Greninjite row from DIFFER to MATCH (`CHANGELOG.md` 6.54.0). The stat-raising-item lead was false: no legal item raises a stat (`CHANGELOG.md` 6.54.0). **Still open, found after 6.52.0 and not fixed, and named by no register row, so the PASS above says nothing about them:** Coaching printing `-fail` after `[notarget]` in one Perish Song game (`docs/RUNNING-NOTES.md` 6.57.0); Tidy Up's boost-and-removal order and its missing `-activate`, and a stat refused at −6 that should print `-unboost|0` (`docs/RUNNING-NOTES.md` 6.59.0). None of them parts a lattice game on this release. Instrument debts from the same rows: `tests/probe_confusion_selfhit_chance.js` was not run in a worktree (6.57.0); `tests/probe_stealeat_derivation.js` pins an old release, and `probe_moldbreaker_refusals.js` builds five illegal fixture sets (6.59.0).

**THE TWO SPEED-TIE CORNER ARMS ARE THE HONEST EDGE OF THIS BLOCK, AND THEY ARE NOT PART OF THE GATE.** `B_ℓ = 0` is a statement about the `middle` arm. The corners hold every damage roll at an extreme and are published beside the headline, never pooled with it. Neither corner was re-measured on this release. The last reading is on release `b42b81899631` (`CHANGELOG.md` 6.22.0): board-material (`state.games` less `state.games_board_never_diverged`) is 1 of 961 at top-tie-first and 2 of 961 at bottom-tie-first, and `state.protocol_diverged_games` is 12 and 15 (`data/verification/game-differential-top-tie-first.json`, `data/verification/game-differential-bottom-tie-first.json`). That is a reading of an older engine, not of this one. The games the corners still part are end-of-battle clock cards, DECLARED and not fixed (ROADMAP #584): a battle that ends in the residual spends one more handler in the authority and every remaining one here, which changes the last board of a finished battle only and so cannot change a click.

**WHAT THIS VERSION DOES NOT COVER, PRINTED BY `node engine/coverage.js` RATHER THAN DESCRIBED.** Every row below is a limit on the word "correct", derived from the artifacts, and every one of them survives an open gate. (1) **No differential body plays a real spread.** An open team sheet carries none, so `engine/game_differential.js` assigns one from the body's slot index, and both engines are handed the same invented spread: the run is internally consistent and its damage is not metagame damage (`node engine/coverage.js` counts no body built on a real spread; `declared_gaps` in `data/game-differential.json` counts 17,440 bodies built with the sheet's own nature and 96 fallen back to Serious). The natures are real. (2) **Some entities are never exercised in a real game**: 852 of 887 entities exercised, with 68 unmeasurable by this instrument and 35 measurable and not exercised (`node engine/coverage.js`, from `coverage` in `data/game-differential.json`); the roster and the census carry them one at a time, and a mechanic nobody brings to the pool is a fact about the metagame, not a hole in the pool (Will, 2026-08-23). (3) **The interior of a ranged mechanic is reached by draw, not by an arm**: the pinned arms reach the two ends of a hit-count range and the damage differential reaches the interior only when its draw lands there (0 of 8 ranged mechanics fully staged by an arm, all eight of them multi-hit moves; the damage differential's own rows arrived at one to six hits (`node engine/coverage.js`)). (4) **A move row can resolve on its announcement alone**, and some leaves print nothing when they fire (7 of 11 move leaves had their effect exercised and 4 resolved on the announcement alone; 76 further leaves print nothing when they fire and sit outside that denominator (`node engine/coverage.js`, from `summary.moves.leaf_effect` in `data/all-mechanics-fire.json`)). (5) **One driver.** Every lattice is played by `empirical-click/v1`, which draws each click from recorded human play; on the `--games 1200` lattice 960 of 961 games reached a result and none stopped at the turn cap (`node engine/coverage.js`). A verdict is about the policy it was taken under. (6) **Declared divergences are subtracted by declaration**, each named in `DECLARED_DIVERGENCE` in `engine/quarantine.js` with its reason (one row: Supreme Overlord `fallenundefined`. The authority guards the ability's start on the faint count and not its end, so when nothing has fainted it prints the literal `fallenundefined` on a `[silent]` line players never see; the ability's damage is correct, and copying the typo would make this engine less correct). (7) **Illusion teams are dropped** from the pool (`closet.teams_dropped` 43, 63 and 71 on the three lattices, ROADMAP #160, Will 2026-08-11).

**THE BASIS MOVED, AND THAT IS WHY THIS IS A MAJOR RATHER THAN A BETTER NUMBER.** The 6.0.0 headline and this one are not two readings of one question, and a reader can no longer be told three things. **First, that the whole-game zero is the same series**: 6.0.0 read one team lattice, and 7.0.0 requires zero on three chosen to share few teams (`CHANGELOG.md` 6.45.0, 6.46.0); the `--games 1200` reading links, and the other two extend it into teams the 6.0.0 gate never played. **Second, that the lab's denominators mean what they meant**: which mechanics exist in the regulation is decided by one module for every instrument (`CHANGELOG.md` 6.10.0), a fixture is derived from each mechanic's own requirements rather than hand-picked (6.9.1, 6.14.0), fixture bodies learn the moves they click (6.24.0), and a could-not-stage row is counted only when the entity is in scope (6.28.0). **Third, that a green in the lab is what it was**: the control arm that makes a roster row a one-variable experiment was found three times to be measuring itself or the swapper rather than the subject (`CHANGELOG.md` 6.34.0, 6.36.0, 6.39.0), and 39 ability greens resting on the control describing itself were refused (6.36.0). The ESS *Guidelines on Revision Policy* (Eurostat, `KS-RA-13-016`, Item 3.4) ask a major revision to give its reasons, its impact and a comparison of the old series with the new; this block and §3.2 are that comparison.

**A LIFTED QUARANTINE MAKES NOTHING DOWNSTREAM TRUE, AND NOTHING DOWNSTREAM WAS RE-RUN.** The gate reads open, so every artifact `node engine/quarantine.js` lists as downstream of MEDICHAM stops being withheld and becomes RE-RUNNABLE — it does not become true, because each was measured under an engine that has since changed. **None was re-run for this version, by the owner's instruction** (Will, 2026-08-11: *"Dont re run the artifacts weve bene over this"* — MAG and MILTANK are being reworked and he does not want the time spent twice); the re-run list is ROADMAP #57. So every figure downstream of the simulator is **absent from this paper with its generator named**: the model family's head-to-heads, the rollout gates R1 to R4, the exploitability runs, the fitted and joint weights, and **leaf calibration, this project's one measurement number, whose generator is `node engine/backtest_winrate.js` and whose artifact is `data/winrate-backtest.json`**. No reader may infer a direction from any of those absences, and **no figure withdrawn from this document in the 6.x passes is restored by this version.** The documents' own quarantine check does not switch off when the gate opens: it keeps charging any figure read from a downstream artifact that does not name the current release (`CHANGELOG.md` 6.30.0). The MAG refit stays owed and is a REFIT rather than a restamp: `data/policy-weights.json` was not touched and the damage table its vector was fitted against has moved, so the feature function's input changed.

**WHAT THIS VERSION DOES NOT ESTABLISH, WRITTEN OUT SO IT CANNOT BE INFERRED.** That MEDICHAM is correct outside the three lattices, the lab and the census: the bound above is a bound, and the limits above are limits. That any model downstream is good, or even measured. That the corner arms are clean. That search pays (ROADMAP #62): phase 1 of §0.3 is a gate verdict rather than a declaration, and phase 2 has not begun.
...
measured. Whether MEDICHAM is correct today is a gate verdict: `engine/quarantine.js` computes it and
...
**Whether they add up to "MEDICHAM is correct" is a GATE, read and never remembered: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld`.**
...
are counts of games this simulator could build a board for. Whether MEDICHAM is correct is a gate verdict —
...
it scored were scored through a simulator the gate does not certify. Whether MEDICHAM is correct is a gate verdict —
