...
**7.0.0 — THE MEDICHAM GATE IS READ ON THREE TEAM LATTICES AND NOT REMEMBERED. IT READS: `GATE: OPEN — MEDICHAM passes both conditions; nothing is withheld` — RELEASE `6180c4712761`, `CHANGELOG.md` 6.69.0. EVERY GATING CLAUSE PASSES, INCLUDING THE BLIND-SPOT CLAUSES ADDED ON 2026-09-19. THAT IS NOT A CLAIM THAT MEDICHAM IS CORRECT: THE LIMITS THE GATE DOES NOT COVER ARE LISTED BELOW, AND NOTHING DOWNSTREAM OF THE SIMULATOR WAS RE-RUN FOR THIS VERSION.**

**THE CURRENT VERDICT, WHICH IS THE ONLY ONE THAT GOVERNS THIS DOCUMENT.** `node engine/quarantine.js`, run on 2026-09-19 at HEAD `165b338c` on release `6180c4712761`, prints ten clauses and every one reads PASS: **game differential** — 0 of 6000 at the midpoint and at `top`, `bottom` and `idx01` to `idx14`, seed 20260804 (`data/engine-diff.json`); **deliberate roster / items**, **abilities** and **moves** — 148 of 148, 194 of 200 and 494 of 497 tested (`data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`); **coverage** — all 412 moves above 25 clicks are measured by the roster or the census; **board leaves** — 0 leaves a legal mechanic writes can stand at a turn boundary uncompared, 56 compared, `BS.snapshot` with 1 call site; **whole-game BOARD-MATERIAL** — zero on every lattice; **whole-game NARRATION** — zero undeclared on every lattice; **mechanics** — 0 diverge (+1 board-only), 0 declared, 1 below the reach shelf, 0 left, every in-scope row proven or excused by the owner, 0 FIRED rows on a live control without the subject's own receipt, and no control arm that parted a board (`data/all-mechanics-fire.json`); **no open, known engine defect** — no open register row names an instrument that is red (194 verdicts read). The gate's words *nothing is withheld* describe the gate, which withholds nothing. They do not describe this paper, which still carries no figure downstream of the simulator, by the owner's instruction (below). Every dated block below this one is true of the release it names and is not rewritten; none of them is a reading of today.

**WHAT THE GATE REQUIRES IN THIS VERSION, STATED AS THE CONDITION THE CODE CHECKS.** Let `L = {1200, 1350, 1950}` be the three `--games` values in `LATTICE_SAMPLES` (`engine/quarantine.js`). For each lattice ℓ ∈ L, the whole-game differential plays the teams that `--games` selects from the frozen pool through MEDICHAM and through the official simulator under shared dice, and the gating quantity is

  B_ℓ = `state.games` − `state.games_board_never_diverged`,

read from `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json` respectively. **A lattice game that threw fails this clause** (`CHANGELOG.md` 6.64.0): it reads `threw` and `errors[]`, takes the larger when they disagree, and cannot answer when both are absent. The gate is open only when B_ℓ = 0 and no game threw for every ℓ ∈ L **and** every other gating clause passes: the damage differential (`data/engine-diff.json`, 0 disagreed at every roll index), the three deliberate-roster stages (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`: 0 FIRED-AND-BOARDS-DIFFER, 0 DID-NOT-FIRE, every red demonstration behaving as its rule predicts, no owner deferral that now passes on its own, and no in-scope COULD-NOT-STAGE row — since `CHANGELOG.md` 6.56.0 an unstaged in-scope mechanic holds the stage shut, per Will's ruling of 2026-09-11, *"stop trying to close medicham out with all these untested mechanics"*), the coverage clause, the board-leaf clause, the staged-mechanics clause, and the open-defect clause (no open register row whose deciding instrument reads red). A lattice whose artifact is missing, stale, unpinned, mislabelled or a duplicate of another lattice is CANNOT-ANSWER, never a pass (ROADMAP #619, `CHANGELOG.md` 6.46.0). **And narration is the second gate Will named on 2026-08-22, which the code makes automatic:** the narration clause reports without gating while any B_ℓ is non-zero, and holds the gate shut the moment every B_ℓ reads zero, so an open gate also means the undeclared narration-only count N_ℓ is zero on every lattice.

**THE BLIND-SPOT CLAUSES — WHAT A PASS HAD NOT REQUIRED BEFORE 2026-09-19, AND NOW DOES.** Will, 2026-09-19: *"stop saying medicham is done when all these blind spots remain"*. The gate read OPEN on release `4c9b0cc4a4da` (`CHANGELOG.md` 6.61.0) under nine clauses. Five holes were then closed in `engine/quarantine.js`, each shown red on a deliberate break before it was trusted, and under them that release's artifacts would not open the gate (`CHANGELOG.md` 6.64.0: 63 in-scope rows unproven). (1) **Proof of every in-scope mechanic** (6.64.0): an in-scope ability or item must read FIRED with a control arm, and a move must resolve on both engines; a missing row fails; only the owner's `tests/roster.js DEFERRED` map and the Illusion closet excuse a row. (2) **A board that parts while the protocol agrees** is judged like a divergence (6.64.0); before, it went to no bucket. (3) **A new clause: no leaf that can stand at a turn boundary goes uncompared** (6.64.0), read from `tests/probe_uncompared_leaves.js`. (4) **A board that parts in a staged row's CONTROL arm fails the mechanics clause** (6.66.1); it is excused only when the control mechanic itself is on the owner's shelf, never because the row's subject is. (5) **A FIRED row on a live control must be earned by the subject's own receipt** (6.67.1); a FIRED credit that only the control's own action produced fails like an unproven row. The re-measure on `d92bdfb50d88` then read `GATE: CLOSED — 2 of 10` (`CHANGELOG.md` 6.65.0: a dead roster anchor and 9 unproven staged rows); both causes were closed (6.66.0, 6.67.0, 6.68.0), and the re-measure on `6180c4712761` reads OPEN (6.69.0).

**WHY THREE LATTICES AND NOT ONE, WHICH IS THE CENTRAL FINDING OF THE 6.x SERIES.** `engine/diff_swarm.js` picks teams by a deterministic stride whose step is computed from `--games`, and pairs adjacent picks, so `--games` decides WHICH teams and WHICH matchups play, not only how many. The 6.0.0 headline read one such lattice. On one release (`8ad1ab5e1f86`), one census pin and one frozen pool, with only `--games` moving, the gate's `--games 1200` run played 961 games with board-material 0; `--games 1350` played 1069 games with board-material 10; and a `--games 12000` run played 7178 games with board-material 84 (`CHANGELOG.md` 6.45.0). The published zero was a correct reading of a lattice that happened to hold no divergence, and the gate had been certifying the engine off it. The wide draw was not adopted as the gate because it costs more wall-clock time per release than the three lattices together; the lattices were instead chosen by walking the shipping swarm builder so that they share few teams — 72.1% of the `--games 1950` picks fall in neither other lattice (`CHANGELOG.md` 6.46.0). **A zero on three lattices is a fact about three samples.** A per-game board-material rate is bounded above, at 95% confidence, by roughly 3/n by the rule of three for a zero numerator (Hanley and Lippman-Hand, *JAMA*, 1983), with n the number of distinct games. **No artifact records that number**: each lattice artifact carries its own totals and no list of its games, and the lattices share teams, so their union cannot be counted from them. The bound is therefore taken on the largest lattice alone, n = 1497 (`state.games`, `data/game-differential.g1950.json`), which gives 3/1497 ≈ 0.20% per game. Treating those games as independent draws is generous, because the lattices share teams and the pool is usage-weighted, so the bound is a floor on the honest uncertainty, not a ceiling.

**WHAT THE GATING ARTIFACTS READ ON THE 7.0.0 RELEASE, WITH BOTH OPERANDS OF EVERY SUBTRACTION NAMED, BECAUSE THE WRONG OPERAND WOULD READ GREENER AND LOOK RIGHT.** Release `6180c4712761`, which all eight gating artifacts stamp as `engine_release` — the three lattices, `data/engine-diff.json`, `data/all-mechanics-fire.json` and the three roster stages. It is tracked: `git ls-files data/releases/6180c4712761` lists 29 files. Pins, identical on the three lattices: arm `middle`, `steering.policy` `empirical-click/v1`, driver code digest `faf70ecbca71` (`steering.driver_code.digest`), `--team-store data/team-pool-frozen`, `turns_cap` 50, `--end-state`, census pin `57deebd09273` (`steering.input_digest` in `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json`; under empirical steering the census is credited only and selects nothing), pool digests `0d103fb9fa87`, `7e7a37ded7fc` and `a5ce76242f8d` (`steering.team_pool_digest`, 1968, 2206 and 3069 teams picked of 8778). The driver digest moved at `CHANGELOG.md` 6.63.0 and 6.65.1, so no lattice artifact before 6.69.0 is comparable to these.
...
| `--games 1350` | `data/game-differential.g1350.json` | 1069 − 1069 = **0** | **0** (1 raw, less 1 declared: Supreme Overlord `fallenundefined`) | **0** | 11,856 = 11,856 |
| `--games 1950` | `data/game-differential.g1950.json` | 1497 − 1497 = **0** | **0** (2 raw, less 2 declared: Supreme Overlord `fallenundefined`) | **0** | 16,725 = 16,725 |

Nothing may be dropped to reach a zero, and each lattice says so on its own fields in `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json`: `state.games_void_excluded` reads 0 against `state.games_before_void_exclusion` 961, 1069 and 1497, and `state.games_cut_off_by_the_turn_cap` reads 0 on all three at `turns_cap` 50. `end_state[0].summary.verdicts` reads SAME-END-STATE 961, 1069 and 1497, and DIFFERENT-END-STATE, ENDED-APART, NO-COMPARABLE-BOARD and THREW 0 on all three. **The four games that threw on `4c9b0cc4a4da` are gone because the driver was fixed, not because they were excused**: the empirical driver no longer clicks a move the authority refuses (Imprison's hidden disable, and the last-resort fallback), and those games now play to the end with no divergence (`CHANGELOG.md` 6.63.0; `tests/probe_driver_refused_click.js`, knobs `MEDI_DRIVER_HIDDEN_DISABLE_UNREAD` and `MEDI_DRIVER_FALLBACK_FIRST_SLOT`). The three protocol divergences left are the declared Supreme Overlord `fallenundefined` games, the same games at the same indices as on `d92bdfb50d88`, and each ends the battle on both engines with the board holding (`docs/_reports/2026-09-19-6180-remeasure.md`).

**THE DAMAGE DIFFERENTIAL, WITH THE VOLLEY LOOP INSIDE IT.** `data/engine-diff.json`: `requested` 6000 and `compared` 6000 with `disagreed` 0 at the midpoint, and `compared` 6000 with `disagreed` 0 at each of the sixteen other indices in `arms` (`top`, `bottom`, `idx01` to `idx14`), seed 20260804; `skipped_multihit` 0 and `skipped_ability_multihit` 0; `volley.move_rows` 130 and `volley.bond_rows` 12; `volley.unstageable` one move, Dragon Darts, whose own hit loop in the authority throws in a one-against-one harness (ROADMAP #574; `dropped_by_exception` 1); and `volley.no_arrival` 4, clicks the authority refused before the first arrival (Sucker Punch 3, Last Resort 1). The harness enters the authority's own volley loop and reads the arrival count back from it (`CHANGELOG.md` 6.1.0), so multi-hit moves and Parental Bond clicks are compared rather than skipped by construction.

**THE DELIBERATE ROSTER — ONE MECHANIC STAGED AT A TIME, WITH SCOPE DECIDED IN ONE PLACE.** From `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`, all on `6180c4712761`: **items** — FIRED-AND-BOARDS-MATCH 148 of 148 in scope, FIRED-AND-BOARDS-DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 0, `plant_anchors` 22 checked with `dead` empty, 22 of 22 red demonstrations CAUGHT; **abilities** — 194 of 200, DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 6, 63 anchors checked with none dead, 63 of 63 CAUGHT, and 116 out of scope by code (no legal carrier 114, validator-refused 1, no legal reader 1); **moves** — 494 of 497, DIFFER 0, DID-NOT-FIRE 0, COULD-NOT-STAGE 0, CONTROL-NOT-QUIET 0, DEFERRED-BY-OWNER 3, 37 anchors checked with none dead, 37 of 37 CAUGHT, and 3 out of scope with no legal carrier. Scope is `engine/legal_scope.js`'s verdict for every instrument, so an entity no legal body can carry is out of scope with a code and is never counted as a staging gap (`CHANGELOG.md` 6.28.0). The rows the owner has set aside are DEFERRED-BY-OWNER by his decision and are named, not hidden: the abilities Anticipation, Forewarn, Frisk (Will, 2026-09-18: *"set frisk aside."*; `CHANGELOG.md` 6.49.0), Illusion (ROADMAP #160), Pickup and Stall (ROADMAP #195); and the moves Axe Kick, Copycat and Electrify. **A deferral is a decision, not a pass: two of these rows carry open items, stated in the limits below.**

**THE CENSUS AND THE STAGED GAMES.** `data/mechanics-census.json`: `live` 969 of `probed` 969, `missing` 0, `unarmed` 0, `hollow` 0, `threw` 0, `run_ok` true. `data/all-mechanics-fire.json`: `games_played` 4638, `games_threw` 0, `sheets_unassembled` 0, `red_ok` true, planner games 3936; `summary.moves.diverged` 0, `summary.abilities.diverged` 0, `summary.items.diverged` 0. **Proof, in scope:** abilities FIRED 198 of 200; moves resolved 497 of 497 on the authority and 497 of 497 on MEDICHAM; items FIRED 148 of 148. The two ability rows not proven are the owner's: Frisk reads DID-NOT-FIRE and Illusion UNPROVEN-UNCONTROLLED. **Control watch** (`summary.abilities.control_watch`): 42 FIRED rows rest on a live control, and all 42 are earned by the subject's own authority receipt (40 state, 2 log); 0 are not. **Control arms** (`summary.control_arm_partings`): 841 rows have a control arm, 2 parted, `board_material` 0; both are announcement-only, on Bitter Malice and Night Daze, rows the owner shelved because their only carriers hold Illusion. The clause's own sentence: *0 diverge (+1 board-only), 0 are declared, 1 are below the reach shelf and 0 were cleared on decision impact, leaving 0*. The board-only row is Axe Kick (below).

**THE OPEN-DEFECT CLAUSE.** The clause prints PASS — *clean: no open row names an instrument that is RED — no open defect is backed by a failing measurement (194 verdict(s) read)*. `node engine/open_work.js` at HEAD `165b338c` reads 585 register rows, 201 open, 20 of them asserting breakage. The clause passes because none of the 20 is decided by an instrument that reads red; it prints 18 of them as asserting breakage with no instrument that decides them — debt, not evidence — and it warns that `data/register-reality.json` (generated 2026-09-12) is older than `docs/ROADMAP.md`, so a row changed since is judged on an older verdict. The seven leads `CHANGELOG.md` 6.52.0 owed to ENGINE are closed or refuted (6.54.0). Tidy Up's order and missing `-activate`, and a drop refused at −6 that should print `-unboost|0`, are fixed (6.63.0). **Still open and named by no register row, so the PASS above says nothing about it:** Coaching printing `-fail` after `[notarget]` in one Perish Song game (`docs/RUNNING-NOTES.md` 6.57.0). It parts no lattice game on this release.

**THE TWO SPEED-TIE CORNER ARMS ARE NOT PART OF THE GATE, AND THEY WERE NOT RE-MEASURED.** `B_ℓ = 0` is a statement about the `middle` arm. The corners hold every damage roll at an extreme and are published beside the headline, never pooled with it. Neither corner was re-measured on this release. The last reading is on release `b42b81899631` (`CHANGELOG.md` 6.22.0): board-material (`state.games` less `state.games_board_never_diverged`) is 1 of 961 at top-tie-first and 2 of 961 at bottom-tie-first, and `state.protocol_diverged_games` is 12 and 15 (`data/verification/game-differential-top-tie-first.json`, `data/verification/game-differential-bottom-tie-first.json`). That is a reading of an older engine, not of this one. The games the corners parted were end-of-battle clock cards, DECLARED and not fixed (ROADMAP #584).

**WHAT THE GATE DOES NOT COVER. EVERY GATING CLAUSE PASSES, INCLUDING THE BLIND-SPOT CLAUSES, AND EACH ITEM BELOW SURVIVES THAT.** These are the limits on any sentence about MEDICHAM. The counts are printed by `node engine/coverage.js` or read from the artifact named, never typed.

1. **Illusion is closeted and unmodelled.** Illusion teams are dropped from the pool (`closet.teams_dropped` 43, 63 and 71 on the three lattices; ROADMAP #160, Will 2026-08-11); the staged row reads UNPROVEN-UNCONTROLLED; and the two moves whose only carriers hold Illusion, Bitter Malice and Night Daze, are shelved and still part their announcements. Nothing about Illusion is measured.
2. **Frisk — OPEN, being fixed.** Its row reads DID-NOT-FIRE and is excused only because the owner deferred it. Its earlier FIRED was its control's work (Cursed Body's Disable); on the quiet control now chosen it does not fire, because the authority announces Frisk only for a foe that holds an item and the fixture's foes hold none (`docs/_reports/2026-09-19-6180-remeasure.md`).
3. **Axe Kick — OPEN, being fixed.** Its staged fixture parts a board with no line difference: the target's confusion counter reads 2 on the authority and 1 here (`rows.moves`, `data/all-mechanics-fire.json`). The mechanics clause judges it below the reach shelf and the roster row is deferred by the owner, so no clause fails on it. It is a measured disagreement, not a pass.
4. **The sample is limited.** Three lattices of 961, 1,069 and 1,497 games (`state.games` in `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json`), all drawn from ONE frozen open-sheet pool of 8,778 teams (`steering.team_pool_teams`; `data/team-pool-frozen`), all capped at 50 turns, all played by one driver (`empirical-click/v1`). The zero bounds a per-game rate at about 0.20% by the rule of three on the largest lattice, and that bound is generous (above).
5. **Closed-sheet and best-of-one play is not sampled.** The pool is the open-team-sheet ladder; the bo1 ladder is a different game and no lattice draws from it.
6. **No differential body plays a real spread.** An open team sheet carries none, so `engine/game_differential.js` assigns one from the body's slot index and both engines get the same invented spread; no body is built on a real spread (`node engine/coverage.js`). The natures are real: 17,440 bodies from the sheet and 96 fallen back to Serious (`declared_gaps` in `data/game-differential.json`). The damage is internally consistent and is not metagame damage.
7. **76 effects print nothing when they fire** — Focus Punch's cancel, Beak Blast's burn, Electrify's retype and the rest — and sit outside the protocol denominator; whether they fired is a counter question. Of the 11 move leaves that do print, 7 had their effect exercised and 4 resolved on the announcement alone (`summary.moves.leaf_effect`, `data/all-mechanics-fire.json`).
8. **Uncomparable leaves are covered only by other instruments.** 56 board leaves are compared, and that is the ceiling: 24 leaves a legal mechanic can write can never be seen by the board comparator. 23 of those 24 are covered only because another instrument fired on them; `volatile:powershift` has no writer that fired, so a clean roster row there is the fixture agreeing with itself.
9. **Some entities are never exercised in a real game**: 860 of 901, with 68 unmeasurable by this instrument and 41 measurable and not exercised (`coverage` in `data/game-differential.json`). The roster and the census carry them one at a time.
10. **The interior of a ranged mechanic is reached by draw, not by an arm**: 0 of 8 ranged mechanics fully staged by an arm, all eight multi-hit moves; the damage differential's rows arrived at one to six hits by draw.
11. **Two tags have a carried row that no engine line reads**: `needsUntrackedState` and `readsOwnItem` (`node engine/coverage.js`).
12. **Declared divergences are subtracted by declaration**, each named in `DECLARED_DIVERGENCE` in `engine/quarantine.js` with its reason. One row: Supreme Overlord `fallenundefined`. The authority guards the ability's start on the faint count and not its end, so when nothing has fainted it prints the literal `fallenundefined` on a `[silent]` line players never see; the ability's damage is correct.
13. **The corner arms were not re-measured** on this release (above).
14. **Downstream models are re-runnable and NOT re-run**, by the owner's ruling (next block).

**THE BASIS MOVED, AND THAT IS WHY THIS IS A MAJOR RATHER THAN A BETTER NUMBER.** The 6.0.0 headline and this one are not two readings of one question, and a reader can no longer be told four things. **First, that the whole-game zero is the same series**: 6.0.0 read one team lattice, and 7.0.0 requires zero on three chosen to share few teams (`CHANGELOG.md` 6.45.0, 6.46.0); the `--games 1200` reading links, and the other two extend it into teams the 6.0.0 gate never played. **Second, that the lab's denominators mean what they meant**: which mechanics exist in the regulation is decided by one module for every instrument (`CHANGELOG.md` 6.10.0), a fixture is derived from each mechanic's own requirements rather than hand-picked (6.9.1, 6.14.0), fixture bodies learn the moves they click (6.24.0), and a could-not-stage row is counted only when the entity is in scope (6.28.0). **Third, that a green in the lab is what it was**: the control arm that makes a roster row a one-variable experiment was found three times to be measuring itself or the swapper rather than the subject (`CHANGELOG.md` 6.34.0, 6.36.0, 6.39.0), and 39 ability greens resting on the control describing itself were refused (6.36.0). **Fourth, that a PASS means what it meant**: the gate grew from nine clauses to ten, and its mechanics clause now requires proof of every in-scope row, an earned credit on a live control and a quiet control arm (`CHANGELOG.md` 6.64.0, 6.66.1, 6.67.1). The ESS *Guidelines on Revision Policy* (Eurostat, `KS-RA-13-016`, Item 3.4) ask a major revision to give its reasons, its impact and a comparison of the old series with the new; this block and §3.2 are that comparison.

**A LIFTED QUARANTINE MAKES NOTHING DOWNSTREAM TRUE, AND NOTHING DOWNSTREAM WAS RE-RUN.** The gate reads open, so every artifact `node engine/quarantine.js` lists as downstream of MEDICHAM stops being withheld and becomes RE-RUNNABLE — it does not become true, because each was measured under an engine that has since changed. **None was re-run for this version, by the owner's instruction** (Will, 2026-08-11: *"Dont re run the artifacts weve bene over this"* — MAG and MILTANK are being reworked and he does not want the time spent twice); the re-run list is ROADMAP #57. So every figure downstream of the simulator is **absent from this paper with its generator named**: the model family's head-to-heads, the rollout gates R1 to R4, the exploitability runs, the fitted and joint weights, and **leaf calibration, this project's one measurement number, whose generator is `node engine/backtest_winrate.js` and whose artifact is `data/winrate-backtest.json`**. No reader may infer a direction from any of those absences, and **no figure withdrawn from this document in the 6.x passes is restored by this version.** The documents' own quarantine check does not switch off when the gate opens: it keeps charging any figure read from a downstream artifact that does not name the current release (`CHANGELOG.md` 6.30.0, 6.62.0). The MAG refit stays owed and is a REFIT rather than a restamp: `data/policy-weights.json` was not touched and the damage table its vector was fitted against has moved, so the feature function's input changed.

**WHAT THIS VERSION DOES NOT ESTABLISH, WRITTEN OUT SO IT CANNOT BE INFERRED.** That MEDICHAM is correct: every gating clause passes, including the blind-spot clauses, and the fourteen limits above still stand. That MEDICHAM agrees with the authority outside the three lattices, the lab and the census: the bound above is a bound. That any model downstream is good, or even measured. That the corner arms are clean. That search pays (ROADMAP #62): phase 1 of §0.3 is a gate verdict rather than a declaration, and phase 2 has not begun.

**UPDATE, 2026-09-12 — THE WHOLE-GAME GATE NOW READS THREE TEAM LATTICES, AND ON THEM IT FAILS.** `node engine/quarantine.js` on release `bc8d7cf849dd` prints `GATE: CLOSED — 1 of 8 GATING clauses fail`, and the failing clause is the whole-game BOARD-MATERIAL clause. That clause used to read one sample. The differential chooses its teams by a fixed stride whose step comes from `--games`, so `--games` decides which teams and which matchups play, and the one sample it read held no divergence. The clause now requires zero on three lattices drawn from the same frozen pool under the same pins (ROADMAP #619). Board-material read zero at `--games 1200` (`data/game-differential.json`) and was non-zero at both `--games 1350` (`data/game-differential.g1350.json`) and `--games 1950` (`data/game-differential.g1950.json`). *(Those two counts were superseded by later releases and are deleted here rather than captioned — `CHANGELOG.md` 6.47.0 and 6.51.0; the reading that governs is the 7.0.0 block above.)* So the 2026-09-11 reading below that both whole-game clauses pass, and that the pool sat still while the lab moved, describe that one lattice and not the pool. Account: `docs/_reports/2026-09-12-lattice-gate.md`.
...
**UPDATE, 2026-09-11 (6.22.0, 6.24.0):** both rows named above are closed. The survival clamp now answers each hit of a volley rather than the total (#511, `CHANGELOG.md` 6.22.0), and the deliberate roster’s fixture bodies learn the moves they use, so no staged pair is one the format refuses (#318, `CHANGELOG.md` 6.24.0). The 1-of-9 reading is kept as dated evidence; the verdict that governs is the one in the 7.0.0 block above.
...
*(SUPERSEDED 2026-09-10 by `CHANGELOG.md` 6.1.0, and kept as the dated evidence of what the instrument could not then see: the harness now enters the authority’s own volley loop, both skip counters read 0, and the volley loop IS damage-compared. The current reading is in the 7.0.0 block above.)*
...
*(SUPERSEDED 2026-09-11: on release `b42b81899631` the corner arms read 1 and 2 of 961, off `data/verification/game-differential-top-tie-first.json` and `data/verification/game-differential-bottom-tie-first.json` (`CHANGELOG.md` 6.22.0). The games left are declared, not fixed (ROADMAP #584). The 7.0.0 block above states whether the corners were re-measured on the 7.0.0 release. The dated readings above stand for the release they measured.)*
...
**WHICH ARTIFACT HOLDS WHICH WHOLE-GAME FIGURE.** `data/verification/fix-batch-M6-sidesel.json` holds board-material 53 of 961 and protocol 154 of 961. `data/game-differential.json` is the artifact the gate clause reads; it was not rewritten in this pass either, and still holds board-material 77 of 961 and protocol 168. **SUPERSEDED 2026-09-08, narration batch R.** The second sentence was true when it was written and is no longer: `data/game-differential.json` has been rewritten by every whole-game run since, and it no longer holds the protocol figure quoted above at all. The figure is historic and belongs to the verification artifact named beside it; what the gate clause prints today is what `node engine/status.js` prints, and nothing here is restated as current. Both are correctly measured on identical pins and only the second is published. Name the artifact whenever either figure is quoted.
...
| deliberate roster (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`) | **148 / 190 / 492 tested**, 0 FIRED-AND-BOARDS-DIFFER and 0 DID-NOT-FIRE on all three |
...
| clause | reading |
|---|---|
| deliberate roster — items / abilities / moves (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`) | **148 / 190 / 492 tested** (of 148, 200 and 497 in scope), 0 FIRED-AND-BOARDS-DIFFER and 0 DID-NOT-FIRE on all three; red demonstrations **22 / 62 / 36**, all caught |
...
| clause | reading |
|---|---|
| deliberate roster — items / abilities / moves (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`) | **148 / 190 / 492 tested**, 0 FIRED-AND-BOARDS-DIFFER and 0 DID-NOT-FIRE on all three; red demonstrations **22 / 62 / 36** |
...
The bound on the word "correct" is stated in the 7.0.0 and 6.0.0 blocks at the head of this paper
