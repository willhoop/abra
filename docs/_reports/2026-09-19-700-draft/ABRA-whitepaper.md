...
**7.0.0 — THE MEDICHAM GATE IS READ ON THREE TEAM LATTICES AND NOT REMEMBERED. IT READS: <<GATE: first line printed by `node engine/quarantine.js` on the 7.0.0 release — publish only if it reads `GATE: OPEN`>>. THE INSTRUMENTS THAT DECIDE IT ARE WIDER THAN THEY WERE AT 6.0.0, AND NOTHING DOWNSTREAM OF THE SIMULATOR WAS RE-RUN FOR THIS VERSION.**

**THE CURRENT VERDICT, WHICH IS THE ONLY ONE THAT GOVERNS THIS DOCUMENT.** <<GATE: the full clause list `node engine/quarantine.js` prints on the 7.0.0 release, one PASS line per clause, with the release id it names>>. Every dated block below this one is true of the release it names and is not rewritten; none of them is a reading of today.
...
read from `data/game-differential.json`, `data/game-differential.g1350.json` and `data/game-differential.g1950.json` respectively. The gate is open only when B_ℓ = 0 for every ℓ ∈ L **and** every other gating clause passes: the damage differential (`data/engine-diff.json`, 0 disagreed at every roll index), the three deliberate-roster stages (`data/roster.items.json`, `data/roster.abilities.json`, `data/roster.moves.json`: 0 FIRED-AND-BOARDS-DIFFER, 0 DID-NOT-FIRE, every red demonstration behaving as its rule predicts, and no owner deferral that now passes on its own — a COULD-NOT-STAGE or CONTROL-NOT-QUIET row does not fail this clause, it is carried and counted beside it), the staged-mechanics clause (`data/all-mechanics-fire.json`: every diverging mechanic cleared by declaration, by the reach shelf or on decision impact, leaving none), the open-defect clause (no open register row whose deciding instrument reads red) and the coverage clause. A lattice whose artifact is missing, stale, unpinned, mislabelled or a duplicate of another lattice is CANNOT-ANSWER, never a pass (ROADMAP #619, `CHANGELOG.md` 6.46.0). **And narration is the second gate Will named on 2026-08-22, which the code makes automatic:** the narration clause reports without gating while any B_ℓ is non-zero, and holds the gate shut the moment every B_ℓ reads zero, so an open gate also means the undeclared narration-only count N_ℓ is zero on every lattice. Nobody switched that on; it is the board clause's own verdict, read on the same run.

**WHY THREE LATTICES AND NOT ONE, WHICH IS THE CENTRAL FINDING OF THE 6.x SERIES.** `engine/diff_swarm.js` picks teams by a deterministic stride whose step is computed from `--games`, and pairs adjacent picks, so `--games` decides WHICH teams and WHICH matchups play, not only how many. The 6.0.0 headline read one such lattice. On one release (`8ad1ab5e1f86`), one census pin and one frozen pool, with only `--games` moving, the gate's `--games 1200` run played 961 games with board-material 0; `--games 1350` played 1069 games with board-material 10; and a `--games 12000` run played 7178 games with board-material 84 (`CHANGELOG.md` 6.45.0). The published zero was a correct reading of a lattice that happened to hold no divergence, and the gate had been certifying the engine off it. The wide draw was not adopted as the gate because it costs more wall-clock time per release than the three lattices together; the lattices were instead chosen by walking the shipping swarm builder so that they share few teams — 72.1% of the `--games 1950` picks fall in neither other lattice (`CHANGELOG.md` 6.46.0). **A zero on three lattices is a fact about three samples.** If the gate reads open, a per-game board-material rate is bounded above, at 95% confidence, by roughly 3/n by the rule of three for a zero numerator (Hanley and Lippman-Hand, *JAMA*, 1983), with n <<GATE: the count of DISTINCT games across the three lattice artifacts, and the bound 3/n computed from it>>. Treating those games as independent draws is generous, because the lattices share teams and the pool is usage-weighted, so the bound is a floor on the honest uncertainty, not a ceiling.

**WHAT THE GATING ARTIFACTS READ ON THE 7.0.0 RELEASE, WITH BOTH OPERANDS OF EVERY SUBTRACTION NAMED, BECAUSE THE WRONG OPERAND WOULD READ GREENER AND LOOK RIGHT.** Release <<GATE: the release id every gating artifact stamps (`engine_release`), identical across all of them; it must be TRACKED — `git ls-files data/releases/<id>` non-empty — or RULE 5 of `engine/provenance.js` forbids publishing any figure below>>. Pins, identical on the three lattices: arm `middle`, `steering.policy` `empirical-click/v1`, `--team-store data/team-pool-frozen`, `turns_cap` 50, `--end-state`, census pin <<GATE: `steering.input_digest`>>, pool digests <<GATE: `steering.team_pool_digest` of each lattice, in `--games` order>>.

| lattice | artifact | board-material B_ℓ | undeclared narration N_ℓ | turn boundaries compared = identical |
|---|---|---|---|---|
| `--games 1200` | `data/game-differential.json` | <<GATE: `state.games` less `state.games_board_never_diverged`, both operands>> | <<GATE: the narration clause's undeclared count, and any declared row subtracted, by name>> | <<GATE: `state.turn_boundaries_compared` and `state.turn_boundaries_identical`>> |
| `--games 1350` | `data/game-differential.g1350.json` | <<GATE: same two operands>> | <<GATE: same>> | <<GATE: same>> |
| `--games 1950` | `data/game-differential.g1950.json` | <<GATE: same two operands>> | <<GATE: same>> | <<GATE: same>> |

Nothing may be dropped to reach a zero, and each lattice says so on its own fields: <<GATE: per lattice, `state.games_void_excluded` against `state.games_before_void_exclusion`, `state.games_cut_off_by_the_turn_cap`, and `end_state[0].summary.verdicts`>>.

**THE DAMAGE DIFFERENTIAL, WITH THE VOLLEY LOOP INSIDE IT.** `data/engine-diff.json`: <<GATE: `compared` and `disagreed` at the midpoint and at every roll index in `arms`; `skipped_multihit` and `skipped_ability_multihit` (both must read 0); `volley.move_rows`, `volley.bond_rows`, `volley.unstageable`, `volley.no_arrival`>>. The harness enters the authority's own volley loop and reads the arrival count back from it (`CHANGELOG.md` 6.1.0), so multi-hit moves and Parental Bond clicks are compared rather than skipped by construction.

**THE DELIBERATE ROSTER — ONE MECHANIC STAGED AT A TIME, WITH SCOPE DECIDED IN ONE PLACE.** <<GATE: for each of `data/roster.items.json`, `data/roster.abilities.json` and `data/roster.moves.json`: `counts` FIRED-AND-BOARDS-MATCH over `scope.in_scope`, FIRED-AND-BOARDS-DIFFER (must be 0), DID-NOT-FIRE (must be 0), `scope.could_not_stage_in_scope`, CONTROL-NOT-QUIET, DEFERRED-BY-OWNER, and `plant_anchors` `checked` with `dead` empty and every red demonstration CAUGHT>>. Scope is `engine/legal_scope.js`'s verdict for every instrument, so an entity no legal body can carry is out of scope with a code and is never counted as a staging gap (`CHANGELOG.md` 6.28.0). The rows the owner has set aside are DEFERRED-BY-OWNER by his decision and are named, not hidden: Anticipation, Forewarn and — since 2026-09-18, Will: *"set frisk aside."* — Frisk (`CHANGELOG.md` 6.49.0).

**THE CENSUS AND THE STAGED GAMES.** `data/mechanics-census.json`: <<GATE: `live` of `probed`, `missing`, `unarmed`, `hollow`, `threw`, `run_ok`>>. `data/all-mechanics-fire.json`: <<GATE: `games_played`, `games_threw`, `planner.played.games`, and `summary.{moves,abilities,items}.diverged` with the mechanics clause's own sentence on how each divergence is cleared>>.

**THE OPEN-DEFECT CLAUSE.** <<GATE: the clause's PASS line, and `node engine/open_work.js`'s count of open rows asserting breakage on the publish commit>>. Known and unfixed at the last notes row before this draft, each named in `CHANGELOG.md` 6.52.0 as owed to ENGINE: Cursed Body rolling once per move where the authority rolls per hit, Triple Axel's per-hit accuracy rolled up front, Clear Smog never clearing the target's boosts, stat-raising items ignoring Contrary, Mold Breaker not suppressing a target's Contrary on the boost road, Cud Chew's pending berry surviving a switch, and Protean converting its user on a status move whose `onTry` fails — the last exposed on the Greninjite row of the item stage once the roster's idle click became a legal one (`CHANGELOG.md` 6.51.0). <<GATE: for each of those seven, closed (with its CHANGELOG entry) or still open; an open one that no gating instrument reaches must be stated here as open, not omitted>>.

**THE TWO SPEED-TIE CORNER ARMS ARE THE HONEST EDGE OF THIS BLOCK, AND THEY ARE NOT PART OF THE GATE.** `B_ℓ = 0` is a statement about the `middle` arm. The corners hold every damage roll at an extreme and are published beside the headline, never pooled with it. <<GATE: either the corner readings re-measured on the 7.0.0 release — `state.games` less `state.games_board_never_diverged` and `state.protocol_diverged_games` in `data/verification/game-differential-top-tie-first.json` and `data/verification/game-differential-bottom-tie-first.json` — or the sentence "neither corner was re-measured on this release; the last reading is on release `b42b81899631` (`CHANGELOG.md` 6.22.0)">>. The games the corners still part are end-of-battle clock cards, DECLARED and not fixed (ROADMAP #584): a battle that ends in the residual spends one more handler in the authority and every remaining one here, which changes the last board of a finished battle only and so cannot change a click.

**WHAT THIS VERSION DOES NOT COVER, PRINTED BY `node engine/coverage.js` RATHER THAN DESCRIBED.** Every row below is a limit on the word "correct", derived from the artifacts, and every one of them survives an open gate. (1) **No differential body plays a real spread.** An open team sheet carries none, so `engine/game_differential.js` assigns one from the body's slot index, and both engines are handed the same invented spread: the run is internally consistent and its damage is not metagame damage (<<GATE: coverage.js "differential bodies on a REAL spread", which reads zero of the bodies built>>). The natures are real. (2) **Some entities are never exercised in a real game**: <<GATE: coverage.js "entities exercised in a real game", with its unmeasurable and measurable-but-not-exercised counts>>; the roster and the census carry them one at a time, and a mechanic nobody brings to the pool is a fact about the metagame, not a hole in the pool (Will, 2026-08-23). (3) **The interior of a ranged mechanic is reached by draw, not by an arm**: the pinned arms reach the two ends of a hit-count range and the damage differential reaches the interior only when its draw lands there (<<GATE: coverage.js "ranged mechanics fully staged">>). (4) **A move row can resolve on its announcement alone**, and some leaves print nothing when they fire (<<GATE: coverage.js "move leaves whose EFFECT was exercised", with its silent-leaf count>>). (5) **One driver.** Every lattice is played by `empirical-click/v1`, which draws each click from recorded human play; a verdict is about the policy it was taken under. (6) **Declared divergences are subtracted by declaration**, each named in `DECLARED_DIVERGENCE` in `engine/quarantine.js` with its reason (<<GATE: the declared rows in force at publish, by name, or "none">>). (7) **Illusion teams are dropped** from the pool (`closet.teams_dropped`, ROADMAP #160, Will 2026-08-11).
...
**Whether phase 1 is reached is the gate’s verdict at the head of this paper — <<GATE: the first line of `node engine/quarantine.js`, identical to the head block's>> — and phase 2
...
**Whether they add up to "MEDICHAM is correct" is a GATE, read and never remembered: <<GATE: the first
line of `node engine/quarantine.js`, identical to the head block's>>.**
...
the moves it had missed (6.52.0). **Every one was shown red against the authority first and sits behind
its own `MEDI_*` switch**, and each was measured on all three lattices, not on the one that had read
zero. <<GATE: the release on which the three lattices first read zero together, and the CHANGELOG entry
that records it>>.
...
6. **A result that does not record its own configuration is not reproducible, and three of the four
   ...
   **7.0.0: <<GATE: the first line of `node engine/quarantine.js`, identical to the head block's>>** Whichever way it reads, none of R1, R2 and R3 was re-run for this version
