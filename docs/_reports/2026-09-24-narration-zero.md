# Reg M-C narration to zero: the last three causes (abra/regmc 0.116.0 – 0.119.0)
...
|---|---|---|---|
| (a) White Herb order | engine: holders walked in side order | `f96ea904` (0.116.0) | `tests/probe_regmc_white_herb_speed_order.js` |
| (b) Grassy Terrain heal on a tie | engine: a spent flinch was missing from the residual handler list | `c1bec57a` (0.117.0) | `tests/probe_regmc_flinch_residual_list.js` |
| (c) Emergency Exit game | engine: the residual door was counted, not modelled | `3669e2c3` (0.118.0) | `tests/probe_regmc_emergency_exit_residual.js` |
...
match 0.115.0: 955 / 1266 / 1497 played, the same one void game at 1200 (`pair-speedctrl …2680752139`, low-identity),
...
/ 1024 probed** each time (the 0.115.0 figure). None of the three fixes added a census row; the probes carry them. The
