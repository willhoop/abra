...
`abra/regmc 0.87.0` (recoil faint) and `0.88.0` (White Herb). Versions may be renumbered at merge.

## 1. Emergency Exit / Berserk against a recoil KO — `0.87.0`
...
## 2. White Herb against a pivot's switch — `0.88.0`
...
Reg M-C: red on release `a1dd33b2d616` (0.87.0 bytes) and under the knob; green after, counters `{early:1, entry:0}`.
...
| M-C | `a1dd33b2d616` | 0.87.0 | 259 | 0 | 0 |
| M-C | `924597092811` | 0.88.0 | 259 | 0 | 0 |
| M-B | `7822a83cc49b` | base (main) | 260 | 1 | 2 |
| M-B | `4662992b5cc1` | 0.87.0 | 260 | 1 | 2 |
| M-B | `1d13b44973d2` | 0.88.0 | 260 | 1 | 2 |
...
`probe_residual_faint_flush`, `probe_afterfaint_boundary` (the last five run on the 0.87.0 engine).
...
`node tests/test-mechanics.js` on the 0.88.0 engine, both regulations, compared with the census committed at
