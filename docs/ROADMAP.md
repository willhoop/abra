...
| #175 | **TWENTY-THREE TAGS HAVE NO CONSUMER — WE DERIVED THE FACT AND NEVER TAUGHT THE ENGINE TO USE IT.** `test-tag-consumed` names them: `stealsItem`, `announcesOnEntry`, `reflectsStatusToSource`, `refusesIndirectDamage`, `clearsScreensOnEntry`, `nameImplementedBySim`, `passesItemToAlly`, `multihitAlwaysMax`, `ignoresRedirection`, `boostsAtHPThreshold`, `curesStatusResidual`, `healsFromOwnStatus`, `addsOwnSecondary`, `formeFollowsWeather`, `secondaryChanceMult`, `typeFollowsTerrain`, `removesOwnMoveFlag`, `inheritsAllyAbility`, `boostsNotVeryEffective`, `boostsAlliesWithAbility`, `allyBasePowerBoost`, `suppressesOwnItem`, `guaranteesNextMove`. The test also prints a longer DEAD list headed by `ignoresRedirection` (47 uses, Propeller Tail / Stalwart). EACH ONE IS A DECISION, not a batch: wire it, or delete the tag and say why. A derived tag nothing reads is indistinguishable from a tag that is wrong. **Expect this count to MOVE when the tag split lands — re-measure before working it** | open — engine DEFECT. 23 -> 22 on 2026-08-11: `ignoresRedirection` wired and closed, 21 rows untouched because EACH IS A DECISION and batching them is the failure mode. **That one fix found a live bug of #181's exact shape one function over:** `tracksTargetOf` looked for an ability-side `tracksTarget` tag THAT DOES NOT EXIST IN THE ARTIFACT, so `ab==='stalwart'|| open — engine DEFECT. **MEASURED 2026-08-11 AND THE PROPOSED FIX MUST NOT BE ADOPTED AS WRITTEN.** The idea was for `tag_dex` to skip abilities with no legal carrier in Reg M-B. Measured: 294 ability rows, **93 have no legal carrier** (formes and `tier:'Illegal'` checked separately — no mega forme is dropped). Skipping them makes **FIVE TAGS VANISH** — `allyBasePowerBoost`, `amplifiesBoosts`, `auraBreak`, `boostsNotVeryEffective`, `secondaryChanceMult` — and shrinks 41 more. **TWO OF THE FIVE CARRY LIVE CENSUS PROBES** (`auraBreak`, `boostsNotVeryEffective`), so the proposal costs the one number that may never go down, by 2. **AND FIVE OF THE 93 RECORD CORPUS USES** — `serenegrace` 5, `stormdrain` 3, `psychicsurge` 2, `tintedlens` 2, `steelyspirit` 1 — which the carrier derivation says is IMPOSSIBLE. Either the usage corpus contains non-Reg-M-B games or the carrier read is incomplete, and that contradiction is registered separately. Settle it before anything is deleted |
...
| #190 | **FIVE ABILITIES RECORD CORPUS USES THAT THE CARRIER DERIVATION SAYS ARE IMPOSSIBLE.** Found 2026-08-11 while measuring #175's proposal. `serenegrace` 5, `stormdrain` 3, `psychicsurge` 2, `tintedlens` 2, `steelyspirit` 1 — all five sit in the 93 abilities derived as having NO LEGAL CARRIER in Reg M-B, and all five appear in the usage corpus. **Exactly one of two things is true and both are serious:** either the corpus contains games that are not Reg M-B — which would contaminate every usage-weighted figure this project quotes, including the 99%-of-usage coverage bar — or the carrier derivation is incomplete, which would mean the legality filter this repo now relies on everywhere under-reports. **This must be settled before #175's carrier-less proposal is adopted, and before any usage weight is quoted as authoritative.** Note the shape: it is the same class as the mega-ability audit — two derived views of one fact that nobody had compared | open — measure |
