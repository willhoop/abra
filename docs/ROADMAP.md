...
| #161 | **THE FIVE LIVE MATRIX DISAGREEMENTS, REGISTERED SO THEY STOP BEING INVISIBLE.** Measured 2026-08-11 at `--full`: `throatchop -> shielddust` (2,946 uses — the volatile lives in `secondary.onHit`, not `volatileStatus`, so it never enters the loop the Shield Dust gate sits on, and #139's wire closed the other three of that family); `psychicnoise -> shielddust` (223 — DOES declare `volatileStatus`, so that diagnosis does not cover it); `instruct -> goodasgold` (the boost lands twice, once in Showdown — Good as Gold must refuse Instruct); `rockwrecker -> bulletproof` (recharge armed on a fully-blocked move — WIRE 43's comment asserts the opposite of what Showdown does and was written from belief); `psyshieldbash -> aftermath` (the self-boost is skipped, suspected `!m.fainted` ordering). Three carry the harness's own verdict: *the knob is UNWIRED, not miscalculated*. **A register cannot audit itself — these were measured for four days with no row behind them, which is what `engine/open_work.js` now prints** | open — engine |
| #162 | **SPLIT THE COLLAPSED TAGS — WILL'S CALL 2026-08-11: *"we need to split the tags, they require very different things"*.** #59 (the protection counter has three behaviours and our tags carry two) and #60 (Upper Hand and Sucker Punch share one signature: Sucker Punch needs the target ATTACKING, Upper Hand needs the target using PRIORITY) are the same defect — one tag standing for two rules, so one of them is always wrong. #71 and #127 are the same shape at larger scale (105,709 uses). The shape decision was the blocker and it is now made; deriving the conditions is mechanical | open — engine |
| #115 | **RESIDUALS RUN IN SPEED ORDER AND OURS RUN IN SLOT ORDER — CONFIRMED 2026-08-11, AND IT DECIDES GAMES.** Will: *"pokemon faint in speed order"*, *"in trick room do they faint in reverse speed order?"*, *"sandstorm damage, etc"*. All DERIVED: `battle.ts:2811` runs `updateSpeed()` at the head of the residual step, `pokemon.ts:557` sets `speed = getActionSpeed()`, and that returns `10000 - speed` under Trick Room — the residual sort is already inverted before anything resolves. **`medicham2-browser.js:14890` is `for(const x of [...actA,...actB])` — pure slot order, no sort.** THE WHOLE RESIDUAL FAMILY rides on it, not just Perish Song: sandstorm and hail chip, burn and poison, Leftovers, Salt Cure, Yawn, Heal Block. Scoped by measurement: the MEGA step (`:9313`) and the SWITCH-IN step (`:8827`) DO speed-sort, so Intimidate and mega ordering are CORRECT and are not the bug. **NOT COSMETIC** — `battle.ts:2604` awards a simultaneous double-KO to `faintData.target.side`, the side of the LAST faint off the queue, not a tie; so in a mutual-perish endgame the residual order picks the WINNER and we pick it by slot position, and the rollout scores wins. Fix the sort, then stage an ENDGAME fixture: a four-actives-with-benches scenario CANNOT see it — one was written and it passed against a deliberately reversed loop | open — engine |
