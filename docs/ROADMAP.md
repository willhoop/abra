...
| #174 | **`test-assert-mode` IS RED ON PURPOSE AND THE SCENARIOS ARE THE DEFECT.** All twelve rows pass — Good as Gold refuses Corrosive Gas / Dragon Cheer / Electrify / Encore / Howl / Toxic Thread, Levitate refuses Bulldoze / Earth Power / Earthquake / Headlong Rush / Mud-Slap / Scorching Sands — and they pass IDENTICALLY with both abilities deleted from the engine (`--break`). The test says so itself: *"ALL 12 ROWS AGREE — AND THAT CURRENTLY PROVES NOTHING."* The fix is scenarios in which the refusal has an observable consequence, not more rows. Left green it would have published 5,769 teams of false coverage | open — engine |
| #175 | **TWENTY-THREE TAGS HAVE NO CONSUMER — WE DERIVED THE FACT AND NEVER TAUGHT THE ENGINE TO USE IT.** `test-tag-consumed` names them: `stealsItem`, `announcesOnEntry`, `reflectsStatusToSource`, `refusesIndirectDamage`, `clearsScreensOnEntry`, `nameImplementedBySim`, `passesItemToAlly`, `multihitAlwaysMax`, `ignoresRedirection`, `boostsAtHPThreshold`, `curesStatusResidual`, `healsFromOwnStatus`, `addsOwnSecondary`, `formeFollowsWeather`, `secondaryChanceMult`, `typeFollowsTerrain`, `removesOwnMoveFlag`, `inheritsAllyAbility`, `boostsNotVeryEffective`, `boostsAlliesWithAbility`, `allyBasePowerBoost`, `suppressesOwnItem`, `guaranteesNextMove`. The test also prints a longer DEAD list headed by `ignoresRedirection` (47 uses, Propeller Tail / Stalwart). EACH ONE IS A DECISION, not a batch: wire it, or delete the tag and say why. A derived tag nothing reads is indistinguishable from a tag that is wrong. **Expect this count to MOVE when the tag split lands — re-measure before working it** | open — engine DEFECT. 23 -> 22 on 2026-08-11: `ignoresRedirection` wired and closed, 21 rows untouched because EACH IS A DECISION and batching them is the failure mode. **That one fix found a live bug of #181's exact shape one function over:** `tracksTargetOf` looked for an ability-side `tracksTarget` tag THAT DOES NOT EXIST IN THE ARTIFACT, so `ab==='stalwart'||ab==='propellertail'` had been carrying the whole rule, uncounted. Legal carriers derived rather than recalled: Archaludon and Skarmory-Mega |
