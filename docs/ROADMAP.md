...
| #179 | **RECYCLE CARRIES NO MECHANIC TAG AND RESOLVES TO A SPENT TURN.** Found while closing #71\'s seven-move signature 2026-08-11. Pain Split reached `sharehp` and Copycat reached `callmove`; Recycle (3 uses) reaches `{kind:\'pass\'}`, which is the terminal do-nothing branch. Low usage, and registered anyway because a move that provably does nothing is a WEIGHTED PREFERENCE rather than a rule — the same shape as #44 — and because the seven-move signature cannot be called closed while a member of it is unmodelled | open — engine DEFECT |
