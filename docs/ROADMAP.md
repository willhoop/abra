...
| #175 | **TWENTY-THREE TAGS HAVE NO CONSUMER — WE DERIVED THE FACT AND NEVER TAUGHT THE ENGINE TO USE IT.** `test-tag-consumed` names them: `stealsItem`, `announcesOnEntry`, `reflectsStatusToSource`, `refusesIndirectDamage`, `clearsScreensOnEntry`, `nameImplementedBySim`, `passesItemToAlly`, `multihitAlwaysMax`, `ignoresRedirection`, `boostsAtHPThreshold`, `curesStatusResidual`, `healsFromOwnStatus`, `addsOwnSecondary`, `formeFollowsWeather`, `secondaryChanceMult`, `typeFollowsTerrain`, `removesOwnMoveFlag`, `inheritsAllyAbility`, `boostsNotVeryEffective`, `boostsAlliesWithAbility`, `allyBasePowerBoost`, `suppressesOwnItem`, `guaranteesNextMove`. The test also prints a longer DEAD list headed by `ignoresRedirection` (47 uses, Propeller Tail / Stalwart). EACH ONE IS A DECISION, not a batch: wire it, or delete the tag and say why. A derived tag nothing reads is indistinguishable from a tag that is wrong. **Expect this count to MOVE when the tag split lands — re-measure before working it** | open — engine DEFECT. 23 -> 22 on 2026-08-11: `ignoresRedirection` wired and closed, 21 rows untouched because EACH IS A DECISION and batching them is the failure mode. **That one fix found a live bug of #181's exact shape one function over:** `tracksTargetOf` looked for an ability-side `tracksTarget` tag THAT DOES NOT EXIST IN THE ARTIFACT, so `ab==='stalwart'|| open — engine DEFECT. **22 -> 10 on 2026-08-11, and the re-measure was the point: 13 of the 23 ALREADY had consumers.** Two more wired: `nameImplementedBySim` (Corrosion and Early Bird — the param was **PROSE**, which is why nothing could read it, now the `ignoresStatusImmunityFor` and `extraStatusTicks` shapes; **Early Bird is an EXTRA TICK, not a halved duration** — the old prose was wrong) and `suppressesOwnItem` (Klutz, folded into the Magic Room predicate because upstream `Pokemon#ignoringItem` is literally one function). **FOUR OF THE REMAINING TEN HAVE NO LEGAL CARRIER IN REG M-B** — Battery / Power Spot / Steely Spirit, Lock-On, Flower Gift, Ice Face, Power of Alchemy — so they can NEVER leave the dead list and the clean answer is for `tag_dex` to skip carrier-less abilities. Wide blast radius; owner's call |
