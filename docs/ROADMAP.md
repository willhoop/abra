...
| #70 | **TWO USAGE NUMBERS DISAGREE BY UP TO 13x, AND THE COVERAGE BAR RANKS WHAT GETS WIRED BY ONE OF THEM.** Every "N uses" in this project is quoted from `data/tags.json`, and nothing reconciles it against the store it is supposed to summarise. **Met head-on 2026-08-10 while characterising Iron Ball:** `tags.json` says **139 uses**; `g.sheets` in `data/games.ladder.jsonl` is populated on only **1.7% of sides** (open-sheet games) and yields **15** rows; `g.sets` yields **0**. Three sources, three answers, no reconciliation — so no Fling-versus-Trick-Room split could be quoted for the item, and the 139 that justified the fix is itself one of the disagreeing figures. This is not cosmetic: the roster and the WIRE queue are both ORDERED by these numbers, so a 13x error reorders what gets built. Needs one function that answers "how many uses" from a named scope, and every quote moved onto it | open — `docs/ENGINE.md`, `docs/MEASURE.md` |
