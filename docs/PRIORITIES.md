...
| 13a | **`benchRisk` leaks the opponent's actual bring** — CONFIRMED | `fit_policy.js:384` hands `g.brought` (a scan of the **whole replay**) to the FOE's party, so at turn 1 `bench(foe)` is exactly the two of four they really chose, before either is seen. **19.99% of decisions** differ, always *narrowing* — the fit is more confident than the player was. **How much it moves the fit was measured by refitting MAG with and without it — weight movement against the published SE, held-out log-likelihood, and a split-half floor — and that comparison is withdrawn (2026-09-11): it compares two MAG weight vectors, which are withheld, so neither its figures nor its verdict are carried.** Worth fixing for correctness; whether it justifies a refit on its own is open | MEASURE |
...
| 13b | **THE FIT NEVER PASSED ABILITY OR MOVES TO THE BOARD** | `fit_policy.js:376` calls `setSheet(side, sp, {nature, item})`; `magnemite.js:522` calls it with `{nature, item, ability, moves}`. So **every board mon in the fit carries `ability: ''` and `moves: []`** while the bot that plays reads both. **51.52% of decisions** have a different vector (the corpus counts behind that share are MAG's and are withheld with its vector). How far the refit moves the weights was measured and is withdrawn with the MAG vector (2026-09-11). This is CLAUDE.md's named failure — *fitted WITH the sheet visible, played WITHOUT it* — **running in reverse**. Same two-field call in `joint_rows.js:95`, `branch_recall.js:68`, `ko_calibration.js:65`, `redirect_audit.js:120`, `surprise.js:79` and four more | MEASURE |
...
| 13d | **`benchRisk` is identically ZERO in every real MAG game** | `engine/magnemite.js` never calls `setParty` and never reads `bench()` — yet the feature carries a non-zero fitted weight (its value and z-score are MAG's and are withheld with its vector, withdrawn 2026-09-11). A weight fitted on a feature that is always zero in play. Only `miltank.js:394-402` populates it, and with a *guess* | MEASURE |
...
| 43 | ~~ROCKY HELMET IS NOT IN THE ARTIFACT~~ **WITHDRAWN — MY PREMISE WAS WRONG, and it found something better** | 146 items in `data/abra-tags.js` and `rockyhelmet` is not one of them. `linkage.contact.items` is `[]` — **zero items react to contact.** Rocky Helmet is legal in this format (only Choice Specs, Choice Band, Assault Vest and Safety Goggles are banned) and returns 1/6 max HP on every contact hit. **Rocky Helmet is `isNonstandard: 'Past'` in this format — it is BANNED**, exactly like Assault Vest and Safety Goggles, and `TeamValidator` says *"does not exist in Gen 9."* Of the **148 legal items, ZERO test contact**, so `linkage.contact.items = []` is **correct**. The derivation is not item-blind either — with the gate removed it finds the same 40 item reactors. **What it did find: the umbrella `CLAUDE.md` banned-items list was INCOMPLETE.** Also `Past`, therefore also illegal: **Covert Cloak, Clear Amulet, Rocky Helmet**, and **Silk Trap, Obstruct, Burning Bulwark** as moves. Anything team-built off that list was wrong. Fixed, with the one-command check that replaces the hand-maintained list | closed |
...
| 45 | **Half withdrawn.** `silktrap` is `Past` — its absence is CORRECT. `beakblast` carries `inflictsBurn` but **not** `punishesContact` — it burns the attacker on contact during its charge turn, which is precisely the interaction | ENGINE |

| 46 | **The next regulation is NOT a seamless swap today — 20 code files bypass the indirection** | (Will, 2026-08-04: *"As long as it can seamlessly update for the next regulation"*.) `data/regulations.json` **is** the single source of truth and its own `_note` spells out the procedure — archive, add the new regulation, set `active`, and ingest/analysis pick it up. **But 20 code files hardcode `gen9championsvgc2026regmb` past it**, including `engine/mag_bot.js` (**the live bot**) and `engine/mew.js` (self-play). `engine/champions_sim.js` does it right and reads the file.<br><br>The 117 **data** files carrying the string are **correct and must stay** — an artifact recording the format it was computed under is provenance, not a hardcode. Do not "fix" those.<br><br>Full list: `build/triggers.js`, `engine/{analyze,chomp_ev,durable-ingest,fetch_smogon_stats,fit_policy,ingest_ots,mag_bot,mega_harvest,meta-ingest,showdown_bot,smogon_priors,status,validate_damage_sim}.js`, `sim/{champions-battle,generate-dataset,selfplay}.js`, `tests/test-team-preview-race.js`, `web/status-data.js`.<br><br>**The guard, in the shape everything else got tonight:** a test asserting that **no file outside `data/` and `data/regulations.json` contains a literal format id**. Then the next regulation is one edit, and the check proves it rather than somebody hoping | OPS + ENGINE |

| 39a | **TESTING ENVIRONMENT AND PLAYING ENVIRONMENT DO NOT MATCH — the clock edition** | (Will, 2026-08-04: *"when i play the bot i have turned the timer off"*.) That is why a constant `budgetMs: 20000` has always looked fine: **with the timer off there is no budget at all**, so every game anyone has watched MILTANK play had unlimited thinking time.<br><br>But `Config.forcetimer` is **off**, so the timer is per-player and an opponent can type `/timer on` **mid-game**. On the ladder as MAGABRA that means the constraint appears at turn 9, after MILTANK has already spent bank it did not know it had. In a tournament it is on from turn 1. Bank at zero is a **forfeit**, not a skipped move.<br><br>This is `CLAUDE.md`'s *fitting environment and playing environment must match* one layer out — the same error that put MAG's sheet-visible weights into sheet-blind play. **The fix is not a smaller constant**: spend against `secondsLeft / expected remaining decisions`, hold a reserve (a turn timeout costs one move, a bank timeout costs the game), and stop early on positions heading for the tie band, since MILTANK pays full search on the turns it then hands to MAG anyway (their share is a MILTANK-run figure, withheld with MILTANK, withdrawn 2026-09-11). Missing measurement: the per-decision wall-clock **distribution** over a real game — the mean is not what times you out | SEARCH |
...
> sampled with `run_stamp.sourceDigests()`. **The engine was not wrapped.** The old figure is
> retracted anyway, so the position is now *no exploitability number at all*. Two findings survive
> because they are about the tool: the hill-climb stalls in the shipped dimension (its step count is
> withheld with the void run, withdrawn 2026-09-11), and `provenance.js` **falsely cleared** the new artifact on an mtime test it passes by
...
only one of them has been moving.* We have just spent a day trying to improve it on average.
...
to test the exploitability argument DODUO's head-to-head explicitly does **not** settle.
...
| 15 | **17 twin-test statistics on the site with no artifact** | `web/index.html:1107-1108` — a decisive-pair share with its CI, a game count and a paired count, stamped "Confirmed 2026-07-30". They are a head-to-head with no artifact, so they are not restated here (withdrawn 2026-09-11). Only `586,816` is backed. Either MEASURE writes the artifact or the paragraphs go | MEASURE + WEB |
| 16a | **RESOLVED — the MAG fit's game count vs 6,943 is not a bug** | Different populations, both current. **6,943** = clean games in `games.ladder.jsonl` only, **no sheet requirement**. **The fit's count** = clean **open-sheet** games across **three** stores — `fit_policy.js:272` iterates bo3 + ots + ladder and `:250` rejects anything without both sheets. The fit corpus's game count and its split by store are MAG's and are withheld with its vector (withdrawn 2026-09-11); the ladder's share is small because the ladder format's Open Team Sheets are optional and only ~1% of it carries one. **Never print them side by side without their population** | closed |
...
| 25 | **MILTANK ships TWO leaves** | `miltank.js:216-222` is a second hand-rolled playout loop that never calls `rolloutWinProb`. The preview-vs-in-game leaf contrast (its figures withheld with the leaf backtest, withdrawn 2026-09-11) is partly a contrast between *leaves*, not settings | SEARCH |
| 26 | **Preview calls `battleInit({seeded:true})` on a fresh game** | suppresses turn-1 Intimidate and weather setters | SEARCH |
| 27 | **`board.js` scores every switch with one flat feature** | MAG's switching was measured head to head against never switching (retracted 2026-08-06 as confounded, and a head-to-head through the simulator in any case), and R3 counted how often a switch was on the menu (withheld with R3, withdrawn 2026-09-11). A hole in the live policy, not a measurement problem | MEASURE *(owns the refit)* |
...
| 31 | **Self-play volume → 10:1** | the external pipeline that works runs 10 self-play games per human game; MEW has 1,000 against a larger human corpus (its usable count is withheld with the leaf backtest, withdrawn 2026-09-11), built and gated. **After the release boundary** | MEASURE |
...
- **More MAG features.** Four feature additions were measured head to head, with an overdispersion
  check beside them. Those head-to-heads were played through the simulator and are withdrawn
  (2026-09-11), so this rests on priority, not on their results.
- **Deeper search.** The external evidence says the leaf bounds a search agent, and our leaf's
  calibration is withheld. A better search is worth no more than the evaluator it calls.
