...
|---|---|---|---|
| **MEDICHAM** | Hand-written doubles doubles-battle simulator | ⚠️ **Being replaced** | Within 5% of the Smogon calculator on 31 scenarios, but disagrees with the OFFICIAL Champions engine by 31.1 points of win probability. ADR-001: becomes a lookup over precomputed tables. **Mechanics census 208 live of 211 probed, 3 missing with reasons** (`data/mechanics-census.json`, wires 82–116 landed 3.40.0–3.42.0); **generated interaction matrix 99.6% — 1,027 of 1,031 live carrier x reactor cases** of 1,675 staged from a theoretical 8,676 (3.43.0 closed the generator’s own arithmetic: `theoretical = staged + dropped` is now asserted per axis, and the three faults it found moved the denominator, the depth-cap tail and the outcome buckets — the four disagreements are UNWIRED knobs on pairs the smaller generator never emitted, not a regression), **multi-turn field axis 156/156** (`data/interaction-matrix.json`); damage differential **1/150**, the one row a documented harness-layer artifact (Disguise); two-rulebook collision ratchet **2 clashes / 151 comparable facts** (`data/rulebook-collision.json`); DEAD-tag ratchet **61 → 38** |
