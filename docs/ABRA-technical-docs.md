...
**ROSTER.** Scope is decided by legal carrier. Read `scope.carrier_derivation` in `data/roster.items.json`. There are **347** legal species. **72** are mega formes. **83** are battle-only formes. Read `scope` in `data/roster.items.json`: in scope 148, tested **142**, differ 0, DID-NOT-FIRE 0, fixture gaps 6. Read `scope` in `data/roster.abilities.json`: total 316, out of scope **115** with no legal carrier, in scope 201, tested **139**, differ 0, DID-NOT-FIRE 0, fixture gaps 43, CONTROL-NOT-QUIET 14, deferred 5. Read `scope` in `data/roster.moves.json`: total 500, out of scope 2, in scope 498, tested **487**, differ 0, DID-NOT-FIRE 0, fixture gaps 8, deferred 3. Read `plant_anchors` in each. `dead` is empty in all three. `reds_ran` is true in all three. **RULE.** Do not write "unaccounted". Write the category.
