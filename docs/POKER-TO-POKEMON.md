...
**(b) Action space and horizon.** No-limit hold'em has a handful of actions per decision and ~4 betting rounds. A doubles turn has, per active Pokémon, (each legal move × each legal target) plus switches — easily dozens of joint choices — sustained over ~15–20 turns. The game tree is astronomically larger and deeper. This is *why the value network is not optional*: exact solving is hopeless, so depth-limited search with a learned leaf value (DeepStack/ReBeL) is mandatory, together with action abstraction (prune dominated moves, bucket targets).
