...
**Win% backtest — the hard finding (2026-07-23):** on 300 held-out real games, MEDICHAM's predicted P(win) **does not beat a coin**: log-loss 1.16 vs 0.69, and it picks the actual winner only **44% of decisive calls** (CI ~39–50%, i.e. at/below chance). Temperature calibration can't rescue it (best 0.75, still > coin). So **the win% is a matchup heuristic, NOT a validated game-outcome predictor**, and DITTO — which optimises it — cannot be said to produce outcome-optimal teams on this evidence. Partly the format's irreducible ceiling (team sheets don't determine games; JOLTEON found the same), partly a policy bias worth chasing. Harness: `engine/backtest_winrate.js`, report `data/winrate-backtest.json`. This is the review's compounding-GIGO concern, confirmed with data.
