...
**Policy validation (2026-07-23):** the behaviour-clone (the policy's backbone) predicts held-out human moves at ~~top-1 35.9% (CI 35.2–36.5), top-3 71.6%, cross-entropy 2.27 nats, baselines 4.54 / 2.91~~ — **withdrawn 2026-09-09: those figures are in no artifact.** `data/policy-eval.json` (`species_only_clone`, committed 2026-07-31) reads top-1 0.2979 [0.2914, 0.3045], top-3 0.6564, cross-entropy 2.6353 nats against baselines 4.7346 (global move frequency) and 3.0286 (uniform over moveset) — the clone beats both there too, so the priors carry signal, but human move choice has genuine entropy (the clone is a *modest* predictor). A phase-conditioning improvement was tried and did **not** beat the proper score, so it wasn't shipped. This is a conservative lower bound on the full policy (the KO-take/Protect overrides only raise agreement on those turns). Harness: `engine/eval_policy.py`, report `data/policy-eval.json`. **So MEDICHAM's win rate is `P(win | realistic cloned play)`, now with the clone's fidelity measured — not `P(win)` ground-truthed.**
...
**Honest status / result:** role-level factorization is the clean cut — recon-err **0.682** (`data/nmf-roles.json:archetype_recon_error`, 2026-08-04; ~~0.53~~ withdrawn 2026-09-09, an earlier run's value), six named archetypes (Intimidate+Fake-Out control; physical offense; special offense+sustain; **bulky wall+screens+redirection**; Tailwind+Encore; priority). Move-level is coarser (recon-err 0.79; attacking moves dominate). Rank and the human names are the only non-data choices, and the rank is NOT defended: `data/nmf-rank-selection.json` (bootstrap factor stability against a shuffled null) scores the shipped rank 6 at **−0.107** excess over null and selects rank 4 (+0.0775). Reconstruction error is not comparable across weightings and `engine/nmf_rank.py` states it cannot select a rank. ~~Topic coherence (Mimno 2011) is the noted next refinement~~ — deleted 2026-09-09: it has not been run, and *next* is not a justification.
