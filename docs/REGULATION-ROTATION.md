...
| **A tag keyed on one handler misses a member with the same effect in another handler.** `failsWithoutTerrain` reads Steel Roller's `onTry` + `onHit`; Ice Spinner clears the terrain from `onAfterHit` and has no `onTry`, so it matched nothing and the engine never removed a terrain for it -- in Reg M-B too, where it has 299 sheet uses (`data/tags.json:moves.icespinner.uses`; ~~301~~ before the 0.111.2 regeneration re-weighted usage from the stores) and every gate lattice read zero (no lattice dealt it into a standing terrain before the terrain ran out). | A Reg M-C card that is a shared-engine gap, invisible to every Reg M-B instrument. | When a card names an EFFECT (`clearTerrain`, `setTerrain`, `addSideCondition`), grep that call across every handler of every legal entity in both checkouts, not the handler the existing tag reads. Fixed 0.45.0 (`clearsTerrainAfterHit`). |
...
| **A REGISTER instrument can hardcode the old regulation too, and then an open row reads CANNOT-ANSWER in the new regulation's defect clause.** The 0.82.1 grep covered probes the gate stages, not the ROADMAP markers `engine/register_reality.js` runs. #310's probe forced the Reg M-B checkout and pinned a Reg M-B release; #442's forced the checkout and read the census as `data/mechanics-census.json`, so it found the old regulation's copy. 53 more files in `tests/` and `engine/` still default `SHOWDOWN_PATH` to the Reg M-B checkout, 17 of them named in a ROADMAP `VERIFIED BY:` cell. | "no open known engine defect" CANNOT-ANSWER under the new regulation, with the refusal naming a checkout, a release or a timestamp rather than the mechanic. | Grep every `VERIFIED BY:` script for literal checkout paths, release ids and unsuffixed artifact names; resolve through `engine/showdown_path.js`, the selected regulation's current release and `artifactFor`. Fixed 0.100.3 for #310 and #442. |
...
| **The frozen pool's stores are gitignored, so a worktree holds only its receipt.** `data/team-pool-frozen-regmc/*.jsonl` is ignored; a fresh worktree has `FROZEN.md` and `pool-receipt.json` and no games. | A lattice run there with `--write --out <gate path>` played 0 games, exited 1 with "THE ARM THIS RUN MEASURES PLAYED NO GAMES", and had already OVERWRITTEN the published gate artifact with the empty one (restored from git; 2026-09-27, `docs/_reports/2026-09-27-prankster-dark.md`). | Before any lattice in a worktree, hard-link the two stores from the main tree (`ln`, no bytes copied) and check both sha256 against `pool-receipt.json`. Do not pass the main tree's absolute path instead: the gate compares the RECORDED `--team-store` to `data/team-pool-frozen-regmc` and would read the sample as not this regulation's. |
| **A SOLVER test can be aimed at an engine defect.** `solver/tests/test-gates.js` ENCORE proved its `purposeresult` break red on a Prankster Encore into a Dark body, which reads "live" on the move result only because the engine's result was wrong there. | Fixing the result (abra/regmc 1.21.0) left that break GREEN: `test-gates` exit 3, BLIND, every other break still red. With `MEDI_PRANKSTER_RESULT_TRUE=1` it is exit 0 again, so the fix is the whole cause. | A downstream red demonstration must not depend on an open engine defect; when ENGINE closes one, run SOLVER's suites and route any BLIND clause to SOLVER in the same pass. |
