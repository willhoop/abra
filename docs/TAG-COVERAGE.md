...
| tag | uses |
|---|---|
| `resistBerry` | 13,134 *(`data/tags.json`, 18 entities, 2026-08-09 — the original 3.40.0 reading is in the snapshot table at the top of this file)* |
| `passiveHeal` | 8,363 |
| `blocksBerries` | 2,293 |
...
Two tags also appear to describe one mechanic — `contactPunish` (**since retired: zero entities carry it in `data/tags.json` today, merged into `punishesAttacker` at 11,751 uses, which is exactly what this paragraph argued for. Its original DEAD reading is in the snapshot table at the top**) beside
