...
function dmgRange(att,def,mv,field,spread,isCrit){
  stampMoveIds();
   ...
   *
   * TODAY THE CHAIN HAS EXACTLY ONE MEMBER, and `mdChain(v, ch4096(CH_ONE, m))` is byte-identical to
   * `md4096(v, m)` for one member, so this costs nothing and changes no number. It is written as a
   * chain because the OTHER `onBasePower` members -- Technician, Tough Claws, Sharpness, Iron Fist,
   * the eighteen type items, Helping Hand, the terrains -- are currently applied at the WRONG STAGE
   * (docs/DAMAGE-STAGES.md §2 measures Technician at 40.3% and Black Glasses at 65.0%), and the pass
   * that moves them belongs HERE, folded into `_bpChain`, not beside it. Leaving a bare `md4096` here
   * would have made the natural next edit a second spend, which is the bug one level down. */
  {
    let _bpChain=CH_ONE;
    ...
    if(_ae){
      ...
      if(Array.isArray(_m)&&_m.length===2){_bpChain=ch4096(_bpChain,_m);MEDSEEN.auraApplied++;}
    ...
    if(_bpChain!==CH_ONE)mvBP=Math.max(1,mdChain(mvBP,_bpChain));
  }
   ...
   *
   * THE x1.5 STAYS KEYED ON THE CERTAIN CRIT and NOT on `_critHere`, deliberately. A rolled crit's
   * 1.5 is applied by the battle loop at its own site (it is the only place that knows the die
   * landed), so making this line read `_critHere` would double it on exactly the rolled path. */
  ...
  if(TR)TR.turn(S.turn+1);
  {
    ...
    for(let actIdx=0;actIdx<acts.length;actIdx++){
      ...
      const _stepDamage=(R)=>{const tg=R.tg;
         ...
         * whether the crit landed after it had already priced the hit. The two hit-site multipliers
         * below (Friend Guard, Helping Hand) are not in dmgRange and never can be -- dmgRange is
         * handed two bodies and knows nothing about an ally or a boosted click -- so re-pricing with
         * the crit flag has to re-apply them, and doing that by copying the two blocks is exactly the
         * FACTS-ARE-GLOBAL failure CLAUDE.md names. One implementation, called with a boolean. */
        const _price=(isCrit)=>{
