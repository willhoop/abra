 ...
 * edit anywhere. */
const invSign=x=>TAGS.param('ability',x&&x.ability,'invertsBoosts')?-1:1;
...
function applyStatDrop(f,stat,n){
  ...
  if(TAGS.param('ability',ab,'invertsBoosts')){
    f.boosts[stat]=clamp(f.boosts[stat]+n,-6,6); return 'contrary';
  }
  f.boosts[stat]=clamp(f.boosts[stat]-n*(ab==='simple'?2:1),-6,6);
