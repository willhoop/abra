...
const TAGS = (function(){
  ...
  return {
    ...
    /* The WHOLE record, so a consumer can ask what ELSE an entity carries. ROADMAP #85 needs it:
...
const MEDSEEN = { flinch: 0, flinchBlockedByInnerFocus: 0, flinchTooLate: 0,
  ...
  /* ROADMAP #85 -- THE DAMAGE-STAGE CLASS, one counter per family that MOVED STAGE, because moving a
...
/* ROADMAP #85 -- THE ARTIFACT STORES A FLOAT AND THE AUTHORITY STORES A PAIR, AND FOR 1.3 THEY ARE
...
const CH_EXACT = { toughclaws:[5325,4096], sheerforce:[5325,4096], punkrock:[5325,4096],
                   transistor:[5325,4096], analytic:[5325,4096], sandforce:[5325,4096] };
...
/* ROADMAP #85 -- `hit` IS THE SEVENTH PARAMETER AND IT CARRIES THE FACTS ONLY THE HIT SITE KNOWS.
...
function dmgRange(att,def,mv,field,spread,isCrit,hit){
  ...
  /* ROADMAP #85 -- THE POWER HALF IS A CHAIN MEMBER, NOT ITS OWN FLOOR. `pixilate.onBasePower` is
   ...
   *
   * ROADMAP #85 -- AND IT IS ONE CHAIN PER STAT NOW, NOT ONE SPEND PER MEMBER. WIRE 4 got the
  ...
  /* ROADMAP #85 -- THICK FAT, HEATPROOF, PURIFYING SALT AND WATER BUBBLE MODIFY A **STAT**, AND WE
  ...
  /* ROADMAP #85 -- THE STAT-STAGE HALF OF `damageBoost`, AND THE MEMBERSHIP WAS PRINTED FIRST.
   *
   ...
   *
   * ROADMAP #85 -- AND THE CHAIN NOW HAS EVERY MEMBER, WHICH IS THE WHOLE OF THIS WIRE. What was
   ...
   * members reads as the untested case it is rather than as a pass. */
  {
    ...
    /* ROADMAP #85 -- THE FIELD TERRAINS THEMSELVES, ABSENT ENTIRELY UNTIL NOW. Not "in most games":
   ...
   *
   * ROADMAP #85 -- AND IT NOW READS `_critHere`, WHICH IS THE FIX. The certain-crit path was always
   ...
   *
   * ROADMAP #85 -- AND ALL THREE ROUTES HAVE LEFT THIS CHAIN, WHICH IS WHERE NONE OF THEM BELONGED.
  ...
  /* ROADMAP #85 -- SNIPER IS `onModifyDamage`, SO IT IS A MEMBER OF *THIS* CHAIN. It used to be
  ...
  /* ROADMAP #85 -- FRIEND GUARD (`onAnyModifyDamage`, 1,015 sheets) IS THE RIGHT STAGE AND WAS THE
  ...
  /* ROADMAP #85 -- ALL SIXTEEN ROLLS, ON REQUEST, IN THE AUTHORITY'S OWN INDEX ORDER. Index i is
  ...
  if(TR)TR.turn(S.turn+1);
  {
    ...
    for(let actIdx=0;actIdx<acts.length;actIdx++){
      ...
      const _stepDamage=(R)=>{const tg=R.tg;
         ...
         *
         * ROADMAP #85 -- THE TWO HIT-SITE MULTIPLIERS ARE NO LONGER APPLIED HERE. The comment that
        ...
        {const _cc=critChance(a.move.id,m,suppressedAbility(m,tg)),_cr=rng();   // WIRE 128 -- one owner
         if(_cc>0&&_cc<1&&_cr<_cc){
            ...
            *
            * ROADMAP #85 -- AND THE x1.5 ITSELF IS GONE FROM HERE, WHICH IS THE LAST HALF OF THE WIRE.
