...
/* ROADMAP #81 WIRE 7 -- THE DOLL IS CEIL, AND IT ANNOUNCES ITS SOURCE MOVE.
 *
 * `substitute`'s condition (data/moves.ts) is one line for both members and it does not round the way
 * the COST does:
 *     onStart(target, source, effect) {
 *       if (effect?.id === 'shedtail') this.add('-start', target, 'Substitute', '[from] move: Shed Tail');
 *       else                           this.add('-start', target, 'Substitute');
 *       this.effectState.hp = Math.ceil(target.maxhp / 4);
 * -- CEIL for the doll, while the cost is `directDamage(maxhp / 4)` which truncs. This engine floored
 * both, so every odd-HP body got a doll one point short: measured, a 183 HP Garchomp built 45 where the
 * authority builds 46, and a 145 HP body 36 against 37. One rule, both members, no name.
 *
...
function dmgRange(att,def,mv,field,spread){
  ...
  if(phys&&att.item==='choiceband')A=md4096(A,1.5);
  if(!phys&&att.item==='choicespecs')A=md4096(A,1.5);
  if(!phys&&def.item==='assaultvest')D=md4096(D,1.5);
  ...
  if(phys&&field.weather==='snow'&&def.types.includes('Ice'))D=md4096(D,1.5);
  if(!phys&&field.weather==='sand'&&def.types.includes('Rock'))D=md4096(D,1.5);
  ...
  if(attAb==='guts'&&phys&&att.status&&att.status!=='none')A=md4096(A,1.5);
  if(att.ability==='solarpower'&&!phys&&field.weather==='sun')A=md4096(A,1.5);
  if(att.ability==='orichalcumpulse'&&phys&&field.weather==='sun')A=md4096(A,[5461,4096]);
  if(att.ability==='hadronengine'&&!phys&&terrainId(field.terrain)==='electric')A=md4096(A,[5461,4096]);
  ...
  {const _cs=TAGS.param('ability',defAb,'condStatMult');
   if(_cs&&_cs.mult&&_cs.when==='statused'&&def.status&&def.status!=='none'){
     if(_cs.stat===(phys?'def':'spd'))D=md4096(D,+_cs.mult);
   }}
  // Ruin abilities lower everyone-else's stat (field-wide; handled pairwise)
  if(phys&&att.ability==='swordofruin')D=md4096(D,0.75);
  if(!phys&&att.ability==='beadsofruin')D=md4096(D,0.75);
  if(phys&&defAb==='tabletsofruin')A=md4096(A,0.75);
  if(!phys&&defAb==='vesselofruin')A=md4096(A,0.75);
  ...
  if(_bf&&att._fallenStuck)base=md4096(base,1+_bf.perFallen*Math.min(att._fallenStuck,_bf.max));
   ...
   * defensive read fire on the attacker's ability. */
  {
    ...
    if(_htd){
      if(_htd.types&&_htd.types.indexOf(mvT)>=0&&_htd.attackerStatMult)MODMUL(_htd.attackerStatMult);
      if(_htd.basePowerTypes&&_htd.basePowerTypes.indexOf(mvT)>=0&&_htd.basePowerMult)MODMUL(_htd.basePowerMult);
    }
  }
  if(att.ability==='waterbubble'&&mvT==='Water')MODMUL(2);
  ...
  if(_ty&&_ty.onType===mvT&&_ty.mult)MODMUL(_ty.mult);
  ...
  if(att.item==='muscleband'&&phys)MODMUL(1.1);
  if(att.item==='wiseglasses'&&!phys)MODMUL(1.1);
...
function applyStatDrop(f,stat,n,eff){
...
function applyEntryDrops(m,foes){
  ...
  for(const f of foes){
    ...
    for(const k in _bo){const _s=SD2ENG[k];if(_s&&_bo[k]<0)applyStatDrop(f,_s,-_bo[k],m.ability);}
  }
}
...
function bringIn(act,i,bench,foes,sf,field,wanted){
  ...
  if(sf&&sf.hz){
    ...
    if(sf.hz.stickyweb&&_grounded&&nx.curHP>0){
      ...
      applyStatDrop(nx,'sp',1,'Sticky Web');MEDSEEN.hazardResolvedOnEntry++;
    }
    ...
    for(let actIdx=0;actIdx<acts.length;actIdx++){
      ...
      if(a.kind!=='sub'&&(a.mv||(a.move&&a.move.id))){
        ...
        if(_cu&&_cu.costsFraction&&m.st){
          ...
          if(m.curHP<=Math.floor(m.st.hp*(+_cu.failsBelow||+_cu.costsFraction))){m._lastMove=a.mv||a.move.id;mvFail(m);continue;}
      ...
      if(a.kind==='status'){
        ...
        if(!applyStatus(t,st)){m._mvRes=false;if(TR)TR.fail(t);}                    // applyStatus enforces the immunities
        continue;
      }
      ...
      /* WIRE 47 / WIRE 129 -- CRASH ON MISS. High Jump Kick, Axe Kick and Supercell Slam (209 uses)
         ...
         all. The fraction is the tag's, not a number typed here.
      ...
      if(_mvAcc<100&&rng()*100>_mvAcc){
         ...
         * This engine wrote ONE line with an EMPTY target field, because `_accDef` is deliberately
         * null for a spread move -- three protocol fields where the authority writes four.
         *
         * THE DECLARED DIVERGENCE ABOVE IS UNCHANGED AND IS NOT WIDENED HERE: the ROLL is still one
         * roll for the whole move. What changes is the ANNOUNCEMENT, which is a statement about which
         * bodies the move failed to reach -- and with a single roll that is all of them. An engine
         * rolling per target could miss one and hit the other; this one cannot, and says so above
         * rather than pretending otherwise by emitting a partial list.
         *
         * `targets` is the post-shield list and already includes the ally for a `spreadAll` move,
         * which is right: Showdown rolls accuracy against the partner an Earthquake is about to hit
         * and announces that miss too. */
        ...
        /* ROADMAP #84 -- A MISS COUNTS. `hitStepAccuracy` returns literal `false` for the target it
           ...
           false. This is the case Stomping Tantrum is actually played for. */
        m._mvRes=false;
        _crashOnFail();
        continue;
      }
      ...
      for(const tg of targets){if(!tg||tg.fainted)continue;
        ...
        if(tg._invuln){_explicitFail=true;if(TR)TR.miss(m,tg);continue;}
           ...
           looking it up from `it.side` would then read the wrong pair. */
        {
          ...
          if(_fg&&_fg.mult&&d&&(d.min||d.max))
            d={min:md4096(d.min,+_fg.mult),max:md4096(d.max,+_fg.mult),eff:d.eff};
        }
